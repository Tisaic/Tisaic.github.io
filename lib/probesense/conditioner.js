...
export class InputConditioner {
  ...
  constructor({ nSignals, groups = [], lp = 1, cmr = false, warmup = 120 }) {
    ...
    this._ring = [];
    this._at = -1;
    // A CONFIGURATION THAT FILTERS NOTHING MUST NOT COST A WARMUP. The calibration
    // exists only for the rejection's per-channel units; with no rejection and no
    // averaging there is nothing to calibrate, and imposing a delay anyway would
    // make "conditioning off" behave differently from no conditioner at all.
    if (this.lp <= 1 && !this.cmr) {
      this.frozen = true;
      this.std.fill(1);
    }
  }
