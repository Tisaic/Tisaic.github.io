...
export class SoftSensorBank {
  ...
  restartCalibration() {
    ...
    this._sum.fill(0); this._sq.fill(0); this._cnt = 0;
    return true;
  }
    ...
    if (!this.frozen) {
      ...
      if (this._cnt >= this.warmup) {
        ...
        for (let i = 0; i < this.base; i++) {
          ...
          this.fmean[i] = mu;
          sd[i] = Math.sqrt(Math.max(0, this._sq[i] / this._cnt - mu * mu));
        }
...
export class FieldSoftSensor {
  ...
  _makeBank() {
    ...
    return new SoftSensorBank({
      nSignals: c.inputs.length, lag: c.lag, stride: c.stride, warmup: c.warmup,
  ...
  _recalibrateIfUnrepresentative() {
    if (!this.bank.calibrated() || this.recalibrations >= 3) return false;
    const since = this.bank.nPushed - this.bank.depth - this.cfg.warmup;
    if (since < Math.max(40, this.cfg.warmup >> 1)) return false;
    const slots = since * this.bank.base;
    if (this.bank.clamped / Math.max(1, slots) <= 0.02) { this._settled = true; return false; }
    this.recalibrations++;
    this.bank = this._makeBank();
    this._tSum = 0; this._tSq = 0; this._tCnt = 0; this._tFrozen = false;
  ...
  sample(step, sensorRead, targetRead) {
    ...
    if (this.mode === 'training' && !this._settled && this._recalibrateIfUnrepresentative()) {
  ...
  status() {
    ...
    // asked nothing.
    const mean = this.estScore.n ? this.estScore.sy / this.estScore.n : 0;
