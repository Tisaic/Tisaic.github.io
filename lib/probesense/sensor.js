...
export class FieldReconstructor {
  ...
  constructor({ nSignals, nLocations, lag, stride, warmup = 200, expand = true,
    ...
    this.samples = 0;
    // THE DO-NOTHING RIVAL, ACCUMULATED ALONGSIDE. Each location's own running time
    // average, using no sensor at all. It is here rather than in the caller because
    // a reconstruction reported without it is not a result: on a steady field the
    // static map scores 1.7e-3 while this model scored 0.086, i.e. FIFTY TIMES
    // WORSE THAN DOING NOTHING, and the gate that was supposed to catch that asked
    // only "is the error small". A small number is not a result.
    this._smMu = new Float64Array(this.K);
    this._smM2 = new Float64Array(this.K);
    this._smN = 0;
  }
  ...
  /** Each location's time average over the training window: the do-nothing rival. */
  staticMap() { return this._smMu; }

  /**
   * Grade an estimate against the truth AND against doing nothing.
   *
   * `ratio` is the number that means something: below 1 the sensors are not
   * earning their place, whatever the error looks like on its own. `activity` is
   * the prior question -- how much a location moves in TIME against how much the
   * field varies in SPACE, which is what nRMSE is normalised by. Below about a
   * percent there is nothing to infer and the task has collapsed into recall, so
   * every score becomes noise divided by noise. That is the same question the
   * physics side of this project asks of a wake before scoring a forecast on it.
   * @param {number[]} truths @param {number[]} est
   */
  grade(truths, est) {
    const K = this.K;
    let m = 0;
    for (let k = 0; k < K; k++) m += truths[k];
    m /= K;
    let sd = 0, se = 0, sb = 0;
    for (let k = 0; k < K; k++) {
      sd += (truths[k] - m) ** 2;
      if (est) se += (est[k] - truths[k]) ** 2;
      sb += (this._smMu[k] - truths[k]) ** 2;
    }
    const den = Math.max(sd, 1e-30);
    const model = est ? Math.sqrt(se / den) : null;
    const staticMap = this._smN > 1 ? Math.sqrt(sb / den) : null;
    let temporal = 0;
    if (this._smN > 1) {
      for (let k = 0; k < K; k++) temporal += Math.sqrt(this._smM2[k] / this._smN);
      temporal /= K;
    }
    const spatial = Math.sqrt(sd / K);
    return { model, staticMap,
      ratio: (model && staticMap) ? staticMap / model : null,
      activity: spatial > 0 ? temporal / spatial : null };
  }
  ...
  observe(truths) {
    const col = this.bank._column(0);
    if (!col) return null;
    // The rival is given exactly the history the model is, and simply never looks
    // at a sensor. Updated before the model is graded so both see the same window.
    this._smN++;
    for (let k = 0; k < this.K; k++) {
      const d = truths[k] - this._smMu[k];
      this._smMu[k] += d / this._smN;
      this._smM2[k] += d * (truths[k] - this._smMu[k]);
    }
