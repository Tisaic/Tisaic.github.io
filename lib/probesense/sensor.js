...
export class FieldReconstructor {
  ...
  constructor({ nSignals, nLocations, lag, stride, warmup = 200, expand = true,
    ...
    this._tSum = new Float64Array(this.K); this._tSq = new Float64Array(this.K);
  ...
  observe(truths) {
    ...
    if (!this._tFrozen) {
      for (let k = 0; k < this.K; k++) { this._tSum[k] += truths[k]; this._tSq[k] += truths[k] * truths[k]; }
      this._tCnt++;
      if (this._tCnt >= this.bank.warmup) {
        for (let k = 0; k < this.K; k++) {
          const mu = this._tSum[k] / this._tCnt;
          const v = this._tSq[k] / this._tCnt - mu * mu;
          this._tMean[k] = mu; this._tStd[k] = v > 1e-30 ? Math.sqrt(v) : 1;
        }
        this._tFrozen = true;
      }
    }
