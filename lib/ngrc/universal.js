...
/**
 * Full structured-prior variance vector (four levels: linear/bias `lin`,
 * quadratic `quad`, random ReLU+Fourier `rand`, reciprocal `recip`). Mirrors
 * `UniversalMap.prior`. `1/variance` is the ridge on that weight.
 * @param {number} base @param {number} nh @param {number} nf
 * @param {PriorOpts} [opts]
 * @returns {number[]}
 */
export function universalPrior(base, nh, nf, opts = {}) {
  const { lin = 100.0, quad = 1.0, rand = 0.001, nRecip = 0, recip = 1.0 } = opts;
  const quadLen = Math.floor(base * (base + 1) / 2);
  const prior = [];
  for (let i = 0; i < 1 + base; i++) prior.push(lin);
