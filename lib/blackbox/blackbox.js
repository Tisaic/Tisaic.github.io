...
export class BlackBox {
  ...
  constructor(o) {
    const { ref, sampleEvery, probeSamples = 900, probeAmp = 0,
      ...
      sliceBudget = true, mpcIters = null, correctionLimit = 0 } = o;
  ...
  _design() {
    ...
    const uCap = Math.min(
      ...
      this.correctionLimit > 0 ? this.correctionLimit : Infinity);
    ...
    const mpcCands = [];
    {
      ...
      for (const horizon of [24, 48]) {
        ...
        for (const iters of [1, 2, 4, 8, 16]) {
          if (iters > most) break;
          for (const lambda of [1e-1, 1e-2, 1e-3]) {
            const cfg = { h: hc, horizon, U: uCap, lambda, iters };
    ...
    if (mpcCands.length) {
      ...
      if (bestGain > 0) {
        ...
        bestMpc = mpcCands.filter((c) => base - c.res >= 0.95 * bestGain)
  ...
  cost() {
    ...
    const mac = m.mac + (this.mpc ? this.mpc.cost().mac : taps);
  ...
  _correct(k) {
    ...
    if (this.mpc) { this.u = this._mpcStep(k); return; }
  ...
  _applyTrial(i, k0) {
    ...
    if (t.kind === 'mpc') {
      this.mpcCfg = { ...this.mpcAlt, U: this.mpcBaseU * t.scale };
  ...
  _verify(k, truth) {
    ...
    if (this.mpc) {
      eh = this.mpcWin[0];
      this.u = this._mpcStep(k);
    } else {
      this.eHead = (this.eHead + 1) % L;
      this.eRing[this.eHead] = this.map.predict(this.ref, k + this.qCentre * this.grid,
        this.grid);
      let sq = 0;
      for (let j = 0; j < L; j++) sq += this.q[j] * this.eRing[(this.eHead - j + L) % L];
      this.u = sq;
      eh = this.eRing[(this.eHead - this.qCentre + L) % L];
    }
    this.i++;
    ...
    this.note = `on the machine: ${t.kind} ×${t.scale} `
    ...
    this.vResults.push({ kind: t.kind, scale: t.scale, ratio: got });
    ...
    if (best.ratio < 1.02) {
      ...
      this._applyTrial(this.vScales.indexOf(
        this.vScales.find((c) => c.kind === best.kind && c.scale === best.scale)), k);
      if (best.kind === 'mpc') {
