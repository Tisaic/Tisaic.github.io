...
export class BlackBox {
  ...
  _verify(k, truth) {
    ...
    this.note = `on the machine: ${t.kind}${t.gentle ? '-gentle' : ''} ×${t.scale} `
    ...
    this.vResults.push({ kind: t.kind, scale: t.scale, gentle: !!t.gentle, ratio: got,
    ...
    const bestRatio = Math.max(...this.vResults.map((r) => r.ratio));
    const best = this.vResults.filter((r) => r.ratio >= 0.95 * bestRatio)
    ...
    if (best.ratio < 1.02) {
      ...
      this._applyTrial(this.vScales.indexOf(this.vScales.find((c) =>
        c.kind === best.kind && c.scale === best.scale
