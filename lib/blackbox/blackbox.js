...
export class BlackBox {
  ...
  _identify() {
    ...
    // move it was supposed to explain.
    //
    // IT COST THE WHOLE OF THE BLACK BOX'S SHORTFALL, and every symptom pointed
    // elsewhere. The map explained 50% of the disturbance out of sample, so the design
    // honestly predicted ~1.4x and honestly achieved 1.07x; the identification was
    // excellent; the prediction matched the achievement. Nothing looked broken -- it
    // looked like the price of knowing nothing, and I said so. What found it was asking a
    // different question: a crude 96-bin PHASE-INDEXED average explained 100% of the same
    // disturbance, which proves it is perfectly predictable and therefore that the map
    // was failing rather than the problem being hard. With the taps in steps the same map
    // explains 100.0% too. A wrong unit is not a modelling limit, and the difference is
    // only visible against something that does not share the mistake.
