...
/**
 * GEOMETRIC BLOCK SIZES, fine at the front and coarse at the back.
 *
 * A receding horizon applies `u[0]` and re-decides everything else on the next update, so the
 * far end of the plan is computed at full resolution and thrown away. Blocking holds `u`
 * constant over spans instead — and the spans are geometric so the FIRST decision is its own
 * block and is not constrained at all, which is the one that reaches the machine.
 *
 * @param {number} N horizon
 * @param {number} growth how fast the blocks widen (1.6 is fine near, coarse by the far end)
 * @returns {number[]} block lengths summing to exactly N
 */
export function geometricBlocks(N, growth = 1.6) {
  const out = [];
  let len = 1, total = 0;
  while (total < N) {
    const take = Math.min(Math.round(len), N - total);
    out.push(take); total += take; len *= growth;
  }
  return out;
}
...
export function boxQP(h, f0, u, { U, lambda, uPrev = 0, iters = 8, notch = null,
  blocks = null }) {
  const N = u.length;
  if (blocks) return blockedQP(h, f0, u, { U, lambda, uPrev, iters, blocks });
...
/**
 * MOVE BLOCKING — the same problem solved over spans instead of samples, and the reason the
 * deployed arithmetic can come down by most of an order of magnitude.
 *
 * WHERE THE COST IS. Measured on the EMPS model at the shipped setting: the QP is 89% of the
 * deployed 42,914 MAC/cycle, and it is `2*N*min(N,M)` per iteration because `T` is a full
 * lower-triangular Toeplitz — every iteration convolves the whole horizon, twice.
 *
 * WHY IT IS MOSTLY WASTE. A receding horizon applies `u[0]` and nothing else; the rest of the
 * plan is re-decided on the next update. So the far end is computed at full resolution and
 * discarded, every cycle, for ever.
 *
 * THE PARAMETERISATION IS CHOSEN TO KEEP THE PROJECTION A CLAMP, which is the property the
 * top of this file says makes the solver cyclic-task-safe. Blocks are DISJOINT and `u` is
 * constant on each, so `|u_i| <= U` is exactly `|z_b| <= U`: no active-set bookkeeping, no
 * factorisation, no data-dependent run time. A smooth basis — Laguerre, polynomial — would
 * have destroyed that for the same variable count.
 *
 * AND THE PLANT RESPONSE TO A BLOCK IS PRECOMPUTED. `T e_b` is a partial sum of `h` and does
 * not depend on `z`, so the `N x nb` matrix `C = [T e_0 ... T e_nb-1]` is built ONCE per
 * (h, blocks) and cached beside the workspace. Each iteration is then `C z` and `C' r` —
 * `2*N*nb` instead of `2*N*N`. At N 68 and nb 8 that is 1,088 against 9,520.
 *
 * THE INCREMENT PENALTY COMES ALONG FOR FREE. `D u` is zero inside a block and jumps only at
 * boundaries, so `||D u||^2` is `nb` terms rather than `N`.
 *
 * THE STEP SIZE IS MEASURED, NOT BOUNDED CRUDELY. `test/pilot/rti.test.mjs` measured that the
 * `(sum|h|)^2` bound is 1.8x above the true spectral norm and costs exactly 2x in iterations,
 * so here the largest eigenvalue of `C'C` is taken by power iteration at BUILD time — twenty
 * products on an `N x nb` matrix, once, and free at run time.
 */
function blockedQP(h, f0, u, { U, lambda, uPrev, iters, blocks }) {
  const N = u.length, M = h.length, nb = blocks.length;
  const key = `${N}:${blocks.join(',')}`;
  let w = WS.get(key);
  if (!w || w.hRef !== h) {
    // C[b] holds the horizon's response to a unit hold over block b.
    const start = []; let acc = 0;
    for (const b of blocks) { start.push(acc); acc += b; }
    const C = [];
    for (let b = 0; b < nb; b++) {
      const col = new Float64Array(N);
      for (let i = 0; i < N; i++) {
        let s = 0;
        for (let j = start[b]; j < start[b] + blocks[b] && j <= i; j++) {
          const m = i - j;
          if (m < M) s += h[m];
        }
        col[i] = s;
    ...
    // The largest eigenvalue of C'C, by power iteration. Twenty products, once.
    let v = new Float64Array(nb).fill(1 / Math.sqrt(nb));
    const tmp = new Float64Array(N), gv = new Float64Array(nb);
    let lam = 0;
    for (let t = 0; t < 20; t++) {
      tmp.fill(0);
      for (let b = 0; b < nb; b++) { const c = C[b], z = v[b]; for (let i = 0; i < N; i++) tmp[i] += c[i] * z; }
      for (let b = 0; b < nb; b++) { const c = C[b]; let s = 0; for (let i = 0; i < N; i++) s += c[i] * tmp[i]; gv[b] = s; }
      let nrm = 0; for (let b = 0; b < nb; b++) nrm += gv[b] * gv[b];
      nrm = Math.sqrt(nrm); if (!(nrm > 0)) break;
      lam = nrm; for (let b = 0; b < nb; b++) v[b] = gv[b] / nrm;
    }
    w = { hRef: h, C, start, blocks, lam: lam * 1.02,
      z: new Float64Array(nb), y: new Float64Array(nb), zOld: new Float64Array(nb),
      g: new Float64Array(nb), r: new Float64Array(N) };
    WS.set(key, w);
  }
  const { C, start, lam } = w;
  const z = w.z, y = w.y, zOld = w.zOld, g = w.g, r = w.r;
  // WARM START FROM THE PLAN ALREADY IN `u`, one sample per block, so a shifted previous
  // solution still seeds this one exactly as it does the unblocked solver.
  for (let b = 0; b < nb; b++) z[b] = u[start[b]];
  y.set(z);
  const L = (2 * (lam + 4 * lambda)) || 1;
  const step = 1 / L;
  let t = 1;
  for (let it = 0; it < iters; it++) {
    r.set(f0);
    for (let b = 0; b < nb; b++) { const c = C[b], zb = y[b]; for (let i = 0; i < N; i++) r[i] += c[i] * zb; }
    for (let b = 0; b < nb; b++) {
      const c = C[b];
      let s = 0;
      for (let i = 0; i < N; i++) s += c[i] * r[i];
      const back = b > 0 ? y[b - 1] : uPrev;
      const fwd = b < nb - 1 ? y[b + 1] : y[b];
      g[b] = 2 * s + 2 * lambda * ((y[b] - back) - (fwd - y[b]));
    }
    zOld.set(z);
    for (let b = 0; b < nb; b++) {
      let v2 = y[b] - step * g[b];
      if (v2 > U) v2 = U; else if (v2 < -U) v2 = -U;
      z[b] = v2;
    }
    const tN = (1 + Math.sqrt(1 + 4 * t * t)) / 2, beta = (t - 1) / tN;
    for (let b = 0; b < nb; b++) y[b] = z[b] + beta * (z[b] - zOld[b]);
    t = tN;
  }
  // Expand back, because every caller reads `u[0]` and shifts `u` for the next warm start.
  for (let b = 0; b < nb; b++) {
    for (let i = start[b]; i < start[b] + w.blocks[b] && i < N; i++) u[i] = z[b];
  }
  return u;
}
