...
export class Pilot {
  ...
  _finishProbe() {
    ...
    const band = Math.max(0.02 * Math.abs(dc), 4 * noise / p.amp);
    let Tset = 0;
    for (let i = 0; i < resp.length; i++) {
      if (Math.abs(resp[i] - dc) > band) Tset = i;
    }
    if (!(Tset > 0)) Tset = Math.round(0.5 * resp.length);
    // AND THE RISE IS KEPT SEPARATELY, because the two answer different questions and
    // making one number serve both was measured to cost a deploy: the SETTLE says how
    // far ahead a plan must reach, the RISE says how fast the loop has to be sampled.
    // Driving the sample rate from the settle coarsened it 8 → 14 steps at the softest
    // gearbox and the pilot's own verify fell 4.62x → 0.99x — REFUSED. Horizon from the
    // settle alone, every other cadence from the rise, and both corners improve.
    let Ts = resp.length;
    for (let i = 0; i < resp.length; i++) {
    ...
    let rings = 0;
    for (let i = Math.round(0.5 * Ts); i < resp.length - 1; i++) {
      if ((resp[i] - dc) * (resp[i + 1] - dc) < 0) rings++;
    }
    this.hs.push({ resp, dc, noise, Ts, Tset, rings, cross: nCross,
    ...
    this.Tset = Math.max(this.Ts, Math.max(...this.hs.map((h) => h.Tset), 200));
  ...
  _startExcite() {
    ...
    try {
      series = buildExcitation({ channels: this.channels, steps,
        ...
        dwell: !!dwellOpt,
        chirpBand: this.hs.some((h) => h.rings >= 2)
  ...
  status() {
    return { phase: this.phase, note: this.note, k: this.k, sample: this.sample,
      Ts: this.Ts, Tset: this.Tset, rings: this.hs.map((h) => h.rings), grid: this.grid, N: this.N, lambda: this.lambda,
