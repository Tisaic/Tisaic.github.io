...
export class Pilot {
  ...
  constructor({ nMeasured, channels, uMax, start, guards = [], workspace = null, seed = 1,
    exciteSteps = null, verifySegLen = null, dwell = false }) {
  ...
  work() {
    ...
    if (F.stage === 'tune') {
      ...
      if (F.ci >= F.combos.length) {
        ...
        F.chosen.push({ stride: nearP[0].stride, ridge: nearP[0].ridge, r2: nearP[0].r2,
          poly: usePoly, linR2: bestLin, polyR2: bestPoly });
        F.ci = 0; F.ch++;
    ...
    const { X, y } = this._buildXY(c, L, chosen.stride, 1, chosen.poly);
  ...
  _finishFits() {
    ...
    this.report.readouts = this.readouts.map((r, c) => ({
      ...
      basis: r.poly ? 'linear+quadratic' : 'linear',
  ...
  _startVerify() {
    ...
    const plan = [...half.map((p) => ({ ...p, regime: 'scribble' })),
      ...half.map((p) => ({ ...p, regime: 'program' }))];
    ...
    try {
      prog = buildProgram({
        channels: this.channels.map((c) => ({ ...c,
          vMax: c.vMax / 4, aMax: c.aMax / 4, jMax: c.jMax / 4 })),
        steps,
        start: this.channels.map((_, c) => series.pos[c][steps - 1]),
        workspace: this.workspace, seed: this.seed + 907 });
    } catch (e) {
      this.phase = 'done';
      this.verdict = { deploy: false, why: 'verify (program regime): ' + e.message };
      return;
    }
    // The two regimes are ONE commanded sequence, the scribble starting where the
    // program left off, so the join costs no rate-limit violation and no re-settle.
    const joined = this.channels.map((_, c) => {
      const out = new Float64Array(2 * steps);
      out.set(series.pos[c], 0);
      out.set(prog.pos[c], steps);
      return out;
    });
    this.report.verifyRegimes = { tRamp: prog.meta.tRamp, moves: prog.meta.moves,
      tc: series.meta.tc, program: prog.meta.measured, scribble: series.meta.measured };
    this._ver = { exc: { series: { pos: joined, total: 2 * steps, ramp: series.ramp },
      k0: this.k, steps: 2 * steps },
  ...
  _verifyStep(measured, truth) {
    ...
    const h = Math.min(1, Math.floor(i / V.halfLen)), j = i - h * V.halfLen - V.pad;
  ...
  _finishVerify() {
    ...
    if (!identifiable) {
      ...
      this.verdict = { deploy: false,
        why: `the verify round measured ${ratio.toFixed(2)}x against doing nothing on the `
          + `${worst.name} regime (${ratios.map((r) => `${r.name} ${r.ratio.toFixed(2)}x`).join(', ')}) — `
          + 'this pilot does not deploy a controller the machine has not vouched for' };
    } else {
      this.verdict = { deploy: true,
        why: `verified ${ratio.toFixed(2)}x on the machine, worst of `
  ...
  status() {
    return { phase: this.phase, note: this.note, k: this.k, sample: this.sample,
      Ts: this.Ts, Tset: this.Tset, rings: this.hs.map((h) => h.rings), overshoot: this.hs.map((h) => h.overshoot), grid: this.grid, N: this.N, lambda: this.lambda,
