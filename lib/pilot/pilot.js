...
export class Pilot {
  ...
  constructor({ nMeasured, channels, uMax, start, guards = [], workspace = null, seed = 1,
     ...
     *
     * `{ mu, clamp, freezeAfter }` — the step, a bound on the per-update weight change as a
     * fraction of the weight's own magnitude, and how many consecutive worsening windows end
     * the adaptation. Default off, so every plant on record is untouched.
     */
    this.adapt = adapt ? { mu: 0.02, clamp: 0.05, freezeAfter: 3, ...adapt } : null;
  ...
  _adaptStep(c, ro, entry, truth) {
    ...
    for (let j = 0; j < w.length; j++) {
      const d = g * row[j];
      const lim = A.clamp * (Math.abs(w[j]) + 1e-12);
      w[j] += Math.max(-lim, Math.min(lim, d));
    }
    st.n++;
  ...
  _deployObserve(measured, truth = null) {
    ...
    if (this.k % this.sample === 0) {
      this._run.ring.push(measured.slice());
      this._run.kSamp++;
      // CONTINUOUS RE-IDENTIFICATION, when and only when a truth is supplied.
      if (this.adapt && truth) {
        for (let c = 0; c < this.nc; c++) {
  ...
  _controlTick(active, lookAhead, deployed = false) {
    ...
    for (let c = 0; c < this.nc; c++) {
      ...
      for (let i = 0; i < this.N; i++) {
        ...
        if (this.adapt && deployed && i === 0) aRow = this._captureRow(ro, R, lookAhead, L, deployed);
        ...
        if (this.adapt && deployed && i === 0) { aConv = conv; aLead = L; }
        R.f0[i] = s2;
      }
      if (aRow) {
        ro._aQ = ro._aQ || [];
        ro._aQ.push({ row: aRow, conv: aConv, due: this._run.kSamp + Math.round(aLead) });
        if (ro._aQ.length > 4096) ro._aQ.splice(0, 2048);
      }
