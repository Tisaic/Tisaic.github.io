...
export class Pilot {
  ...
  constructor({ nMeasured, channels, uMax, start, guards = [], workspace = null, seed = 1,
    ...
    sampleFixed = null, gateForecasts = true, leadTrust = false }) {
  ...
  _startExcite() {
    ...
    try {
      series = buildExcitation({ channels: this.channels, steps,
        ...
        dwell: !!dwellOpt,
        chirpBand: this.hs.some((h) => h.rings >= 2 && h.overshoot > 1.05)
  ...
  _row(c, k, L, stride, poly = false, ML = null, FL = null) {
    ...
    for (let ch = 0; ch < this.nc; ch++) {
      for (let l = 0; l < fL; l++) {
        ...
        if (l === 0) nl.push(p0, (p0 - p1) * (this.Ts / this.sample));
      }
    }
  ...
  work() {
    ...
    if (F.stage === 'window') {
      ...
      F.wscores.push({ ch: c, lag: cand, ridge: ch.ridge, r2: acc / leads.length });
      F.wi++;
