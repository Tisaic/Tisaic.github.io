...
export class Pilot {
  ...
  constructor({ nMeasured, channels, uMax, start, guards = [], workspace = null, seed = 1,
     ...
     *
     * AND THE OBVIOUS USE OF THIS KNOB IS THE WRONG ONE. Measured on that arm, sharp
     * square, contour rms against the pilot's own open loop:
     *
     ...
     *
     * EXCITING AT THE NATURAL FREQUENCY COSTS 26%, and raises the oscillation it was aimed
     * at. The excitation's amplitude is bounded by the machine's own rate limits, so energy
     * spent at the mode is energy taken from the SLOW band -- and the slow band is where
     * the BIAS lives, which is the part a correction can actually remove. Bias tracks the
     * score monotonically across all four rows; the oscillation barely moves except to get
     * worse. The ringing is not a modelling deficiency: telling the model about the
     * resonance does not help it suppress the resonance.
     *
     * So the band that pays is the one ABOVE the mode, near the settling time -- which is
     * what the default already computes. This knob exists to make that statement
     * falsifiable, and to serve a plant where the engineer knows the probe under-reports.
     */
  ...
  _row(c, k, L, stride, poly = false, ML = null, FL = null) {
    ...
    if (poly) { row.nBase = row.length; polyTerms(nl, row); } else row.nBase = row.length;
    return row;
  }
  ...
  work() {
    ...
    if (F.stage === 'tune') {
      ...
      F.scores.push({ ...combo, ch: F.ch, r2: acc / leads.length, poly: false });
      F.scores.push({ ...combo, ch: F.ch, r2: accP / leads.length, poly: true });
      F.ci++;
      if (F.ci >= F.combos.length) {
        ...
        const bestLin = Math.max(...mine.filter((s2) => !s2.poly).map((s2) => s2.r2));
        const bestPoly = Math.max(...mine.filter((s2) => s2.poly).map((s2) => s2.r2));
        const usePoly = (1 - bestPoly) < 0.95 * (1 - bestLin);
        const pick = mine.filter((s2) => s2.poly === usePoly);
        ...
        F.chosen.push({ stride: nearP[0].stride, ridge: nearP[0].ridge, r2: nearP[0].r2,
          poly: usePoly, linR2: bestLin, polyR2: bestPoly, mLag: 12, fLag: 12 });
        F.ci = 0; F.ch++;
    ...
    if (F.stage === 'window') {
      ...
      for (const L of leads) {
        const { X, y } = this._buildXY(c, L, ch.stride, 2, ch.poly, cand, cand);
        if (!X.length) { usable = false; break; }
        acc += this._blockSplit(X, y, ch.ridge, ch.poly ? this._colScale(X[0]) : null).r2;
      }
    ...
    const { X, y } = this._buildXY(c, L, chosen.stride, 1, chosen.poly, chosen.mLag, chosen.fLag);
    const v = this._blockSplit(X, y, chosen.ridge, chosen.poly ? this._colScale(X[0]) : null);
  ...
  _finishFits() {
    ...
    this.report.readouts = this.readouts.map((r, c) => ({
      ...
      basis: r.poly ? 'linear+quadratic' : 'linear', lags: r.mLag,
      lagScores: F.chosen[c].lagScores,
      r2Lin: F.chosen[c].linR2, r2Poly: F.chosen[c].polyR2,
