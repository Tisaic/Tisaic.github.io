...
export class Pilot {
  ...
  constructor({ nMeasured, channels, uMax, start, guards = [], workspace = null, seed = 1,
    ...
    notchPeriod = null, notchWeight = 0, decisionsPerTs = 30, qpIters = 60,
    adapt = null,
  ...
  _finishProbe() {
    ...
    this.N = Math.max(8, Math.ceil(1.5 * this.Tset / this.sample / this.grid));
    ...
    for (let c = 0; c < this.nc; c++) {
      ...
      for (let li = 0; li < lambdas.length; li++) {
        ...
        for (let t = 0; from + t * this.grid < to; t++) {
          ...
          boxQP(hg, f0, warm, { U: this.uMax, lambda: lambdas[li], uPrev, iters: 60 });
          uPlan.push(warm[0]);
   ...
   * every scan between updates. Fitting is not counted and must not be: it happens during
   * commissioning, and the whole point of freezing is that the deployed path does not fit.
   */
  cost() {
    ...
    for (const ro of this.readouts) {
      ...
      const nf = ro.w[0].length, leads = ro.w.length;
    ...
    return { perUpdate, perStep, features: feat, dots, qp, channels: this.nc,
