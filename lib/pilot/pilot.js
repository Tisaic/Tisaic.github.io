...
export class Pilot {
  ...
  _finishProbe() {
    ...
    for (const h of this.hs) {
      ...
      for (let m = 0; m < this.N; m++) {
        hg[m] = s[Math.min((m + 1) * this.grid, s.length - 1)] - s[m * this.grid];
      }
      h.hGrid = hg;
  ...
  _startExcite() {
    ...
    const series = buildExcitation({ channels: this.channels, steps,
  ...
  work() {
    ...
    if (F.stage === 'tune') {
      ...
      for (const L of leads) {
        ...
        const w = solveRidge(X.slice(0, cut), y.slice(0, cut), combo.ridge);
        const p = (r) => { let s2 = 0; for (let i = 0; i < r.length; i++) s2 += r[i] * w[i]; return s2; };
        acc += r2(y.slice(cut), X.slice(cut).map(p));
      }
    ...
    const cut = Math.floor(X.length * 0.75);
    const w = solveRidge(X.slice(0, cut), y.slice(0, cut), chosen.ridge);
    const p = (r) => { let s2 = 0; for (let i = 0; i < r.length; i++) s2 += r[i] * w[i]; return s2; };
    const v = r2(y.slice(cut), X.slice(cut).map(p));
    F.ladder.push({ ch: c, L, w, val: v });
    F.li++;
  ...
  _controlTick(active, lookAhead, deployed = false) {
    ...
    for (let c = 0; c < this.nc; c++) {
      ...
      if (!active || !haveHist) { R.uTarget[c] = 0; hist.push(0); continue; }
