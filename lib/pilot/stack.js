...
export class Stack {
  ...
  _admitted(p) { return !!(p.verdict && p.verdict.deploy && !p._stackRefused); }
  ...
  observe(measured, truth) {
    ...
    if (this._done) {
      for (const p of this.layers) if (this._admitted(p)) p.observe(measured, null);
      return;
    }
  ...
  _advance() {
    if (this._cur.phase === 'done' && !this._done) {
      ...
      // A LAYER THAT CANNOT FORECAST EVERY CHANNEL DOES NOT JOIN A MULTI-CHANNEL STACK.
      // A gated channel is not a channel that corrects nothing — on a COUPLED plant it is
      // a layer that corrects the OTHERS, pushing the output along the only direction it
      // has left, which is not the direction the QP chose when it priced a plan assuming
      // all of them were free. Measured on ⑥ at the softest sliders: layer 2 forecast
      // R^2 [0.848, -0.117], gated the elbow, VERIFIED 1.85x on the program regime — and
      // delivered 3.40x -> 2.93x on 3.1x the copper with the peak correction up 0.40 ->
      // 0.91. Its own verify said deploy and the machine said otherwise. ⑤'s layer 2, with
      // both channels alive at 0.440/0.571, took 6.43x -> 12.21x.
      //
      // THE SAME QUESTION EXISTS FOR A SINGLE PILOT and is NOT answered here: nothing
      // stops layer 1 gating a channel and correcting the rest. This is the layer
      // ADMISSION rule, where the evidence is, and it is one measurement — a second plant
      // with an unequally forecastable pair would be what confirms or kills it.
      ...
      if (!this._admitted(this._cur) || this.layers.length >= this.depth) {
        ...
        this.report.layers = this.layers.map((p, i) => ({ layer: i + 1,
          ...
          why: p._stackRefused || (p.verdict && p.verdict.why),
