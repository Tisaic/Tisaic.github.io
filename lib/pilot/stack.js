...
export class Stack {
   ...
   */
  /** The current layer's commanded pose `off` steps from now (see `Pilot.posAt`). */
  posAt(off = 0) { return this._cur.posAt(off); }
  ...
  /** Re-phase every live layer at a lap start (see `Pilot.syncLap`). */
  syncLap() { for (const p of this.layers) if (this._admitted(p) && p.syncLap) p.syncLap(); }
