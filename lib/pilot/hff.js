...
export class HarmonicFF {
  ...
  async _refine(run, base, passes, backtracks, amp = 1) {
    ...
    const alive = () => { for (let h = 0; h < nh; h++) if (stepH[h] >= floorH) return true; return false; };
    while (budget-- > 0 && alive()) {
      ...
      for (let h = 0; h < nh; h++) {
        ...
        if (stepH[h] < floorH) continue;          // this one has run out of step
        for (let c = 0; c < channels; c++) {
          trial.re[c][h] += sh * stepH[h] * v[2 * c];
          trial.im[c][h] += sh * stepH[h] * v[2 * c + 1];
        }
      }
