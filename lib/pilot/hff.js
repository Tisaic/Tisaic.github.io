...
export class HarmonicFF {
  ...
  constructor(o) {
    ...
    this.admit = !!o.admit;   // affordability CUT: measured WORSE than the weighting (4.53x
                              // against 12.47x on the toy plant), kept as the recorded null
  ...
  async commission(run) {
    ...
    if (cands.length > 1) {
      for (const cd of cands) {
        ...
        this.reachPow = cd.pow; this.adm = null;
        const trial = await this._refine(run, base, this.trialPasses, 0);
        laps += (mm + 1) + trial.laps;
        rep.candidates.push({ ...cd, cross, score: trial.best, set });
      }
      ...
      this.reachPow = pickC.pow; this.adm = null;
