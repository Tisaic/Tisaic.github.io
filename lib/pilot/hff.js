...
export class HarmonicFF {
  ...
  constructor(o) {
    ...
    // WHERE THIS RUNG IS, FOR SOMEBODY WATCHING IT. Commissioning is tens of minutes and the
    // only two questions being asked are "is it nearly done" and "is it stuck"; a run counter
    // answers neither. A reporter must never be able to fail a measurement.
    this.onStage = typeof o.onStage === 'function'
      ? (d) => { try { o.onStage(d); } catch { /* a reporter must not fail a run */ } }
      : () => {};
  ...
  async commission(run) {
    ...
    // WHERE THE LAPS ACTUALLY GO, counted rather than reasoned about. This rung's cost is
    // most of its argument against the alternatives, and 'identification is expensive' was
    // an assertion nobody had broken down. Aiming an optimisation at the wrong line is how
    // the DFT nearly got hand-tuned when it is a thousandth of the cost of the simulation
    // it sits inside.
    //
    // DECLARED HERE, ABOVE EVERYTHING THAT READS IT. Put next to the code it counts, it sat
    // below the first line that incremented it and threw a ReferenceError on the first call
    // — the temporal dead zone this project has on record three times and which I have now
    // added to twice in one day.
    const budget = { base: 0, probeSizing: 0, probes: 0, trials: 0, refocus: 0, refine: 0 };
    const rep = { base: 0, best: 0, hist: [], stopped: null, probeAmp: 0, probePeak: 0,
      gain: null, rel: null, alive: 0 };

    this.active = null;
    const PLAN = this.plan();
    this.onStage({ stage: 'baseline', i: 0, of: 1, plan: PLAN,
      ends: 'one lap to score the machine as it arrived' });
    ...
    if (this.operator) {
      ...
      Object.assign(rep, finR.report);
      rep.laps = finR.laps + 1;
      this.W = finR.W; this._tbl = null; this.active = this._table(finR.W);
      rep.best = finR.best;
      rep.operator = this.exportOperator();
      return rep;
    }
    this.onStage({ stage: 'sizing the probe', i: 0, of: 1, plan: PLAN,
      ends: 'one lap on the CLEAN machine — the probe is sized from what it probes, not from '
        + 'the error a deployed layer has already removed' });
    const idBase = await run(null, 'probe');
    budget.probeSizing += 1;
    let epk = 0;
    ...
    const mm = 2 * channels;
    // THE COST OF ONE PROBE SET, DEFINED ONCE. It was written out at the point of use and
    // then hard-coded as `mm + 1` at three separate accounting sites, so when the banded
    // model raised the count to 3*mm+4 every reported lap total understated the truth by
    // eleven per set — while the machine paid the real number. A cost that is quoted in one
    // place and charged in another eventually disagrees, and a lap count is what this rung's
    // whole cost argument rests on.
    const nqOf = () => (this.banded ? 3 * mm + 4 : mm + 1);
    ...
    // The trial sweep's own counter, so "scoring candidates 7/16" is the position in the
    // WHOLE sweep rather than within one design.
    let trialNo = 0;
    const trialsPlanned = PLAN.stages.find((x) => x.name === 'scoring candidates').runs
      / Math.max(1, this.trialPasses);
    rep.candidates = [];
    if (cands.length > 1) {
      ...
      for (const [ci, pc] of probeCands.entries()) {
        this.onStage({ stage: 'probing candidate designs', i: ci, of: probeCands.length, plan: PLAN,
          ends: `design ${ci + 1} of ${probeCands.length}: ${nqOf()} probe laps, then scored `
            + 'across the reach exponents and band ceilings — the MACHINE picks, because the '
            + 'fit ranks them backwards' });
        const set = await probeSet(pc.style, pc.frac * epk);
        laps += nqOf(); budget.probes += nqOf();
        ...
        for (const pw of pows) {
          for (const cut of cutOpts) {
            ...
            this.onStage({ stage: 'scoring candidates', i: trialNo++, of: trialsPlanned,
              plan: null,
              ends: `design ${ci + 1}/${probeCands.length}, reach exponent ${pw}, band `
                + `ceiling ${cut || 'full'} — ${this.trialPasses} passes on the machine` });
            const trial = await this._refine(run, base, this.trialPasses, 0, amp);
            laps += trial.laps; budget.trials += trial.laps;
    ...
    if (this.refocus && this.nhCut > 0 && this.nhCut < nh) {
      ...
      laps += nqOf(); budget.refocus += nqOf();
      this.onStage({ stage: 'identifying the operator', i: 0, of: 1, plan: PLAN,
        ends: `${nqOf()} probes at the chosen design, on the CLEAN machine` });
    ...
    rep.gain = Array.from(this.gain); rep.rel = Array.from(this.rel);
    rep.operator = this.exportOperator();
    this.onStage({ stage: 'refining', i: 0, of: this.passes + this.backtracks, plan: PLAN,
      ends: 'one damped Newton lap per pass, STOPPING EARLY when four passes in a row improve '
        + "by less than the machine's own repeatability — the remaining budget would be "
        + 'selecting on noise' });
    const fin = await this._refine(run, base, this.passes, this.backtracks, amp, true);
    laps += fin.laps; budget.refine += fin.laps;
    // ALL OF IT, not the third of it that happened to be wired. `damped` and `stepH` were
    // recorded by `_refine` and never copied here, so the harness's diagnostic read
    // `undefined`, defaulted to an empty array, and printed '0 harmonics still above the
    // floor' as though it were a measurement. Rule 25: not-measured rendering as a
    // meaningful zero, in the instrument added to make the plateau readable.
    Object.assign(rep, fin.report);
    rep.perHarmonicStep = this.perHarmonicStep;
   ...
   */
  /**
   * WHAT THIS RUNG IS ABOUT TO SPEND, AND WHAT ENDS EACH STAGE.
   *
   * A progress line that says "run 37" and nothing else proves the machine is alive and
   * tells the operator nothing they can act on: no idea how far through it is, and no idea
   * what has to happen for it to move on. Commissioning here is tens of minutes, so "is this
   * nearly done?" and "is this stuck?" are the only two questions being asked, and a bare
   * counter answers neither.
   *
   * Every number below comes from the same fields `commission()` reads, so the estimate
   * cannot drift from the run (rule 30: generate the description from the thing). The
   * refinement figure is a CEILING, not a prediction — it stops early, and deliberately, when
   * improvements fall inside the machine's own repeatability.
   *
   * @returns {{total:number, stages:{name:string, runs:number, exact:boolean, ends:string}[]}}
   */
  plan() {
    const mm = 2 * this.channels;
    const nq = this.banded ? 3 * mm + 4 : mm + 1;
    const styles = this.banded ? 1 : (this.probeStyle === 'auto' ? 2 : 1);
    const fracs = (this.probeFracs || [0, 0]).length;
    const cands = styles * fracs;
    // THE TRIAL COUNT IS NOT KNOWABLE EXACTLY, AND SAYING SO IS THE POINT. Each probe set is
    // scored across the reach exponents AND the band ceilings, and the ceiling list is
    // `[0, knee]` only when a knee is found in the measured error spectrum — one option
    // otherwise. So the ceiling here is the sweep at its widest, marked inexact. A first
    // version of this ignored the sweep entirely and published 40 runs for a commission that
    // spent 76, which is the failure mode that matters: a denominator read as fact, in the
    // direction that makes a healthy run look stuck.
    const pows = (this.reachPows || [1, 0.5]).length;
    const cuts = this.cuts ? this.cuts.length : 2;
    const stages = [
      { name: 'baseline', runs: 1, exact: true,
        ends: 'one lap to score the machine as it arrived' },
      { name: 'sizing the probe', runs: 1, exact: true,
        ends: 'one lap on the CLEAN machine — the probe is sized from what it probes, not '
          + 'from the error a layer beneath it has already removed' },
      { name: 'probing candidate designs', runs: cands * nq, exact: true,
        ends: `${cands} design${cands === 1 ? '' : 's'} x ${nq} probe laps each — a probe set `
          + 'is what the operator is fitted from, and the designs differ in how they spread '
          + 'the excitation across the lap harmonics' },
      { name: 'scoring candidates', runs: cands * pows * cuts * this.trialPasses, exact: false,
        ends: `each design scored across ${pows} reach exponents and up to ${cuts} band `
          + `ceilings, ${this.trialPasses} passes each — the MACHINE picks, because the fit `
          + 'ranks them backwards: the best-fitting design is routinely the worst controller' },
      { name: 'identifying the operator', runs: nq, exact: true,
        ends: `${nq} probes at the chosen design (${this.banded ? 'banded, 3*2c+4' : '2c+1'})` },
      { name: 'refining', runs: this.passes + this.backtracks, exact: false,
        ends: 'one damped Newton lap per pass, STOPPING EARLY when four passes in a row '
          + "improve by less than the machine's own repeatability — spending the rest would "
          + 'be selecting on noise' },
    ];
    return { total: stages.reduce((a, x) => a + x.runs, 0), stages };
  }

  async _refine(run, base, passes, backtracks, amp = 1, report = false) {
    ...
    // THE INSTRUMENT'S OWN NOISE, if the host reports it, and how long the best has stood.
    let sigma = 0, stall = 0, stopped = null, bestAtStall = Infinity;
    ...
    while (budget-- > 0 && alive()) {
      // PER PASS, BECAUSE THIS IS THE STAGE THAT GOES QUIET. Refinement is the longest part
      // of the rung and it emits nothing until it finishes, so it is the one an operator
      // reads as a stall. `stall` is the number the stop rule is actually counting to.
      if (report) {
        this.onStage({ stage: 'refining', i: laps, of: passes + backtracks, stall, plan: null,
          best, sigma,
          ends: `stops at 4 consecutive passes improving by less than 2 sigma `
            + `(${(2 * sigma).toExponential(2)} against ${best.toExponential(3)}) — `
            + `${stall} of 4 so far` });
      }
      ...
      // ---- EVERY DECISION BELOW COMPARES TWO MEASURED SCORES, AND THE MACHINE HAS A FLOOR.
      // On the arm the deployed machine repeats to 5.84e-4 against a score of 2.0e-2 —
      // 2.9%. The accept threshold is 1e-3 of the score, which is 0.03 of one standard
      // deviation: this guard has been resolving differences thirty times finer than the
      // machine can produce. Past the point where a pass's true gain falls under the noise
      // the loop accepts and rejects on coin flips, and `bestW` becomes the luckiest draw
      // rather than the best table — the argmin of fifteen noisy passes sits about 1.7
      // sigma low, which on these numbers is 5% of the score it reports.
      if (Number.isFinite(next.spread) && next.spread > sigma) sigma = next.spread;
      ...
      cur = next;
      // ---- STOP WHEN IMPROVEMENTS STOP BEING RESOLVABLE, rather than spending the rest of
      // the budget selecting on noise. `best` must move by more than two standard
      // deviations of the machine's own repeatability to count as movement; four passes
      // without that is a plateau, and every pass after it both costs a lap and gives the
      // argmin another draw to get lucky on. Inert when the host reports no spread — not
      // measured and zero stay different states.
      if (sigma > 0) {
        if (best < bestAtStall - 2 * sigma) { bestAtStall = best; stall = 0; }
        else if (++stall >= 4) {
          stopped = `improvements fell below the machine's own repeatability (2 sigma = `
            + `${(2 * sigma).toExponential(2)} against a score of ${best.toExponential(3)})`
            + ` — the remaining budget would select on noise`;
          break;
        }
      }
    }
    ...
    // ONLY stepH[0] MOVES IN THE GLOBAL PATH, so the max over the whole array is the
    // untouched initial value of every other entry and reports 'the step never shrank' on a
    // run that halved it repeatedly. Read the one that is actually in use.
    let fin2 = perStep ? 0 : stepH[0];
    if (perStep) for (let h = 0; h < nh; h++) fin2 = Math.max(fin2, stepH[h]);
    // HOW MANY PASSES LANDED WITHIN THE NOISE OF THE BEST. If most did, the deployed table
    // is one draw from a cluster the instrument cannot order, and the score beside it is
    // optimistic by roughly the spread of that cluster.
    const withinNoise = sigma > 0 ? hist.filter((v) => v <= best + sigma).length : null;
    // ---- EVERYTHING THE HOST SHOULD REPORT, IN ONE OBJECT.
    //
    // These were returned as loose fields and copied to the report one line at a time, and
    // the copy was forgotten three times in a day: `damped` and `stepH` — which printed
    // '0 harmonics still above the floor' while every harmonic was still moving — and a
    // third, forgotten within hours of adding a check specifically for missing report
    // fields, which did not catch it because that check tests a list somebody remembered to
    // write. (That third field has since been removed with the null it belonged to; the
    // reason it is remembered here is the pattern, not the field.)
    //
    // A check that you did not forget is weaker than not being able to forget. `report` is
    // assigned wholesale by the caller, so a field added here reaches the report whether or
    // not anyone remembers it exists.
    return { best, W: bestW, laps,
      report: { hist, step: steps, damped, finalStep: fin2, stepH: Array.from(stepH),
        sigma: sigma || null, stopped, withinNoise } };
  }
}
