...
export class HarmonicFF {
  ...
  constructor(o) {
    ...
    // How far a single pass may move the correction, as a multiple of the probe amplitude.
    // MEASURED WORSE and default 0 — see the block in `_refine` for why the premise is wrong.
    this.trust = o.trust ?? 0;
  ...
  async _refine(run, base, passes, backtracks, amp = 1) {
    ...
    let sigma = 0, stall = 0, stopped = null, bestAtStall = Infinity, clipped = 0;
    ...
    while (budget-- > 0 && alive()) {
      ...
      // ---- A TRUST REGION AT THE PROBE AMPLITUDE: BUILT, MEASURED WORSE, DEFAULT OFF.
      //
      // The motivation was real. On the arm a run otherwise sitting between 1.8e-2 and
      // 2.1e-2 threw 3.43e-2 at pass 7 and 4.27e-2 at pass 13 — the machine twice as bad
      // for a pass — and six step halvings were spent recovering, about an eighth of the
      // rung's laps undoing steps that should not have been taken. Bounding the per-pass
      // increment by the amplitude the operator was identified over looked like the
      // principled answer: an operator is only known where it was probed.
      //
      // IT IS NOT, AND THE PREMISE IS THE ERROR. The probe amplitude bounds the
      // identification NOISE, not the range over which a LINEAR operator is valid — on a
      // linear plant extrapolating ten times past the probe is exactly as correct as
      // interpolating within it. Worse, the correction the machine needs is the size of the
      // ERROR while the probe is a tenth of it, so this bound throttles the one step that
      // must be large: the first. Measured on a coupled synthetic plant, best 3.607e-14
      // without against 6.567e-11 with, and the worst pass went 3.171e-2 to 3.573e-1
      // because the opening correction was capped to a tenth of what was needed.
      //
      // What would be right for a NONLINEAR plant is a RELATIVE region — bound the increment
      // against the accumulated correction rather than against the probe — and that is not
      // built, because the evidence for a problem is two passes in fifteen and the first
      // attempt at fixing it cost more than the excursions did. Default 0, off.
      if (this.trust > 0 && amp > 0) {
        let pk = 0;
        for (let c = 0; c < channels; c++) for (let h = 0; h < nh; h++) {
          pk = Math.max(pk, Math.hypot(trial.re[c][h] - acc.re[c][h], trial.im[c][h] - acc.im[c][h]));
        }
        const lim = this.trust * amp;
        if (pk > lim) {
          const sc2 = lim / pk;
          for (let c = 0; c < channels; c++) for (let h = 0; h < nh; h++) {
            trial.re[c][h] = acc.re[c][h] + (trial.re[c][h] - acc.re[c][h]) * sc2;
            trial.im[c][h] = acc.im[c][h] + (trial.im[c][h] - acc.im[c][h]) * sc2;
          }
          clipped++;
        }
      }
    ...
    // '0 harmonics still above the floor' while every harmonic was moving), and
    // `trustClipped` — forgotten within hours of adding a check specifically for missing
    // report fields, which did not catch it because that check tests a list somebody
    // remembered to write.
    //
    ...
    return { best, W: bestW, laps,
      report: { hist, step: steps, damped, finalStep: fin2, stepH: Array.from(stepH),
        sigma: sigma || null, stopped, withinNoise, trustClipped: clipped } };
  }
}
