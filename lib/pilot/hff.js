...
export class HarmonicFF {
  ...
  async _refine(run, base, passes, backtracks, amp = 1) {
    ...
    while (budget-- > 0 && step >= this.step / (1 << backtracks)) {
      ...
      //
      // The operator is identified with reactive layers DISARMED, because identifying
      // through a receding-horizon controller is noise-dominated. That is a claim about
      // magnitude and a weak one about everything else: measured on the arm, clean against
      // deployed differs by 16.8 degrees of phase (energy-weighted, up to 46) and by GAIN
      // FACTORS FROM 0.44 TO 3.06. The gain mismatch is what plateaus the refinement — a
      // damped Newton step converges at harmonic h by (1 - step*G/Ghat), so a ratio of 3.06
      // DIVERGES at full step, the guard backtracks the GLOBAL step to protect it, and every
      // well-matched harmonic then crawls at a fraction of its possible rate.
      //
