...
/**
 * A TRAIN OF CORNER EVENTS — sparse, brief, program-speed velocity reversals, superimposed on
 * the slow scribble.
 *
 * WHY EVENTS AND NOT FASTER NOISE. The sharp square's fast content is v ~ 63% of vMax turned
 * around in a few solver steps: acceleration and jerk at hundreds of times the declared limits,
 * VELOCITY still inside them, because the a/j peaks are brief and rare. Stationary noise cannot
 * have that shape — its velocity scales as amplitude over correlation time, so noise fast enough
 * to carry the square's jerk runs at many times vMax continuously (measured: matching the
 * square's j = 1.2e-4 with 3-pole noise at any tc the sampling can see needs ~25x vMax). A
 * sparse event train is the same trade the program itself makes: rare sharp transitions, smooth
 * travel between them. The dwell warp was this idea built wrong — it slowed the noise everywhere
 * and starved the fast dynamics (measured null-to-negative); this INSERTS the fast transient and
 * leaves the noise alone.
 *
 * Each event is an out-and-back velocity trapezoid: ramp to -dv over `ramp` steps, hold `width`,
 * swing to +dv over 2*`ramp`, hold `width`, ramp home — zero net displacement by symmetry, so
 * the train adds no drift for the box acceptance to reject. Peak |a| ~ dv/ramp and peak |j| ~
 * dv/ramp², which is the point: they are meant to EXCEED the declared aMax/jMax the way the
 * program does, so the composed series is measured and REPORTED in meta rather than checked
 * against limits it is designed to break. Velocity stays additive to the noise's and small.
 *
 * @param {function} rnd  the channel's RNG
 * @param {number} steps  series length
 * @param {number} from   first step events may start (past the approach ramp)
 * @param {object} evt    { vShare, ramp, width, every }
 * @param {object} lim    the channel's declared limits (dv = vShare * vMax)
 * @returns {Float64Array} a position bump series to ADD to the channel's position
 */
function cornerEvents(rnd, steps, from, evt, lim) {
  const out = new Float64Array(steps);
  const dvMax = (evt.vShare ?? 0.5) * lim.vMax;
  const r0 = Math.max(1, Math.round(evt.ramp ?? 2));
  const w0 = Math.max(0, Math.round(evt.width ?? 16));
  const every = Math.max(4 * r0 + 2 * w0 + 8, Math.round(evt.every ?? 600));
  // A SEVERITY LADDER, NOT ONE SEVERITY. Each event draws its own speed and width, so the
  // record carries a SPECTRUM of corner harshness rather than one point on it — which is what
  // a CONTINUOUS regime blend has to be fitted against. A single-severity train can only teach
  // a binary distinction, and the binary router is the thing this replaces: it mis-filed the
  // rounded rectangle, whose corners are two orders of magnitude milder than the square's.
  let k = from + Math.round(every * (0.5 + rnd()));
  let n = 0;
  for (;;) {
    const dv = dvMax * (0.3 + 0.7 * rnd());
    const r = r0;
    const w = Math.max(0, Math.round(w0 * (0.5 + 1.5 * rnd())));
    // The velocity profile of one event: out-and-back trapezoid, zero net area by symmetry.
    const seg = [];
    for (let i = 0; i < r; i++) seg.push(-dv * (i + 1) / r);
    for (let i = 0; i < w; i++) seg.push(-dv);
    for (let i = 0; i < 2 * r; i++) seg.push(-dv + dv * (i + 1) / r);
    for (let i = 0; i < w; i++) seg.push(dv);
    for (let i = 0; i < r; i++) seg.push(dv - dv * (i + 1) / r);
    if (k + seg.length >= steps) break;
    const sign = rnd() < 0.5 ? -1 : 1;
    let p = 0;
    for (let i = 0; i < seg.length; i++) { p += sign * seg[i]; out[k + i] = p; }
    // p ends at ~0 by construction; anything left is float dust, not drift.
    n++;
    k += seg.length + Math.round(every * (0.5 + rnd()));
  }
  out.events = n;
  return out;
}
...
export function buildExcitation({ channels, steps, start, workspace = null, seed = 1,
  ...
  for (let attempt = 0; attempt < 24; attempt++) {
    ...
    // THE EVENT TRAIN GOES ON AFTER THE DECLARED-LIMIT ACCEPTANCE AND BEFORE THE WORKSPACE
    // CHECK. After, because its a/j peaks are designed to exceed the declared limits the way
    // the program does, and gating them on those limits would delete them — the composed peaks
    // are MEASURED and reported in `meta.events` instead, so nothing is silent (rule 27).
    // Before the workspace check, because the bump is a real excursion the machine will make.
    if (events) {
      meta.events = [];
      for (let c = 0; c < nCh; c++) {
        const ev = cornerEvents(lcg(seed + 271 * c + 17 * attempt), steps, rampN,
          events, channels[c]);
        for (let k = 0; k < steps; k++) pos[c][k] += ev[k];
        meta.events.push({ n: ev.events, composed: peakDiffs(pos[c]) });
      }
    }
    if (workspace) {
      let ok = true;
