...
function cornerEvents(rnd, steps, from, evt, lim) {
  const out = new Float64Array(steps);
  const dv = (evt.vShare ?? 0.5) * lim.vMax;
  const r = Math.max(1, Math.round(evt.ramp ?? 2));
  const w = Math.max(0, Math.round(evt.width ?? 16));
  const every = Math.max(4 * r + 2 * w + 8, Math.round(evt.every ?? 600));
  // The velocity profile of one event, built once: out-and-back trapezoid, zero net area.
  const seg = [];
