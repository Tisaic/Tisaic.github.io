...
| `hff.js` | Harmonic feedforward: invert the machine at the lap's own harmonics. **A memory** — it is retired by the north star and kept because it is the thing distillation distils, and because its operator is a plant model even though its table is not. In `autostack.js`, `periodic: null` is the retirement switch: it gates this rung and nothing else, so a caller who has retired the memory declines to declare a lap and gets a ladder that cannot ship one. | lap phase |
