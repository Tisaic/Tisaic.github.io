...
export class AutoStack {
  ...
  constructor(o) {
    ...
    this.built = { classic: null, stack: null, hff: null };
    this._identifying = false;   // true only during the harmonic rung's PROBE laps
    this.report = { rungs: [] };
    this._clip = 0; this._clipPk = 1; this._acts = 0;
  }
  ...
  async commission(host) {
    ...
    // floor rises to the largest one seen. It only ever rises, so a comparison is made at
    // the coarsest resolution either side of it was measured at, never at a finer one that
    // was true only of a quieter configuration. A host that returns nothing keeps the
    // static floor: "not measured" and "zero" stay different states (rule 26).
    const scored = async (corr, name) => {
      this.beginRun();
      const r = await host.run(corr, name);
      r.clip = this.clipping();
      if (Number.isFinite(r.spread) && r.spread > this.floor) {
        this.floor = r.spread;
        rep.floorFrom = name || 'as it arrived';
      }
      return r;
    };
    const base = await scored(null, null);
    rep.base = base.score;
    ...
    if (this.basis) {
      ...
      const rc2 = await scored(null, null);
    ...
    if (host.drivePilot) {
      ...
      for (let depth = 1; depth <= this.maxDepth; depth++) {
        ...
        const st = this.built.stack = new Stack({ ...this.pilotOpts, channels: this.channels,
    ...
    if (this.periodic) {
      ...
      const rh2 = await scored(null, null);
    ...
    for (const name of armed()) {
      ...
      const without = await scored(null, null);
  ...
  act(ctx, extra = null, extraName = null) {
    ...
    if (this.deployed.stack && ctx.look && !this._identifying) add(this.stack.act(ctx.look), 'stack');
