...
export class AutoStack {
  ...
  async commission(host) {
    ...
    if (this.basis) {
      const c = new ClassicFF({ basis: this.basis, channels: this.channels.length, uMax: this.uMax });
      const r = await c.commission(host.run);
    ...
    if (this.periodic) {
      const h = new HarmonicFF({ lap: this.periodic, channels: this.channels.length, uMax: this.uMax });
      const r = await h.commission(host.run);
      const kept = beats(r.best, best);
  ...
  act(ctx) {
    ...
    if (this.deployed.classic && ctx.v) {
      const u = this.classic.live(ctx.v, ctx.a);
      for (let c = 0; c < nc; c++) out[c] += u[c];
    }
    if (this.deployed.stack && ctx.look) {
      const u = this.stack.act(ctx.look);
      for (let c = 0; c < nc; c++) out[c] += u[c];
    }
    if (this.deployed.hff && ctx.k !== undefined) {
      const u = this.hff.at(ctx.k);
      for (let c = 0; c < nc; c++) out[c] += u[c];
    }
