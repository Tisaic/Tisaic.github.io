    ...
    if (this.demoOpts && this.stack && this.deployed.stack) {
      if (!host.recordDemo) {
        ...
        if (p1 && p1.verdict && p1.verdict.deploy) {
          ...
          // ---- ②c THE LAG LAYER, FIRST (it changes the row width, so the corner banks
          // must be fitted after it, on rows that include the long block). The mechanism
          // campaign measured the deployed residual as the servo loop's rate-history
          // response, pose-modulated — invisible to the commissioning scribble (the long
          // candidate loses the tune there, rule 34) and fully present in program-class
          // rows, which the demo records are. Refit, score on the machine, keep or revert:
          // the ladder decides, the fit does not.
          if (recs && recs.length) {
