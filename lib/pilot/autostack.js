...
export class AutoStack {
  ...
  async commission(host) {
    ...
    const rung = (name, score, note, clip) => {
    ...
    if (this.basis) {
      ...
      rung('conventional (self-tuned)', rc2.score,
        ...
        rc2.clip);
      if (kept) { this.classic = c; this.deployed.classic = true; best = rc2.score; }
      rep.classic = r;
    }
    ...
    if (host.drivePilot) {
      for (let depth = 1; depth <= this.maxDepth; depth++) {
        const st = this.built.stack = new Stack({ ...this.pilotOpts, channels: this.channels, uMax: this.uMax, depth });
    ...
    if (Number.isFinite(this.uMax)) {
      for (let c = 0; c < nc; c++) {
        ...
        if (v > this.uMax || v < -this.uMax) {
          this._clip++;
          this._clipPk = Math.max(this._clipPk, Math.abs(v) / this.uMax);
