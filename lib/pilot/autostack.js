...
export class AutoStack {
      ...
      if (!host.distilRuns) {
        ...
        if (runs && runs.length) {
          ...
          const pol = new DistilPolicy({
            ...
            offsets: this.distilOpts.offsets, signOffsets: this.distilOpts.signOffsets,
          ...
          for (let i = 0; i < runs.length; i++) {
            ...
            if (!(gain > 1.5)) { conv.push({ lap: tr.lap, gain, used: 0, dropped: true }); continue; }
            const prefix = new Array(tr.lap);
            for (let k = 0; k < tr.lap; k++) prefix[k] = h.at(k);
            const used = pol.addProgram({ refAt: tr.refAt, n: tr.lap, prefix, speedAt: tr.speedAt });
            conv.push({ lap: tr.lap, gain, used, dropped: false });
          }
          ...
          if (!kept.length) {
            ...
            if (!fitRep.deploy) {
              ...
              if (win) best = r.score;
              else { this.distil = null; this.deployed.distil = false; }
              rung(win ? '②d distilled' : '②d distilled — REFUSED', r.score,
                `${kept.length}/${conv.length} training runs kept, ${fitRep.rows} rows, `
  ...
  act(ctx, extra = null, extraName = null) {
    ...
    if (this.deployed.distil) {
      ...
      if (dl) {
        const ud = this.distil.actLook(dl, ctx.speed ?? null);
