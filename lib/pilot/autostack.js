...
export class AutoStack {
  ...
  constructor(o) {
    ...
    this.built = { classic: null, stack: null, stacks: [], hff: null, distil: null };
  ...
  act(ctx, extra = null, extraName = null) {
    ...
    if (this.deployed.distil) {
      // DRIVEN BY THE HOST'S OWN LOOK-AHEAD CLOSURE — the same `ctx.look` the cascade is driven
      // by, so a host that can run the pilot can run this with no new deploy-time plumbing. It
      // is addressed by the COMMANDED REFERENCE and carries no lap index, so unlike the rung
      // below it is not withheld off its program: that is the whole point of distilling.
      if (ctx.look) add(this.distil.actLook(ctx.look, ctx.speed ?? null), 'distil');
