...
export class AutoStack {
    ...
    if (this.distilOpts) {
      if (!host.distilRuns) {
        ...
        if (runs && runs.length) {
          ...
          if (parametric) {
            const P = Math.max(1, this.distilOpts.passes ?? 4);
            const geo = (xs) => Math.exp(xs.reduce((a, x) => a + Math.log(x), 0) / xs.length);
            const state = runs.map(() => ({ rec: null, base: 0, score: 0, corr: null }));
            if (this.onStage) this.onStage({ stage: 'starting', rung: 'distil', i: 0, of: runs.length,
              ends: 'measure every training program bare, then iterate the policy against all of them' });
            for (let i = 0; i < runs.length; i++) {
              const r0 = await runs[i].run(null, { trace: true, laps: runs[i].teachLaps });
              state[i].rec = r0.rec; state[i].base = r0.score; state[i].score = r0.score;
            }
            let cur = null, curScore = geo(state.map((x) => x.score)), passes = 0, bestTargets = null;
            for (let pass = 0; pass < P; pass++) {
              ...
              const cand = mkPol(); cand.stride = distilStride;
              for (const { tr, target } of targets) cand.addProgram({ refAt: tr.refAt, n: tr.lap, prefix: target, speedAt: tr.speedAt, stride: distilStride, closed: !!tr.closed });
              const fr = cand.fit();
              if (!fr.deploy) break;
              // THE POLICY IS RE-MEASURED, not its target: what the machine does under the
              // projected correction is the number, and it is also the next pass's record.
              const scores = [];
              const recs = [];
              for (let i = 0; i < runs.length; i++) {
              ...
              passes = pass + 1;
              if (g < curScore) {
                cur = cand; curScore = g; bestTargets = targets;
  ...
  beginRun() {
    ...
    this._peak = new Array(this.channels.length).fill(0);
    this._nonfinite = 0;
    this._distilHold = null; this._distilCtx = null;   // no held decision from a previous run
  ...
  act(ctx, extra = null, extraName = null) {
    ...
    if (this.deployed.distil) {
      ...
      if (dl) {
        ...
        if (!hold) this._distilCtx = { look: dl, speed: ctx.speed ?? null, applied: ud };
        add(ud, 'distil');
