...
/**
 * THE LAG-LAYER REFIT (plan §38): the readout bank refitted on PROGRAM-CLASS records with
 * the long basis armed. The mechanism campaign measured the deployed residual as the servo
 * loop's rate-history response, pose-modulated (R² 0.9917 with these features) — and the
 * commissioning scribble cannot see it (the long candidate LOSES there, 0.987 vs 0.997 and
 * 0.811 vs 0.833 on the arm), which is rule 34 exactly: the model must be fitted in the
 * configuration it will run in. The demo records ARE that configuration, captured by the
 * host with the rungs below armed. This refits every lead's weights on those rows with the
 * long block appended, returns a revert handle, and decides NOTHING — the ladder scores
 * the machine and keeps or reverts, like every rung.
 *
 * @param {object} pilot a commissioned, deployed pilot
 * @param {Array} recs   records in the `recordOpenLoop`/`recordDemo` shape
 * @returns {{fitted: number, kept: number, revert: Function}}
 */
export function refitLong(pilot, recs) {
  const saved = pilot.readouts.map((ro) => ({ w: ro.w, longR: ro.longR || null }));
  let fitted = 0, kept = 0;
  for (let c = 0; c < pilot.readouts.length; c++) {
    const ro = pilot.readouts[c];
    const longR = pilot._longR();
    const newW = [];
    for (let li = 0; li < pilot.N; li++) {
      const L = ro.leads[Math.min(li, ro.leads.length - 1)];
      const back = Math.max((ro.mLag - 1) * ro.stride, (ro.fLag - 1) * ro.stride - L);
      ...
      for (const rec of recs) {
        ...
        try {
          for (let k = Math.max(back, rec.lap); k < rec.e.length - L - 1; k++) {
            X.push(pilot._row(c, k, L, ro.stride, ro.poly, ro.mLag, ro.fLag, ro.sched, longR));
            y.push(rec.e[k + L][c]);
          }
        } finally { pilot._rec = savedRec; }
      }
      if (X.length > 4 * X[0].length) { newW.push(solveRidge(X, y, ro.ridge)); fitted++; }
      else { newW.push(null); kept++; }
    }
    // a lead the rows could not overdetermine keeps nothing — the whole channel reverts,
    // because a bank mixing two row widths cannot be evaluated.
    if (newW.some((w) => !w)) continue;
    ro.w = newW;
    ro.longR = longR;
  }
  return { fitted, kept,
    revert: () => pilot.readouts.forEach((ro, i) => {
      ro.w = saved[i].w; ro.longR = saved[i].longR; }) };
}

/**
 * ARM THE LONG BLOCK ON A DEPLOYED PILOT, ZERO-EXTENDED — the byte-identical start the
 * guided phase then fills. `refitLong`'s batch route was put to the machine and REFUSED
 * (1.96e-1 against the cascade's 8.3e-2): its rows were recorded pilot-SILENT, so the
 * deployed measured-signal distribution and the correction's own bookkeeping were both
 * off — rule 34 from the other side. The honest route is the one the take-away
 * installation already owns: extend the weights with zeros (the block contributes nothing
 * — the machine is unchanged until something is learned), reset the RLS so it re-seeds at
 * the new width, and let GUIDED adaptation learn the long columns in the configuration
 * they will run in, conv-subtracted truth and innovation gate included.
 */
export function armLong(pilot) {
  for (const ro of pilot.readouts) {
    if (!ro || !ro.w || ro.longR) continue;
    const longR = pilot._longR();
    const extra = pilot.nc * longR.taps * (1 + pilot.nm);
    const shared = ro.w.length > 1 && ro.w.every((v) => v === ro.w[0]);
    const ext = (v) => { const n = new Float64Array(v.length + extra); n.set(v); return n; };
    if (shared) { const e = ext(ro.w[0]); ro.w = ro.w.map(() => e); }
    else ro.w = ro.w.map(ext);
    ro.longR = longR;
    // A BLOCK PRIOR, NOT A NULL ONE. The first arming nulled the posterior and the RLS
    // re-seeded loose over EVERY column; the innovation gate then passed five rows and
    // those five moved the GOOD commissioned weights violently (2.17e-2 → 7.70e-2, the
    // machine 3.5x worse, frozen). Ignorance belongs only to the new columns: the old
    // ones get a tight prior (they are the commissioned fit), the long ones a loose prior
    // at the rich-block discipline (100x the ridge), so the recursion learns where it is
    // ignorant and holds where it is not.
    {
      const n = ro.w[0].length, old = n - extra;
      const ridge = Math.max(1e-12, ro.lam || ro.ridge);
      const P = new Float64Array(n * n);
      for (let i = 0; i < n; i++) {
        P[i * n + i] = i < old ? 1e-4 / ridge : 1 / (100 * ridge);
      }
      ro.cov = P;
    }
    delete ro._rls;
    if (ro._row0) ro._row0.length = 0;
    ro._wEff = null;
  }
}
