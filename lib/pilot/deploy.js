...
/**
 * THE PLANT-SIDE GUARD'S GAIN (plan §82), the exact twin of `coverageGain` one level down.
 *
 * `coverageGain` fades outside the commanded-SPEED span the fit saw. This fades when the MACHINE
 * is more distressed than the commissioning ever observed — §75 recorded that no such analogue
 * existed and §81 measured what it costs: under a sustained unmodelled load the object goes on
 * applying full authority, silently, past the point where it makes the machine WORSE (0.87x at one
 * tauMax), and because its demand is ADDITIVE on one drive it consumes the headroom the loop needs
 * to reject the disturbance.
 *
 * `load` is a normalised distress reading in [0,1] — on a servo, the fraction of recent steps the
 * drive could not deliver what it was asked for. It is FREE: the drive already reports its own
 * limit status, which is the one instrument in this project a customer does not have to buy.
 *
 * `report.loadMax` is the worst reading the COMMISSIONING itself observed, stored exactly as the
 * speed span and `bendMax` are, so the threshold carries no constant in the plant's units. The
 * gain is in [0,1] and can only ever REDUCE the correction, so this guard cannot add energy.
 */
export function loadGain(rec, load) {
  const lm = rec.report && rec.report.loadMax;
  if (load == null || !rec.loadGuard || !(lm > 0)) return 1;
  const lo = lm * (rec.loadLo ?? 4), hi = lm * (rec.loadHi ?? 10);
 ...
 */
/**
 * A DECLARED OPERATING POINT'S COVERAGE (plan §90.4), the deployed twin of `distil.js`'s
 * `_declCoverage` — and this file's whole value is that it is a SEPARATE implementation the
 * artefact test pins bit-identical, so a guard added on one side and forgotten on the other goes
 * red rather than quietly shipping a different controller.
 *
 * `rec.report.declSpan` is `{ name: [lo, hi] }`, the span of each declared scalar the
 * COMMISSIONING observed — the same shape and the same reasoning as `speedSpan`. §89.2 is why it
 * exists: the cold mill's win rests on two declarations, one read by an ENCODER (honest at any
 * operating point, and measured exactly inert across ±10% of target gauge) and one TYPED IN at
 * commissioning (the transport delay, whose lead is baked into the weights, and which reads
 * 0.770x then 0.562x of its commissioned factor at another line speed).
 *
 * A POINT SPAN REFUSES OUTSIDE ITSELF: a commissioning at one line speed has no evidence about
 * another, and widening it with a margin would be a per-plant constant invented to soften a
 * refusal (rule 31). Tolerance comes from declaring the scalar ACROSS a span at commissioning,
 * which is target 2's own lesson — feed-invariance comes from training across feeds.
 */
export function declGain(rec, decls) {
  const sp = rec.report && rec.report.declSpan;
  // OPT-IN AND OFF, exactly as `loadGuard` and `bendGuard` are (plan §102). The span is recorded
  // whenever a host declares — an investigation wants it, and `logSpec` names it — but it only
  // reaches the output when the record says a guard was asked for. Until §102 this function armed
  // itself off the presence of `declSpan`, so declaring a scalar for forensics silently bought the
  // refusal §100 measured at 2.020x -> 1.000x and 1.474x -> 1.000x on rows where the object HELPS.
  if (!rec.declGuard || !sp || !decls) return 1;
  let g = 1;
  for (const name of Object.keys(sp)) {
    const v = decls[name];
    // NOT TOLD is neither in range nor out of it — a host that declared at commissioning and says
    // nothing at deploy is a wiring fault, and a guard that silently refuses on a missing field is
    // unfindable (rule 25). `logSpec` names the fields an installation must record.
    if (v == null) continue;
    const lo = sp[name][0], hi = sp[name][1];
    if (v >= lo && v <= hi) continue;
    const m = (hi - lo) * rec.coverageFade;
    if (!(m > 0)) return 0;
...
export function decide(rec, look, speed = null, state = null, load = null, decls = null) {
  ...
  //
  // AND THE "ZERO FALSE REFUSALS" THAT FIRST JUSTIFIED IT IS RETRACTED (plan §78.5). The sweep
  // that produced it — every margin from 1 to 64, in sample and on a program the fit never saw —
  // scored both halves on a SINE, which is never locally straight. On the quadruple tank, whose
  // recipes HOLD and RAMP, the guard fires constantly and moved the delivered result 2.593x ->
  // 3.09x with the fit byte-identical, which is a false refusal improving a score by accident and
  // exactly rule 14. What the statistic cannot do is tell an isolated corrupted tap from a
  // legitimate CORNER, because both put one tap off the local line — and on a geometrically
  // spaced window a corner at a wide gap is barely resolved at all. Corrupted-tap detection
  // (200/200 at margins 1-4, 198 at 8) stands; the false-refusal rate is NOT characterised on any
  // plant with holds, which is why this is opt-in.
  //
  ...
  // OPT-IN, AND THE REASON IS A MEASURED FALSE-REFUSAL RATE (plan §78.5). It was armed by the
  // presence of `bendMax` and that was wrong: the statistic is a RATIO against the window's own
  // median bend, and a window that is LOCALLY STRAIGHT — a hold, or a constant-slope ramp, which
  // is most of a process plant's program — has a median of exactly zero, so any single tap off
  // that line reads Infinity and a healthy window is refused. `bendGuard` must be set explicitly.
  const bm = rec.bendGuard ? (rec.report && rec.report.bendMax) : 0;
  if (bm > 0 && windowBend(rec, look) > bm * (rec.bendMargin || 8)) return out.fill(0);
  const row = featureRow(rec, look, state), g = coverageGain(rec, speed) * loadGain(rec, load) * declGain(rec, decls), cap = rec.uMax;
  for (let c = 0; c < n; c++) {
    const W = rec.W[c];
    let s = 0;
...
export function logSpec(rec) {
  ...
  return {
    ...
    // AND THE DECLARED SCALARS, because a decision that was FADED by one cannot be replayed
    // without it — the spec is checked SUFFICIENT by replaying from these fields alone and
    // nothing else (plan §77, §90.4), so a guard that reads a field the spec omits would make
    // that replay disagree rather than merely look incomplete.
    // NAMED WHETHER OR NOT THE GUARD IS ARMED (plan §102), which OVERSTATES the budget by the
    // declared count when it is not. That direction is deliberate: a superset of the fields is
    // safe for replay and an understated log budget is not, and an investigation wants the
    // operating point recorded whether or not it faded anything.
    declared: rec.report && rec.report.declSpan ? Object.keys(rec.report.declSpan) : [],
    numbersPerDecision: offs.size * rec.refDim + (rec.report && rec.report.speedSpan ? 1 : 0)
      + (rec.stateDim || 0)
      + (rec.report && rec.report.declSpan ? Object.keys(rec.report.declSpan).length : 0),
  };
}
 ...
 */
export function explain(rec, look, speed = null, state = null, load = null, decls = null) {
  ...
  const g = coverageGain(rec, speed) * loadGain(rec, load) * declGain(rec, decls), cap = rec.uMax;
  const channels = [];
