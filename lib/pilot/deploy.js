 ...
 * @param {number[]|null} state        only if the record carries `stateDim`
 */
export function decide(rec, look, speed = null, state = null, load = null) {
  ...
  const row = featureRow(rec, look, state), g = coverageGain(rec, speed) * loadGain(rec, load), cap = rec.uMax;
  for (let c = 0; c < n; c++) {
    const W = rec.W[c];
    let s = 0;
 ...
 */
export function explain(rec, look, speed = null, state = null, load = null) {
  ...
  const g = coverageGain(rec, speed) * loadGain(rec, load), cap = rec.uMax;
  const channels = [];
