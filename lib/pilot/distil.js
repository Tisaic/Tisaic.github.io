...
export class DistilPolicy {
  ...
  constructor(o) {
    ...
    // THE STATE TERM (plan §52.27). The converged correction this block regresses is NOT a
    // function of the reference window alone: on the arm it reads R² 0.83 leave-one-program-out
    // on the window and 0.97 with the newest measured sample, pose-scheduled, beside it —
    // 0.995/0.953 on a square the fit never saw against 0.84/0.82. `stateDim` columns are
    // appended to every row, supplied by the host at fit (`stateAt`) and at every act; a
    // policy fitted with a state refuses to act without one rather than acting on zeros.
    // WHAT IT IS AND WHAT IT DELIVERS, so nobody reads the R² as a result: the gain is
    // entirely in the measured DEVIATION from the reference (the reference's own newest sample
    // scheduled the same way adds nothing, 0.80 against 0.84), so the term is FEEDBACK through
    // the plant, and on the arm it deploys BELOW the reference-only map — 1.44x fitted on the
    // prefix's state, 3.24x with the policy's own state aggregated over rounds, against 6.04x
    // without it — because the tool answers a command over ~1,000 steps (§52.26.5). Opt-in.
    this.stateDim = o.stateDim | 0;
    ...
    // ---- THE PLANT-SIDE COVERAGE GUARD (plan §82). Opt-in, off, and byte-identical unset.
    //
    // The speed guard above fades the correction outside the commanded-speed span the fit saw.
    // §75 recorded that there is NO analogue for the PLANT — the object cannot tell the machine
    // changed — and §81 measured what that costs: under a sustained unmodelled load the policy
    // goes on applying full authority, silently, past the point where it makes the machine worse
    // (0.87x at one tauMax), and because its demand is ADDITIVE on one drive it CONSUMES the
    // headroom the loop needs (14.0% saturation against the bare machine's 8.1% at half tauMax).
    //
    // `load` is a normalised plant-side distress reading in [0,1] — on a servo, the fraction of
    // recent steps the drive could not deliver what it was asked for. It is FREE on a real
    // installation: the drive already reports its own limit status, unlike a tracker.
    //
    // The threshold carries no picked constant in the plant's units: `report.loadMax` is the
    // worst reading the COMMISSIONING itself observed, stored exactly as the speed span and
    // `bendMax` are, and the guard fades between MULTIPLES of it. It can only ever multiply the
    // output by a factor in [0,1], so it cannot add energy and cannot destabilise anything.
    this.loadGuard = !!o.loadGuard;
    this.loadLo = o.loadLo ?? 4;
    this.loadHi = o.loadHi ?? 10;
    // THE WINDOW-CORRUPTION GUARD (plan §78), OPT-IN and OFF: its false-refusal rate is measured
    // at zero on a sine and is UNCHARACTERISED on any plant that holds or ramps (§78.5).
    this.bendGuard = !!o.bendGuard;
    this.bendMargin = o.bendMargin ?? 8;
    // THE DECLARED-POINT GUARD (plan §90.4), OPT-IN and OFF — and it is opt-in because of what
    // happened when it was finally reached (plan §100, §102). DECLARING a scalar and REFUSING on
    // it are different things and were one switch: the span was recorded by `addProgram` and
    // `_declCoverage` armed itself off the presence of that record, so a host that declared
    // `vLine` purely so `logSpec` and `explain` could name it in a forensic log — which is §77's
    // whole column, and the reason `logSpec` lists `declared` at all — silently bought a refusal.
    // Measured on the mill it costs 2.020x -> 1.000x and 1.474x -> 1.000x, both of them rows where
    // the frozen object HELPS. So the flag lives here beside the other two, and a declaration is
    // free: the span is still recorded, still reported and still named in the log spec.
    //
    // This is rule 9b's mirror image. That rule came from three guards shipped ARMED AND
    // UNREACHABLE; this one was REACHABLE AND ARMED WITHOUT BEING ASKED.
    this.declGuard = !!o.declGuard;
    /*
     * STANDARDISED ROWS (`standardize`, plan §52.17). The window's features are the reference
     * at 23 offsets as DIFFERENCES from now, so their scales differ by the offset — a step's
     * worth of travel against a lap's — and the absolute pose sits beside them at radians. A
     * ridge and a covariance prior act on every feature alike (rule 32), so on an EXACTLY
     * linear target the streaming fit returned weights 30-150x the batch fit's and refused
     * rows the batch route deployed. With this on, each feature is divided by its rms over
     * the training rows — the scale is measured on a first pass over the rows, which are
     * rebuilt from the references rather than stored, and is carried into `toJSON` so the
     * deployed row is divided by the same numbers.
     */
    this.standardize = !!o.standardize;
    this.xScale = null;
    this._pending = [];
    /*
     * A POSE-SCHEDULED MAP (`schedule: 'pose'`, plan §52.17). Fitted on the bench square ITSELF
     * the window reads 9-14x on it; fitted on four polygons it reads 6x, and with the square
     * ADDED to those four it reads 7.5x — the polygons dilute it. One linear map of the window
     * is being asked to serve geometries whose inverse differs, because a compliant arm's
     * inverse is pose-dependent: the Jacobian, the gravity load and the reflected inertia all
     * move with q. So every window feature is also offered multiplied by the pose's offset
     * from `scheduleCentre` — a first-order schedule on the pose, three times the features.
     */
    this.schedule = o.schedule || null;
    this.scheduleCentre = o.scheduleCentre ? Array.from(o.scheduleCentre) : [0, 0];
    ...
    // THE DECLARED OPERATING POINT'S SPAN, tracked exactly as the commanded-speed span is
    // (plan §90.4). `{ name: [lo, hi] }`, empty until a host declares something, so every
    // existing fit produces `declSpan: null` and is byte-identical.
    this._decl = Object.create(null);
    this._bMax = 0;   // the worst window bend the FIT saw — the corruption guard's threshold (§78)
  }

  /** Features per row, computable before any data arrives. */
  get nFeatures() {
    const base = this.refDim * this.offsets.length + 2 * this.refDim * this.signOffsets.length;
    return base * (this.schedule ? 3 : 1) + this.stateDim + 1;
  }
  ...
  _rowFrom(get, state = null) {
    ...
    if (this.schedule) {
      const n0 = r.length, d1 = q0[0] - this.scheduleCentre[0], d2 = q0[1] - this.scheduleCentre[1];
      for (let j = 0; j < n0; j++) r.push(r[j] * d1);
      for (let j = 0; j < n0; j++) r.push(r[j] * d2);
    }
    if (this.stateDim) {
      if (!state || state.length !== this.stateDim) throw new Error(`DistilPolicy: fitted with a ${this.stateDim}-term state and given ${state ? state.length : 'none'}`);
      for (let j = 0; j < this.stateDim; j++) r.push(state[j]);
    }
    r.push(1);
    if (this.xScale) for (let j = 0; j < r.length; j++) r[j] /= this.xScale[j];
    return r;
  }

  _row(refAt, k, speedAt, stateAt = null) {
    // A closed program's `refAt` wraps; a finite record's window is clamped at its start.
    const look = this.closed ? (o) => refAt(k + o) : (o) => refAt(Math.max(0, k + o));
    const r = this._rowFrom(look, stateAt ? stateAt(k) : null);
    if (speedAt) {
      const s = speedAt(k); if (s > 0) { if (s < this._sLo) this._sLo = s; if (s > this._sHi) this._sHi = s; }
      // THE WORST BEND THIS COMMISSIONING SAW, recorded exactly as the speed span above it is
      // (plan §78). It is the window-corruption guard's threshold, so it is the PLANT'S OWN number
      // measured on the plant's own rows rather than a constant anyone picked — and it is tracked
      // only where `speedAt` is, i.e. on the FIT's rows, so a row replayed for scoring cannot widen
      // the very envelope it is being judged against.
      const bv = this.bendOf(look);
      if (Number.isFinite(bv) && bv > this._bMax) this._bMax = bv;
    }
    return r;
  }
   ...
   */
  addProgram({ refAt, n, prefix, speedAt, stride = 1, closed = false, stateAt = null, declare = null }) {
    // A DECLARED SCALAR IS A PROPERTY OF THE PROGRAM'S OPERATING POINT, so it widens the span here
    // rather than being set once on the policy: a diet commissioned across two line speeds should
    // carry a span that covers both, which is the only way this guard is ever anything but a
    // refusal outside one point (plan §90.4).
    if (declare) {
      for (const name of Object.keys(declare)) {
        const v = declare[name];
        if (!Number.isFinite(v)) continue;
        const cur = this._decl[name];
        this._decl[name] = cur ? [Math.min(cur[0], v), Math.max(cur[1], v)] : [v, v];
      }
    }
    if (this.stateDim && !stateAt) throw new Error('DistilPolicy.addProgram: this policy carries a state term and the program supplies no stateAt');
    // WITH STANDARDISATION, THE FIRST PASS ONLY MEASURES THE SCALE: rows are built, their
    // squares accumulated, and the program kept by reference to be streamed at `fit()` once
    // the scale is known. Nothing but the descriptor and one vector of sums is retained.
    if (this.standardize && !this.xScale) {
      this.closed = closed;
      if (!this._ss) { this._ss = new Float64Array(this.nFeatures); this._ssN = 0; }
      let used = 0;
      const lo = closed ? -1 : -this.offsets[0];
      for (let i = lo + 1; i < n; i += stride) {
        if (!prefix[i]) continue;
        const row = this._row(refAt, i, null, stateAt);
        for (let j = 0; j < row.length; j++) this._ss[j] += row[j] * row[j];
        this._ssN++; used++;
      }
      this._pending.push({ refAt, n, prefix, speedAt, stride, closed, stateAt });
      return used;
    }
    // A CLOSED LAP HAS NO START. The window is built from `refAt(k + o)`, and for a finite
    // record the past is clamped at index 0 — right for a record that begins there. A closed
    // program's reference before k = 0 is its reference at n - |o|, which is exactly what the
    // deploy path reads (`lookRaw` wraps modulo the lap), and the rows here began at
    // `-offsets[0] + 1` so that no clamped window entered the fit — which dropped the first
    // 2,304 steps of every training lap, 32% of each program, and the deployed policy then
    // read wrapped windows at the lap's start that the fit had never seen. Measured: 538 rows
    // used of a 7,145-step lap at stride 9, where 794 exist. `closed` wraps the window and
    // admits every row; a finite record keeps the clamp and the skip.
    this.closed = closed;
    ...
    const lo = closed ? -1 : -this.offsets[0];
    let used = 0;
    if (this.online && this._rls && !this._pre) {
      // A policy restored with its recursion continues learning; its prequential tally restarts.
    ...
    for (let i = lo + 1; i < n; i += stride) {
      if (!prefix[i]) continue;
      const row = this._row(refAt, i, speedAt, stateAt);
      if (this.online) {
        const y = [];
  ...
  fit(rand = Math.random, nControl = 5) {
    if (this.standardize && !this.xScale && this._pending.length) {
      const sc = new Float64Array(this.nFeatures);
      for (let j = 0; j < sc.length; j++) {
        const rms = this._ssN ? Math.sqrt(this._ss[j] / this._ssN) : 0;
        sc[j] = rms > 1e-12 ? rms : 1;
      }
      sc[sc.length - 1] = 1;                       // the bias stays a bias
      this.xScale = sc;
      const pend = this._pending; this._pending = [];
      for (const d of pend) this.addProgram(d);
    }
    ...
    this.report = {
      ...
      declSpan: Object.keys(this._decl).length ? { ...this._decl } : null,
      bendMax: this._bMax > 0 ? this._bMax : null,
      reason: deploy ? null
   ...
   */
  /**
   * HOW BENT A WINDOW IS AGAINST ITS OWN TYPICAL BEND (plan §78) — the worst interior tap's
   * departure from the line through its neighbours, over the median tap's. A RATIO, so the
   * plant's own curvature divides out and no constant crosses plants. `lib/pilot/deploy.js`
   * reimplements it and `test/pilot/artefact.test.mjs` pins the two identical.
   */
  bendOf(look) {
    const D = this.refDim, offs = this.offsets;
    ...
    for (let i = 1; i < offs.length - 1; i++) {
      ...
      const t = (ob - oa) / (oc - oa);
      let sm = 0;
      for (let d = 0; d < D; d++) { const lin = a[d] + t * (c[d] - a[d]); sm += (b[d] - lin) ** 2; }
      res.push(Math.sqrt(sm) / (oc - oa));
    }
    res.sort((x, y) => x - y);
  ...
  /** The guard's verdict for one window: true if this window is to be REFUSED (plan §78). */
  _bentAway(look) {
    // OPT-IN (plan §78.5). Armed by the presence of `bendMax` it refused healthy windows on any
    // plant that HOLDS or RAMPS — a locally straight window has a median bend of exactly zero, so
    // one tap off that line reads Infinity — and the sweep that licensed arming it scored both
    // halves on a sine, which is never locally straight (rule 15).
    const bm = this.bendGuard ? (this.report && this.report.bendMax) : 0;
    return bm > 0 && this.bendOf(look) > bm * (this.bendMargin || 8);
  }

  act(refAt, k, speed = null, state = null, load = null, decls = null) {
    if (!this.W || !this.report?.deploy) return new Array(this.channels).fill(0);
    if (this._bentAway((o) => refAt(k + o))) return new Array(this.channels).fill(0);
    const row = this._row(refAt, k, null, state ? () => state : null);
    const g = this._coverage(speed) * this._load(load) * this._declCoverage(decls);
    const out = new Array(this.channels);
    for (let c = 0; c < this.channels; c++) {
      let s = 0; const W = this.W[c];
      for (let j = 0; j < row.length; j++) s += W[j] * row[j];
      s *= g;
      // A NON-FINITE WINDOW MUST NOT REACH THE MACHINE: every comparison with NaN is FALSE, so
      // the clamp below passes it straight through (plan §77.4, rule 55). Zero is the correct
      // control action and not a sentinel (rule 26) — no correction, i.e. the machine below.
      out[c] = Number.isFinite(s) ? (s > this.uMax ? this.uMax : s < -this.uMax ? -this.uMax : s) : 0;
    }
    return out;
  }
   ...
   */
  actLook(look, speed = null, state = null, load = null, decls = null) {
    if (!this.W || !this.report?.deploy) return new Array(this.channels).fill(0);
    if (this._bentAway(look)) return new Array(this.channels).fill(0);
    const row = this._rowFrom(look, state);
    const g = this._coverage(speed) * this._load(load) * this._declCoverage(decls);
    const out = new Array(this.channels);
    for (let c = 0; c < this.channels; c++) {
      let s = 0; const W = this.W[c];
   ...
   */
  observe(look, speed, applied, truth, state = null) {
  ...
  toJSON() {
    ...
    const r = this._rls;
    return {
      v: 1, channels: this.channels, refDim: this.refDim, stateDim: this.stateDim,
      ...
      coverageFade: this.coverageFade, loadGuard: this.loadGuard, loadLo: this.loadLo, loadHi: this.loadHi,
      online: this.online, stride: this.stride || 1, closed: !!this.closed,
      // `declGuard` is NEW in §102 and the record stays at v1 DELIBERATELY. §52.37 bumped the
      // version when a missing field could silently DIFFER; this one cannot — a record written
      // before §102 carries `declSpan: null`, because no host declared anything, and an absent
      // flag against a null span is the same behaviour either way. Retiring every stored record
      // for a field that cannot differ would be the caution, not the correctness.
      bendGuard: this.bendGuard, bendMargin: this.bendMargin, declGuard: this.declGuard,
      standardize: !!this.standardize, xScale: this.xScale ? Array.from(this.xScale) : null,
      schedule: this.schedule, scheduleCentre: this.scheduleCentre,
  ...
  static fromJSON(j) {
    ...
    const pol = new DistilPolicy({ channels: j.channels, refDim: j.refDim, stateDim: j.stateDim || 0, offsets: j.offsets,
      ...
      loadGuard: j.loadGuard, loadLo: j.loadLo, loadHi: j.loadHi,
      bendGuard: j.bendGuard, bendMargin: j.bendMargin, declGuard: j.declGuard,
      schedule: j.schedule || null, scheduleCentre: j.scheduleCentre });
    pol.stride = j.stride || 1;
    pol.closed = !!j.closed;
    pol.standardize = !!j.standardize;
    if (j.xScale) pol.xScale = Float64Array.from(j.xScale);
  ...
  /** The coverage guard's gain at a commanded speed: 1 inside the trained span, fading to 0 outside. */
  coverage(speed) { return this._coverage(speed); }

  /** The plant-side guard's gain: 1 while the machine is no more distressed than commissioning saw. */
  load(load) { return this._load(load); }

  _load(load) {
    const lm = this.report?.loadMax;
    // NOT ARMED, NO READING, OR NO SCALE — and the last is rule 25, not a default: a commissioning
    // that observed no distress at all has no scale to judge distress against, so the guard
    // reports itself unavailable rather than inventing an absolute threshold.
    if (load == null || !this.loadGuard || !(lm > 0)) return 1;
    const lo = lm * this.loadLo, hi = lm * this.loadHi;
    if (!(hi > lo) || load <= lo) return 1;
    if (load >= hi) return 0;
    return smooth((hi - load) / (hi - lo));
  }
  ...
  /**
   * A DECLARED OPERATING POINT'S COVERAGE (plan §90.4), the exact twin of `_coverage` one axis
   * over — and it exists because §89.2 measured that this project's one disturbance-rejection win
   * rests on TWO declarations that behave completely differently.
   *
   * The cold mill's roll phase is read by an ENCODER, so it is honest at any line speed and a
   * change of operating point leaves the object EXACTLY inert (2.625x → 2.625x across ±10% of
   * target gauge, to three figures). Its transport delay is a NUMBER TYPED IN at commissioning,
   * and the window the fit chose against it is baked into the WEIGHTS — so at another line speed
   * the map leads by the wrong amount and reads 0.770x then 0.562x of its commissioned factor.
   * The library could not tell those apart: `refDim` merely widens and `deadTime` is a constant.
   *
   * `declare: [{name, value}]` lets a host state a scalar the weights depend on. The fit records
   * the SPAN observed, exactly as it records the commanded-speed span, and at deploy a value
   * outside it fades the correction to nothing rather than applying a map fitted somewhere else.
   *
   * **A POINT SPAN REFUSES OUTSIDE ITSELF, AND THAT IS THE DESIGN RATHER THAN A LIMITATION.** A
   * commissioning at one line speed has NO evidence about another, so the honest gain there is
   * zero, and widening it with a tuned margin would be a per-plant constant invented to soften a
   * refusal (rule 31). The way to get tolerance is the way target 2 already learned it:
   * *feed-invariance comes from TRAINING ACROSS FEEDS, never from INDEXING BY FEED.* Declare the
   * scalar across a span at commissioning and the guard carries that span.
   *
   * The comparison is relative, because a declared scalar can be any magnitude and an absolute
   * epsilon would be a constant in the plant's units (rule 32).
   */
  _declCoverage(decls) {
    const sp = this.report?.declSpan;
    // NOT ARMED IS NOT FADED (plan §102): the span is recorded whenever a host declares, because
    // an investigation wants it and `logSpec` names it, but it only reaches the output when the
    // caller asked for a guard. Declaring is forensics; refusing is a control decision.
    if (!this.declGuard || !sp || !decls) return 1;
    let g = 1;
    for (const name of Object.keys(sp)) {
      const v = decls[name];
      // NOT TOLD IS NOT IN RANGE, and it is not out of range either — a host that declared a
      // scalar at commissioning and says nothing at deploy is a wiring fault, not an operating
      // point (rule 25). It reads as full coverage and `report.declSpan` is what an investigation
      // checks, because a guard that silently refuses on a missing field is unfindable.
      if (v == null) continue;
      const [lo, hi] = sp[name];
      if (v >= lo && v <= hi) continue;
      const m = (hi - lo) * this.coverageFade;
      if (!(m > 0)) return 0;            // a POINT span: no evidence anywhere else
      g = Math.min(g, v < lo ? smooth((v - (lo - m)) / m) : smooth(((hi + m) - v) / m));
    }
    return g;
  }
