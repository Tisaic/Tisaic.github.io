...
export class DistilPolicy {
  ...
  get nFeatures() {
    return 2 + 2 * (this.offsets.length - 1)
      + 2 * this.channels * this.signOffsets.length + 1;
  }

  _row(refAt, k, speedAt) {
    const q0 = refAt(Math.max(0, k));
    const r = [q0[0], q0[1]];
    for (const o of this.offsets) {
      if (o === 0) continue;
      const q = refAt(Math.max(0, k + o));
    ...
    for (const o of this.signOffsets) {
      const a = refAt(Math.max(0, k + o - 1)), b = refAt(Math.max(0, k + o + 1));
   ...
   */
  addProgram({ refAt, n, prefix, speedAt }) {
    const lo = -this.offsets[0];
    let used = 0;
    for (let i = lo + 1; i < n; i++) {
      if (!prefix[i]) continue;
      this.X.push(this._row(refAt, i, speedAt));
  ...
  cost() {
    ...
    const rowOps = 2 * (this.offsets.length - 1) + 2 * this.channels * this.signOffsets.length;
