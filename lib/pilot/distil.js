...
export class DistilPolicy {
  ...
  get nFeatures() {
    ...
    return base * (this.schedule ? 3 : 1) + 1;
  }
  ...
  _row(refAt, k, speedAt) {
    ...
    const r = this._rowFrom(this.closed ? (o) => refAt(k + o) : (o) => refAt(Math.max(0, k + o)));
   ...
   */
  addProgram({ refAt, n, prefix, speedAt, stride = 1, closed = false }) {
    ...
    if (this.standardize && !this.xScale) {
      ...
      for (let i = lo + 1; i < n; i += stride) {
        ...
        const row = this._row(refAt, i, null);
        for (let j = 0; j < row.length; j++) this._ss[j] += row[j] * row[j];
        this._ssN++; used++;
      }
      this._pending.push({ refAt, n, prefix, speedAt, stride, closed });
      return used;
    }
