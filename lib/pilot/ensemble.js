 ...
 *
 * AND THAT RESULT NO LONGER REPRODUCES, BECAUSE THE GATE IT VOUCHED AGAINST IS GONE. It was taken
 * before `verifyRef` gave the tank a representative regime, and re-run against the current gate
 * (`test/_tankens.mjs`, five seeds, `freezeConfig` giving 5 of 5 draws in ONE layout — a cleaner
 * average than the 5 of 8 above) the draws read verify 0.98 / 0.63 / 1.10 / 0.91 / 0.68 and the
 * AVERAGE READS 0.98x AND REFUSES. One draw deploys, at 1.10x, and delivers 1.086x.
 *
 * So the paragraph above is kept as history and is not the current behaviour: on this plant, under
 * this gate, averaging does not convert the refusal. The mechanism may still be sound — the spread
 * across draws is real and the average is still one vector for k commissionings — but the claim
 * that it VOUCHES where the draws could not was a property of the old gate, and a module that went
 * on asserting it would be describing the behaviour it used to have (rule 30).
 *
...
/**
 * FREEZE ONE PILOT'S CHOSEN CONFIGURATION ONTO ANOTHER, so k draws are averageable BY
 * CONSTRUCTION rather than by luck.
 *
 * WHY IT IS NEEDED. On the tank, 3 of 8 draws could not be averaged because their tuned stride
 * differed — 13, 21 and 32 across seeds, carrying the feature count from 97 to 321. That is not a
 * broken tie-break: the window sorts to the shortest lag inside the 5% band and the tune to the
 * largest ridge then the smallest stride, both exactly as rule 42 asks. What moves is which
 * candidates fall INSIDE the band, because each one's held-out score is measured on a seeded draw.
 * No tie-break repairs that; it is what selecting on a noisy score does.
 *
 * SO THE CONFIGURATION IS NOT MADE STABLE, IT IS MADE SHARED. One pilot commissions fully and its
 * window, stride, ridge and basis are copied onto the rest before they fit, so every draw differs
 * only in the excitation it saw — which is the one difference the average is meant to remove.
 *
 * IT ALSO PRICES THE FEATURE DOWN. The tune stage is a search over combinations and is a real part
 * of commissioning; running it once instead of k times, alongside sharing the probe and running a
 * single verify on the average, is what takes k draws from k commissionings to roughly
 * `1 + k·(excite + fit)`.
 *
 * CALL IT BEFORE THE FIT STAGE — the values are read there. Copying after a pilot has fitted
 * changes what its report says without changing what it did, which is a worse failure than not
 * copying at all (rule 30).
 *
 * @param {object} from a commissioned pilot whose configuration is to be reused
 * @param {object} to a pilot that has not yet fitted
 */
export function freezeConfig(from, to) {
  if (!from || !to || !from.readouts) throw new Error('ensemble: freezeConfig needs two pilots');
  to._frozenConfig = from.readouts.map((r) => ({
    stride: r.stride, ridge: r.ridge, mLag: r.mLag, fLag: r.fLag,
    poly: !!r.poly, sched: !!r.sched, longR: r.longR || null }));
  return to;
}
