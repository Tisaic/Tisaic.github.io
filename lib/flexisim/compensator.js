...
export class PositionServo {
  ...
  torque(ref, enc, ffOverride = null) {
    ...
    let tau = ff + this.kp * (ref.theta - enc.angle) + this.kd * (ref.omega - enc.speed);
...
export class ChainServo {
   ...
   */
  constructor({ arm, bandwidth = 2e-3 }) {
    this.arm = arm;
    this.bw = bandwidth;
    this.retune();
  }
  ...
  torques(refs) {
    ...
    for (let i = 0; i < 2; i++) {
      const j = i === 0 ? a.j1 : a.j2;
      const tauJoint = M[i][0] * al[0] + M[i][1] * al[1] + v[i] - g[i];
      const ff = tauJoint / j.N + j.N * j.Jm * al[i];
      const { kp, kd } = this.gains[i];
      out.push(ff + kp * (q[i] - enc[i].angle) + kd * (w[i] - enc[i].speed));
    }
    return out;
  }
}
