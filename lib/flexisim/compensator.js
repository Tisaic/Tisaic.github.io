 ...
 */
/**
 * THE SHIPPED SERVO LOOP, STATED ONCE (plan §52.37).
 *
 * `bandwidth` was hard-coded as `2e-3` in TWO places — the page's `makePlant` and the test rig's
 * `machine()` — with no link between them. Value for value they agreed, so nothing was ever wrong
 * and no check went red; the duplicate was simply waiting for one of them to move, which is rule
 * 61 exactly. It moves here, so it becomes one constant that both read.
 *
 * 1.6e-2 rather than the carried 2e-3 because that is what the machine was MEASURED to want once
 * the policy's window stopped shrinking with the loop (§52.33): swept with the reach held, the
 * bench square reads 1.5847e-1 against 1.7528e-1 and the two programs the commissioning never saw
 * improve by 1.75x and 1.71x. Note what it is NOT: the CONVENTIONAL machine is 1.2% WORSE here
 * than at 2e-3, so this is the loop the learned controller wants and not the loop that flatters
 * the plant, and it sits ~4x ABOVE the bench cell's slowest structural mode rather than below it.
 * The drive is the rig's standard 32x gravity hold — a bigger one measured slightly HARMFUL at
 * this bandwidth — so what a customer buys is a servo retune and not a bigger motor.
 */
/**
 * THE SLOWEST STRUCTURAL MODE, FROM THE PLANT ITSELF (plan §52.39).
 *
 * Every mode this needs is already on the machine: `joint.naturalFrequency()` is the gearbox
 * two-mass resonance from its own K0/N/Jm/Jl, and each link's `meta` carries the E, section and
 * density its first cantilever mode is computed from. Nothing is passed alongside, so this
 * cannot describe a plant the arm is not (rule 30) — and it is the whole point: the loop must be
 * derived from the machine rather than carried to it.
 */
export function slowestStructuralMode(arm) {
  const w = [];
  for (const j of [arm.j1, arm.j2]) if (j && j.naturalFrequency) w.push(j.naturalFrequency());
  // Euler-Bernoulli first cantilever mode of a square section, from the link's OWN metadata.
  for (const [lk, L] of [[arm.l1, arm.L1], [arm.l2, arm.L2]]) {
    const m = lk && lk.meta; if (!m || !(L > 0)) continue;
    const h = m.H, I = h ** 4 / 12, A = h * h;
    w.push((1.875 ** 2) * Math.sqrt(m.E * I / (m.rho * A * L ** 4)));
  }
  return w.length ? Math.min(...w) : null;
}

/**
 * THE SHIPPED SERVO LOOP, AND A DERIVED RULE THAT WAS BUILT AND REFUSED (plan §52.39).
 *
 * §52.37 made this one constant (it had been hard-coded in the page AND the rig, rule 61) and
 * moved it to 1.6e-2 — which fixed the duplicate and RE-CREATED the fault it replaced: a single
 * absolute bandwidth carried across a 256-fold stiffness range, exactly what §52.28 indicted
 * about the 2e-3. So it was rebuilt as a RATIO against the plant's own slowest structural mode,
 * derived from the machine by `slowestStructuralMode` with nothing passed alongside, at the 4.1472
 * the bench cell measured — which reproduces the shipped loop there to the last bit, and that
 * control came back EXACT (1.6159e-1 -> 1.3025e-1, held-out 9.0436e-3 / 6.2215e-3, byte-identical).
 *
 * AND THE SOFT CELL REFUSED IT. At K 0.25 / E 0.005 the same ratio gives 6.63e-3, and measured
 * against both fixed loops in absolute contour rms after learning:
 *
 *   loop            square      rounded rect   circle
 *   2e-3 (old)      3.2211e-1    3.4771e-2     2.5537e-2
 *   1.6e-2 (ships)  2.6192e-1    2.1061e-2     1.2645e-2      <- best on all three
 *   6.63e-3 (rule)  4.6285e-1    3.0491e-2     3.0533e-2      <- worst on all three
 *
 * The soft cell's own slowest mode is 1.575e-3, so what it actually wants is **10.2x** its
 * structure where the bench cell wants 4.15x. The ratio does not transfer, so "bandwidth = k x
 * slowest mode" is not the law — the third narrowing of §52.28's structural rule, after §52.30
 * reduced it to a fit on the bench square and §52.33 inverted its sign. The fixed 1.6e-2 stays
 * BECAUSE it is what both measured cells prefer, and it is 1.60x geometric better than the old
 * 2e-3 on the soft cell as well as 1.65x on the bench. It remains a carried constant and this
 * comment is the reason it is one; `slowestStructuralMode` stays exported as the instrument that
 * refused the rule, so the next attempt starts from the measurement rather than repeating it.
 */
export const BENCH_SERVO = { bandwidth: 1.6e-2, drive: 32 };

/** What ships, per plant. The derived form is refused above; this is the measured constant. */
export function bandwidthFor(_arm) { return BENCH_SERVO.bandwidth; }
