...
export class Joint {
  ...
  step(tauCmd, tauLoad, dt) {
    const tau = this.stepMotor(tauCmd, dt);
  ...
  /**
   * The MOTOR half alone, returning the torque the gearbox transmitted.
   *
   * SPLIT OUT FOR A COUPLED CHAIN. On a serial arm the load side is not one
   * inertia per joint: the mass matrix is configuration-dependent and the joints
   * are coupled through it, so the load accelerations have to be solved TOGETHER
   * and cannot be integrated inside each joint. The motor half genuinely is
   * per-joint -- it sits upstream of the gear teeth, sees only the reaction
   * tau/N -- so it stays here, and step() above is the single-DOF composition of
