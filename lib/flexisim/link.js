 ...
 */
/**
 * The transverse vacuum margin, in cells. Exported and overridable ONLY so the regression
 * can build both and compare — nothing else should ever pass it.
 */
export const MARGIN = Number(globalThis.__linkMargin) || 1;
...
export async function buildLink({ length, section, clamp = 3, E, nu, rho,
                                  ...
                                  precision = 'f64' }) {
  const H = section;
  // ONE VACUUM LAYER, NOT TWO, AND IT IS AN EXACTNESS CLAIM RATHER THAN A SAVING.
  //
  // Every neighbour access in the elastic kernel is +/-1 on each axis, the diagonal shear
  // nodes `nbr(x,y,z,1,1,0)` included, so ONE vacuum layer is all a traction-free surface
  // can use: a material cell at the edge reaches vacuum and stops. The second layer was
  // being stepped and read by nothing. Measured on the shipped link over 3000 steps, tip
  // deflection, tip slope, mass and inertia all IDENTICAL TO EVERY DIGIT, at 684 cells
  // and 269 us/step against 1216 and 343 -- 1.27x, which at 60 steps/frame is the
  // difference between a 16.2 ms frame and a 20.6 ms one, and the same factor on every
  // lattice check in the Node suite.
  //
  // Because it is an exactness claim it is asserted as one: the regression builds both
  // and compares them digit for digit rather than within a tolerance. A margin that
  // changed the answer in the fourth figure would mean the surface condition depends on
  // how much empty space is around it, which would be a defect in the boundary and not a
  // tuning choice.
  const M = MARGIN;
  const NX = clamp + length, NY = H + 2 * M, NZ = H + 2 * M;
  const sim = new Simulation({ lattice: { size: [NX, NY, NZ], spacing: 1 } });
  sim.addRegion(region.all(CELL.FLUID));                        // vacuum
  sim.addRegion(region.box(CELL.ELASTIC, [0, M, M], [NX, M + H, M + H]));
  sim.addRegion(region.box(CELL.CLAMPED, [0, M, M], [clamp, M + H, M + H]));
  ...
  const pivot = [clamp - 0.5, M + H / 2 - 0.5, M + H / 2 - 0.5];
