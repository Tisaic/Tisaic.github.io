...
export class FlexArm2R {
  ...
  /**
   * @param {number} tauCmd1 motor torque, joint 1
   * @param {number} tauCmd2 motor torque, joint 2
   * @param {number} dt
   * @param {number[]|null} [load] an EXTERNAL LOAD-SIDE torque per joint — a cutting
   *   force, a payload, anything acting past the gear teeth.
   *
   *   IT ENTERS ON THE LOAD SIDE AND THAT IS THE WHOLE POINT. A disturbance injected at
   *   the motor is seen by the encoder, so the position loop rejects it and it barely
   *   reaches the tool — measured on this arm, a slow torque disturbance at 140% of the
   *   operating torque moved the tool error by 11%. A load-side torque winds the gearbox
   *   up instead, and the encoder is structurally blind to wind-up, so the loop cannot
   *   reject what it cannot see. That is the disturbance a contouring machine actually
   *   has and the one a soft sensor exists for.
   *
   *   WHAT IT DOES NOT DO is bend the links: the lattices respond to the frame's
   *   fictitious forces and to gravity, not to a force applied at the tool, so this
   *   carries the gearbox half of a tool load and not the structural half. Stated rather
   *   than hidden, because the two have different signatures and only one is modelled.
   */
  step(tauCmd1, tauCmd2, dt, load = null) {
...
/**
 * THE WHOLE MACHINE'S STATE — both lattices and both joints — and the means to put it back.
 *
 * A scored study builds a fresh machine per run so that every run starts from the same
 * state. That is right, and on this arm it costs 4.2 s of a 13.2 s run: two lattices built,
 * four poses settled for the compliance calibration, then a settle onto the program start.
 * A third of the ladder's 32 minutes goes on re-deriving a state it has already had 126
 * times.
 *
 * RESTORING IS ALSO STRICTLY MORE EXACT. Re-settling only converges towards the same state:
 * the 4000-step settle leaves the wind-up 0.25% from its 40000-step value, so the "identical"
 * machines actually differed. A snapshot is bit-identical, which means the study can ALSO
 * afford to settle properly once instead of approximately many times.
 *
 * @param {{arm: FlexArm2R, l1: object, l2: object}} m
 */
export function snapshotArm(m) {
  const j = (jt) => ({ thM: jt.thM, wM: jt.wM, thL: jt.thL, wL: jt.wL });
  return {
    l1: m.l1.backend.captureState(), l2: m.l2.backend.captureState(),
    j1: j(m.arm.j1), j2: j(m.arm.j2),
    q: m.arm.q.slice(), w: m.arm.w.slice(), alpha: m.arm.alpha.slice(),
  };
}

/** Put a `snapshotArm` state back. The machine must be the one it was taken from. */
export function restoreArm(s, m) {
  m.l1.backend.restoreState(s.l1); m.l2.backend.restoreState(s.l2);
  const put = (jt, v) => { jt.thM = v.thM; jt.wM = v.wM; jt.thL = v.thL; jt.wL = v.wL; };
  put(m.arm.j1, s.j1); put(m.arm.j2, s.j2);
  m.arm.q = s.q.slice(); m.arm.w = s.w.slice(); m.arm.alpha = s.alpha.slice();
}
