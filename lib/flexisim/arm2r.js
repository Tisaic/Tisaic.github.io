...
import { massProperties, armLength, tipDeflection, deflectionProfile } from './link.js';
...
export class FlexArm2R {
  ...
  /**
   * Tool-tip position error transverse to link 2, against where the ENCODERS say
   * the tip is -- the controller's own picture, which is wrong by BOTH wind-ups and
   * both links' bending.
   *
   * THE FIRST JOINT'S WIND-UP IS LEVERED BY THE WHOLE ARM, not just by link 2: an
   * angular error at the shoulder moves the tool by that angle times the distance
   * from the SHOULDER, which is why it is multiplied by the reach and not by L2.
   * Link 1's bending tilts everything downstream of it as well, but that term needs
   * the slope at its tip rather than its deflection, so it is reported separately
   * rather than folded in with a lever arm it does not have.
   */
  tipError() {
    const reach = this.toolRadius();
    const tilt1 = -this.j1.windup() * reach;
    const tilt2 = -this.j2.windup() * this.L2;
    const bend1 = tipDeflection(this.l1);
    const bend2 = tipDeflection(this.l2);
    return { tilt1, tilt2, bend1, bend2, total: tilt1 + tilt2 + bend1 + bend2 };
  }
