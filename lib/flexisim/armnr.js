...
export class FlexArmNR {
  ...
  /**
   * Tool-tip error against where the ENCODERS say it is.
   *
   * EACH JOINT'S WIND-UP IS LEVERED BY THE DISTANCE FROM THAT JOINT TO THE TOOL,
   * not by its own link: an angular error at the base moves the tool by that angle
   * times the whole reach. Each link's bending is reported separately rather than
   * folded in with a lever arm it does not have -- carrying a link's bending
   * downstream needs the SLOPE at its tip, not its deflection.
   */
  tipError() {
    const tilt = [], bend = [];
    let total = 0;
    for (let i = 0; i < this.n; i++) {
      const arm = this._reachFrom(i);
      const t = -this.joints[i].windup() * arm;
      const b = tipDeflection(this.links[i]);
      tilt.push(t); bend.push(b);
      total += t + b;
    }
    return { tilt, bend, total };
  }
