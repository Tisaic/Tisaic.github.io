...
//   reaction   the torque the LINK pushes back through the gear teeth, reflected to
//              the motor (transmitted()/N) -- opt-in, and it is the one measured
//              quantity that survives the machine being PARKED. During a dwell the
//              encoder speed is zero, the acceleration is zero, the pose is fixed
//              and the commanded torque is just the static gravity term, while the
//              tool is still ringing: a flat input must give a flat output whatever
//              the model. The Move tab's single-joint sensor learned this first
//              (v222, where it doubled the estimate's motion during a dwell); this
//              one had no equivalent until a settling-time control made the parked
//              stretch a large fraction of the record and the score fell to 0.64
//              against a naive 1.04. The muting by 1/N costs nothing, measured
//              there: the model standardises its inputs, so a constant factor
//              vanishes into the weights and no torque transducer is needed.
...
export class ChainSensor {
   ...
   */
  /**
   * THE WINDOW HAS TO REACH THE RING, WHICH IT DID NOT, and a settling-time control is
   * what exposed it. The chain's measured bending mode is ~860 steps and the shipped
   * window reached 3 lags x stride 2 x 10 = 40 steps of it -- 4.6% of one cycle -- so the
   * phase of a free vibration was outside what the model could see even in principle. It
   * scored well anyway while every move ran into the next, and collapsed the moment there
   * was a real dwell for the tool to ring through. Exactly the defect v222 found on the
   * single-joint sensor; this one never got the fix.
   *
   * MEASURED, one plant, one stream per row, at a dwell of 1911 steps (34% of samples with
   * both joints parked) and at the old 900 (0% parked) -- whole-arm nRMSE, and the parked
   * stretch on its own:
   ...
   *    400 st   linear     61   0.2206 (parked 0.31)      —
   ...
   * The shipped configuration was WORSE THAN PREDICTING THE MEAN on the long dwell, and
   * it is 23x better with the window right. LINEAR BEATS THE UNIVERSAL MAP at every
   * window and at a fraction of the cost -- also v222's finding, for the same reason: once
   * the model can see the phase it does not need a rich basis to guess it.
   *
   * THE REACTION TORQUE WAS THE OBVIOUS FIX AND IT IS NOT THE FIX HERE. It is opt-in
   * because it is the right signal on a machine that is genuinely parked, but measured on
   * this plant it made the whole-arm model WORSE (1.2367 -> 1.6973) while the window was
   * still short, and the elbow-only one only slightly better. The window was the problem.
   */
  constructor({ joints = [0, 1], sampleEvery = 10, lag = 18, stride = 6, warmup = 200,
                features = 'linear', initVariance = 100, seed = 11, lam = 1.0,
                lead = 0, reaction = false } = {}) {
    this.joints = joints.slice();
    this.sampleEvery = sampleEvery;
    this.lag = lag; this.stride = stride;
    this.lead = lead;
    this.reaction = reaction;
    this.ns = this.joints.length * (SIGNALS_PER_JOINT + (reaction ? 1 : 0));
  ...
  signals(arm, tauCmd, dt = 1) {
    ...
    for (const j of this.joints) {
      ...
      // PER JOINT, because each gearbox is loaded by its own link AND by the coupling.
      if (this.reaction) {
        const jt = j === 0 ? arm.j1 : arm.j2;
        out.push(jt.transmitted() / jt.N);
      }
    }
  ...
  status() {
    return { mode: this.mode, trained: this.trained, features: this.ss.nf,
      ...
      reaction: this.reaction, lag: this.lag, stride: this.stride,
      // …AND THE REACH IN SOLVER STEPS, which is the number that has to be compared with
      // the measured ring. Reporting the lag count alone is what let the page describe a
      // window three times narrower than the one it was running.
      reach: (this.lag - 1) * this.stride * this.sampleEvery,
      lead: this.lead };
  }
}
