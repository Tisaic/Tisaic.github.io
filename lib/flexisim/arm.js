...
export class FlexArm {
  ...
  /**
   * Tip position error transverse to the link, in world units, measured against
   * WHERE THE ENCODER SAYS THE TIP IS -- the controller's own picture. It is the
   * rigid tilt the gearbox wind-up introduces plus the link's own bending, and it
   * is what a laser tracker measures and the encoder cannot.
   *
   * THE TILT IS MINUS THE WIND-UP, AND THE SIGN IS NOT A CONVENTION. theta_link =
   * theta_encoder - windup, so a positive wind-up puts the link BEHIND where the
   * encoder says it is, displacing the tip by -windup*L. Under gravity that is the
   * SAME direction as the sag, so the two ADD: a hanging link lags its motor AND
   * bends downward. The original + here SUBTRACTED them, which made the reported
   * error smaller than either contribution at some stiffnesses and reversed its
   * sign at others -- with no symptom, because the physics baseline built on the
   * same formula was wrong the same way and the learner simply fitted whatever it
   * was shown.
   *
   * The identity that pins it, and it is asserted: for any commanded angle,
   *   tipTrackingError(arm, theta_ref) === total + L*(encoder - theta_ref)
   * i.e. the reference-relative error differs from the encoder-relative one by
   * exactly the following error, which the controller knows. Under the old sign
   * that identity fails by 2*windup*L.
   */
  tipError() {
