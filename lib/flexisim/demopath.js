...
export function randomPolygon(rnd, feed, { centre = [12, 0], star = false,
  accel = 4e-5, cornerDt = 40, rMin = 2.2, rSpan = 1.6 } = {}) {
  const [cx, cy] = centre;
  const n = star ? 8 + 2 * Math.floor(2 * rnd()) : 4 + Math.floor(3 * rnd());
  // THE RADIUS RANGE IS A PARAMETER AND ITS DEFAULT IS THE MEASURED DIET. `designDemoPaths`
  // exists to reproduce the section 36/37 numbers byte for byte, so the defaults here are the
  // ones those were taken on; a caller that needs shapes at the SCALE of the programs under
  // test says so rather than getting a different diet silently.
  const r0 = rMin + rSpan * rnd();
  const pts = [];
  for (let i = 0; i < n; i++) {
...
export function designDemoPaths({ centre = [12, 0], feeds = [4e-3, 8e-3, 5.5e-3],
  seed = 81, accel = 4e-5, cornerDt = 40, rMin = 2.2, rSpan = 1.6 } = {}) {
  const mk = (s) => { let z = s >>> 0;
    return () => (z = (z * 1664525 + 1013904223) >>> 0) / 4294967296; };
  const out = [];
  feeds.forEach((f, i) => out.push(randomPolygon(mk(seed + i), f,
    { centre, accel, cornerDt, rMin, rSpan })));
  feeds.forEach((f, i) => out.push(randomPolygon(mk(seed + 4 + i), f,
    { centre, star: true, accel, cornerDt, rMin, rSpan })));
  return out;
}

/**
 * A LONG CLOSED TOUR — several random shapes chained into ONE replayable lap, for the case
 * where a window has to REACH the plant's memory without becoming a lap index (plan §49).
 *
 * WHY IT EXISTS. This arm's elbow memory is 6363-8649 solver steps and a program lap is 7356,
 * so a correction kernel long enough to describe the plant is longer than the program it is
 * fitted on — and a feature window that long can locate itself IN the lap. Measured: a +/-1024
 * sample window takes the fitted programs to 24.93x and the held-out one to 0.47x, worse than
 * doing nothing. `randomWander` states the same theorem for identification and answers it by
 * being OPEN and non-periodic; iteration cannot use an open path, because the correction it
 * accumulates has to be replayed against the same trajectory.
 *
 * SO THIS IS THE THIRD OPTION: closed, therefore replayable, and LONG — several shapes at
 * different scales and orientations chained through the corner rule, so one lap is many times
 * the plant's memory and a reaching window covers a small fraction of it. The shapes are the
 * measured diet's own (random polygons and stars, which bracket a square's 90 degrees), placed
 * around the workspace rather than concentric so the tour carries pose variation as well.
 *
 * @param {() => number} rnd  a [0,1) generator; the caller owns the seed
 * @param {number} feed
 * @param {object} o { centre, nShapes, spread, rMin, rSpan, accel, cornerDt }
 * @returns {ToolPath} one closed path
 */
export function designTour(rnd, feed, { centre = [12, 0], nShapes = 6, spread = 1.6,
  rMin = 2.0, rSpan = 1.6, accel = 4e-5, cornerDt = 40 } = {}) {
  const [cx, cy] = centre;
  const pts = [];
  for (let s = 0; s < nShapes; s++) {
    ...
    const n = star ? 8 + 2 * Math.floor(2 * rnd()) : 4 + Math.floor(3 * rnd());
    const r0 = rMin + rSpan * rnd();
    // The shapes are OFFSET from one another so the tour sweeps the workspace; the offset is
    // bounded by `spread` because a demo that leaves the region the programs run in is
    // measuring a different machine (the displacement sweep: banks fade with distance).
    const ox = cx + spread * (rnd() - 0.5) * 2, oy = cy + spread * (rnd() - 0.5) * 2;
    const ph = 2 * Math.PI * rnd();
    for (let i = 0; i < n; i++) {
      const th = ph + (2 * Math.PI * i) / n + 0.5 * (rnd() - 0.5) / (star ? 3 : 1);
      const r = r0 * (star ? (i % 2 ? 0.45 : 1) : 0.7 + 0.6 * rnd());
      pts.push([ox + r * Math.cos(th), oy + r * Math.sin(th)]);
    }
  }
  return new ToolPath({ start: pts[0], feed, accel, closed: true, cornerDt,
    segments: pts.slice(1).map((p) => [SEG.LINE, p]).concat([[SEG.LINE, pts[0]]]) });
