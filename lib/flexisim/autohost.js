...
export function makeArmHost(o) {
  ...
  // THE DISTILLED RUNG MAY REPLACE THE COMPLIANCE FEEDFORWARD RATHER THAN SIT UNDER IT. Under
  // it, the rung's teacher is the residual the feedforward leaves — 0.0825 on a polygon
  // against 1.22 bare, a 14.8x removal — while on the sharp square that feedforward is worth
  // only 1.29x, so the diet's residual and the program's error are different objects and the
  // distilled map reads 1.36x there. `test/_distil.mjs` distils the BARE machine's converged
  // prefix on the same diet and reads 4.97x against the conventional machine. `distilReplaces`
  // converges the training runs without the feedforward and deploys the rung without it — one
  // composition, stated, so the two can be measured against each other (plan §52.7). Every
  // run that applies the distilled rung, or is scored with it deployed, goes bare; the base
  // and the rungs below it keep the feedforward, so the gate still compares against the
  // conventional machine.
  const distilReplaces = !!o.distilReplaces;
  ...
  const auto = new AutoStack({
    ...
    frames: { classic: { uMax: 1.5, map: worldToJoint }, stack: { uMax: stackAuth },
      hff: { uMax: stackAuth }, distil: { uMax: stackAuth } },
    ...
    // near t=0 where the fast dynamics are while still reaching out to +/-512.
    ...(o.distil ? { distil: { refDim: 2, ridge: o.distil.ridge ?? 1e-6,
      offsets: o.distil.offsets
        || [-512, -256, -128, -64, -32, -16, -8, -4, -2, -1, 0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 512],
      signOffsets: o.distil.signOffsets || [-128, -32, -8, -2, 0, 2, 8, 32, 128],
    ...
    try {
      for (let l = 0; l < laps; l++) {
        ...
        for (let k = 0; k < LAP; k++) {
          ...
          const lookRaw = (off) => R[(((k + off) % LAP) + LAP) % LAP];
  ...
  function actAt(k, cmd, refs) {
    ...
    const lookRaw = (off) => R[(((kr + off) % L) + L) % L];
    const u = auto.act({ v: [cmd.vx, cmd.vy], a: [cmd.ax, cmd.ay], k, look, lookRaw, q,
    ...
    for (const dp of paths) {
      ...
      try {
        ...
        for (let i = 0; i < 4000; i++) {
          const t = servo.torques(hold); arm.step(t[0], t[1], 1); mSamples++;
  ...
  async function distilRuns() {
    ...
    for (const dp of paths) {
      ...
      const R = new Array(L), SP = new Float64Array(L);
      out.push({
        lap: L,
        refAt: (k) => R[(((k % L) + L) % L)],
        ...
        run: async (corr) => {
          ...
          // exists for the production path and this is a different geometry.
          if (!R[0]) {
            for (let k = 0; k < L; k++) {
              const c = dp.at(k);
              R[k] = arm.ik(c.x, c.y, true);
              SP[k] = Math.hypot(c.vx, c.vy);
            }
          }
          const A = AVG, W = WARMUP, laps = W + A;
          ...
          return { score: Math.sqrt(s2 / Math.max(1, n)), err: [e0, e1] };
        },
      });
    }
    return out;
