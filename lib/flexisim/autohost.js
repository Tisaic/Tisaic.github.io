...
export function makeArmHost(o) {
  ...
  const DEPLOY = { path, lap: LAP, refs: null, program: PROGRAM };
  ...
  const auto = new AutoStack({
    ...
    uMax: 3.0, periodic: o.lapMemory === false ? null : LAP, maxDepth: 2, lapSynced: lapSync,
  ...
  function attach(arm, servo, rc, livePath, liveLap) {
    ...
    if (livePath) {
      ...
      DEPLOY.lap = Math.ceil(liveLap ?? livePath.lap);
      DEPLOY.refs = null;
