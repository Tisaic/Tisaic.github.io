...
import { tipDeflection, tipSlope } from './link.js';
...
export function makeArmHost(o) {
  ...
  // ---- AN UNMODELLED EXTERNAL DISTURBANCE, FOR THE COLUMN THIS OBJECT CANNOT ANSWER (plan §81).
  //
  // `disturb(k)` returns a torque pair added at the MOTORS, on top of whatever the servo asked
  // for — the same injection point `test/pilot/stepresp.mjs` uses, which is a shove, a payload or
  // an external force depending only on its shape. It is the plant being pushed, not the command
  // being changed, and that distinction is the whole experiment: the deployed object reads the
  // COMMANDED REFERENCE and nothing else, so its output cannot depend on this. That is a claim to
  // ASSERT rather than to trust (rule 30) — a future change adding any feedback would break it
  // silently, and the bit-identity check is what would catch it.
  //
  // Null is the default and byte-identical.
  let DISTURB = o.disturb || null;
  // The worst plant-side distress any scored run has shown — what commissioning OBSERVED, which
  // is the guard's scale and is stored on the record exactly as the speed span is (plan §82).
  let satSeen = 0;
  // ---- A DISTURBANCE PRESENT DURING TRAINING (plan §83), which is a different hook from the one
  // above on purpose: `disturb` pushes the plant while it is being SCORED, and this pushes it
  // while the TEACHER is converging the prefixes the map is fitted to. Only the second can change
  // what the map learns.
  let TRAIN_DISTURB = o.trainDisturb || null;
  const AVG = o.avg ?? 4;
  ...
  // THE STATE TERM OF THE DISTILLED POLICY (`distilState`, plan §52.27), opt-in. The newest
  // measured sample — the following errors of both joints, their speeds and torques — multiplied
  // by an order-2 polynomial of the measured positions normalised to the channels' declared
  // box (with its order-0 term), plus the raw positions: 38 columns, no declaration, the same
  // thing on a tank. Measured on the converged prefix: R² 0.83 -> 0.97 leave-one-program-out,
  // 0.84/0.82 -> 0.995/0.953 on the square the fit never saw — and DEPLOYED, 1.44x on the bench
  // square and refused on the soft cell (own training programs harmed), 3.24x / 3.53x with the
  // policy's own state aggregated over rounds, against 6.04x / 3.98x without the term. The
  // information is the measured deviation, which is feedback, which this gearbox cannot pass
  // in time (§52.26.5, §52.27). Off by default; `stateDim` off is byte-identical.
  const distilState = o.distilState || null;
  const DSTATE_DIM = 2 + 6 * 6;
  const distilStateRow = (m, q) => {
    const n = [0, 1].map((c) => (m[c] - centre[c]) / 0.55);
    const sv = [1, n[0], n[1], n[0] * n[0], n[0] * n[1], n[1] * n[1]];
    const base = [m[0] - q[0], m[1] - q[1], m[2], m[3], m[4], m[5]];
    const f = [m[0], m[1]];
    for (const a of base) for (const t of sv) f.push(a * t);
    return f;
  };
  ...
  const auto = new AutoStack({
    ...
    ...(o.distil ? { distil: { refDim: refDimOf, ridge: o.distil.ridge ?? 1e-3,
      ...(distilState ? { stateDim: DSTATE_DIM, stateRow: distilStateRow, stateRounds: o.distilStateRounds ?? 2 } : {}),
      // THE WINDOW IS NOW STATED IN RAW MACHINE STEPS, AND THAT IS A UNITS REPAIR RATHER THAN A
      // TUNING CHANGE (plan §52.37). Specified in PILOT SAMPLES it silently moved with the plant:
      // the pilot's stride is derived from the measured settle, so raising the servo bandwidth
      // shrank the window's reach with nothing in the configuration touched — stride 8 / 4 / 3 at
      // bw 2e-3 / 8e-3 / 1.6e-2, i.e. +/-2048 / +/-1024 / +/-768 raw steps. Every bandwidth number
      // this project quoted before §52.33 therefore moved two variables at once, and restoring the
      // reach recovered the whole regression §52.30 had booked as the machine change's cost
      // (1.9284e-1 -> 1.7864e-1 at bw 8e-3). The ladder below is the SAME 23-tap geometry at the
      // SAME +/-2048-step reach it was measured at, so at the old stride of 8 it is byte-identical
      // and at every other stride it is the window the fit actually saw.
      ...(o.distil.offsets ? { offsets: o.distil.offsets }
        : o.distil.offsetsPerSample ? { offsetsPerSample: o.distil.offsetsPerSample }
        : { offsets: [-2048, -1024, -512, -256, -128, -64, -32, -16, -8, 0, 8, 16, 32, 64, 128, 192, 256, 384, 512, 768, 1024, 1536, 2048] }),
      ...
      ...(o.distil.standardize !== undefined ? { standardize: o.distil.standardize } : o.distilStandardize !== undefined ? { standardize: o.distilStandardize } : {}),
      ...((o.distil.schedule || o.distilSchedule) ? { schedule: o.distil.schedule || o.distilSchedule, scheduleCentre: centre } : {}),
      ...((o.distil.feedbackOnTop ?? o.distilFeedbackOnTop) !== undefined ? { feedbackOnTop: o.distil.feedbackOnTop ?? o.distilFeedbackOnTop } : {}),
      ...((o.distil.feedbackCap ?? o.distilFeedbackCap) !== undefined ? { feedbackCap: o.distil.feedbackCap ?? o.distilFeedbackCap } : {}),
      ...((o.distil.feedbackGain ?? o.distilFeedbackGain) !== undefined ? { feedbackGain: o.distil.feedbackGain ?? o.distilFeedbackGain } : {}),
      ...((o.distil.feedbackBasis ?? o.distilFeedbackBasis) ? { feedbackBasis: o.distil.feedbackBasis ?? o.distilFeedbackBasis } : {}),
      ...((o.distil.feedbackSched ?? o.distilFeedbackSched) ? { feedbackSched: o.distil.feedbackSched ?? o.distilFeedbackSched } : {}),
      ...(o.distil.adaptLag !== undefined ? { adaptLag: o.distil.adaptLag } : o.distilAdaptLag !== undefined ? { adaptLag: o.distilAdaptLag } : {}),
      ...(o.distil.adaptRate !== undefined ? { adaptRate: o.distil.adaptRate } : o.distilAdaptRate !== undefined ? { adaptRate: o.distilAdaptRate } : {}),
    ...
    pilot: {
      nMeasured: o.instruments ? 11 : 6, autoRefuse: false, gateForecasts: false,
      uMax: stackAuth,
  ...
  const ZFF = { dq: [0, 0] };
  // THE MEASURED VECTOR, stated once for the three loops that observe (plan §52.24). Six
  // motor-side signals — angles, speeds, torques — and, when the host is told the machine is
  // INSTRUMENTED, five more a customer could bolt on: both gearbox wind-ups (a load-side
  // encoder against the motor's), both links' tip deflections and link 1's tip slope (strain
  // gauges), scaled to the order of the angles. Off, the vector is byte-identical to before.
  const INSTR = !!o.instruments;
  const measVec = (arm, en, tau) => {
    const m = [en[0].angle, en[1].angle, en[0].speed * 1e3, en[1].speed * 1e3, tau[0] * 1e3, tau[1] * 1e3];
    if (INSTR) m.push(arm.j1.windup() * 1e2, arm.j2.windup() * 1e2, tipDeflection(arm.l1) * 1e2, tipDeflection(arm.l2) * 1e2, tipSlope(arm.l1) * 1e2);
    return m;
  };
  async function run(extra, name, laps, guided = false) {
    let satArmed = false;
    // ---- THE PLANT-SIDE DISTRESS READING, AND IT IS FREE (plan §82).
    //
    // The fraction of RECENT steps the drive could not deliver what it was asked for, as a
    // one-pole average. The drive already counts this — it is the one instrument in this project
    // a customer does not have to buy — and §81 measured why it is the right signal: undisturbed
    // the policy clips LESS than the bare machine (4.8% against 7.7%) because it tracks better,
    // and under load it clips MORE (14.0% against 8.1%), so the reading separates "working" from
    // "fighting the loop for the same drive" without being told a disturbance exists.
    //
    // The averaging length is the POLICY'S OWN WINDOW REACH, not a picked constant: a correction
    // is judged over the span of history it is computed from.
    let satPrev = 0, satEma = 0, satAlpha = 0, satRun = 0;
      ...
      for (let n = 0; n < total; n++) {
        ...
        lastK = k;
        {
          ...
          const ctx = { v: [cmd.vx, cmd.vy], a: [cmd.ax, cmd.ay], k, look, lookRaw,
            ...
            // CAUSAL BY CONSTRUCTION: this decision reads the average as it stood BEFORE the step
            // it is about to take, so the guard can never be fed its own consequence.
            load: satEma };
          ...
          // THE DRIVE'S OWN COUNTERS, RESET ON THE FIRST SCORED STEP (plan §81). "The correction
          // cannot see the disturbance" says nothing about whether the machine can DELIVER it:
          // the policy's demand is ADDITIVE on a loop already fighting the shove, so it makes
          // saturation MORE likely, not less. That is the number a safety claim rests on, and
          // this project has already measured this bench clipping 2.0-6.9% of samples with no
          // disturbance at all (§52.29, §52.30).
          if (!satArmed) {
            if (servo.resetLimitStats) servo.resetLimitStats();
            satPrev = 0; satEma = 0;
            const po = auto.distil && auto.distil.offsets;
            const reach = po && po.length ? Math.max(1, po[po.length - 1] - po[0]) : 1024;
            satAlpha = 1 / reach;
            satArmed = true;
          }
          // THE COMMANDED JOINT REFERENCE IS HANDED TO THE DISTURBANCE TOO (plan §84.7). §83.2
          // needed a disturbance that is a genuine FUNCTION OF THE REFERENCE and could only
          // express one keyed on lap PHASE, which on a diet of four different polygons is a
          // memory index and not a reference. A closure given the commanded angles can difference
          // them itself and build a viscous drag, an inertial payload, a cutting force — every
          // one of them a function of what the machine was ASKED to do, identical on every
          // program, and expressible by a map of the commanded reference. It is an extra argument
          // and every existing closure ignores it, so unset is byte-identical.
          const dz = DISTURB ? DISTURB(k, LAP, tau, [c1 + ff.dq[0] + u[0], c2 + ff.dq[1] + u[1]]) : null;
          arm.step(tau[0] + (dz ? dz[0] : 0), tau[1] + (dz ? dz[1] : 0), 1); mSamples++;
          {
            const sc = Array.isArray(servo.saturated)
              ? servo.saturated[0] + servo.saturated[1] : (servo.saturated || 0);
            const hit = sc - satPrev; satPrev = sc;
            satEma += ((hit > 0 ? 1 : 0) - satEma) * satAlpha;
            if (satEma > satSeen) satSeen = satEma;
            if (satEma > satRun) satRun = satEma;
          }
          ...
          const jt = guided
            ...
            : null;
          auto.observe(measVec(arm, en, tau), jt);
    ...
    // What the drive was ASKED for against what it has — per joint, over the scored steps.
    const lim = servo && servo.limitStats ? servo.limitStats() : null;
    const sat = !lim ? null : (Array.isArray(lim) ? lim : [lim])
      .map((L) => ({ fraction: L.fraction, peak: L.peakDemand / L.tauMax }));
    return { score: rep.totalRms, contour: rep.contourRms, sat, loadPeak: satRun,
  ...
  async function drivePilot(st) {
    ...
    const { arm, l1, l2, servo, rc } = held;
    armRef = arm;
    let guard = 0, step = 0, prevTau = [0, 0];
    try {
      while (st.phase !== 'done' && guard++ < 4e6) {
        ...
        // THE MACHINE IT IS IDENTIFIED ON IS THE MACHINE IT DEPLOYS ON (rule 34). Once the
        // distilled policy is deployed the scored machine runs BARE — the policy replaces
        // the compliance feedforward (`bareFor`) — and a layer identified on top of the
        // policy must be identified bare too. It was not: the feedback layer (plan §52.20)
        // was identified and verified UNDER the feedforward and scored deployed WITHOUT it,
        // and read 0.17x-0.86x at every authority, monotone toward 1.0x as its cap shrank —
        // the signature of a correction biased by a term the deployed machine does not
        // carry, read there as an unobservable state (plan §52.21 measures it half-observable).
        const ff = bareFor(null) ? ZFF : rc.feedforward([[1, 0], [0, 1]], tgc, { enableToolff: false });
        ...
        // THE DISTILLED POLICY'S WINDOW ON THE PILOT'S OWN COMMAND SERIES (plan §52.20), so a
        // layer identified on top of it sees the machine it will deploy on. The rows are the
        // same `refRow` shape the policy was fitted on — angles and reference torques — built
        // from the series at each offset; `k` is the identification step, `speed` the tool's.
        // `speed: null` — the coverage guard is OFF during identification: the pilot's
        // excitation runs outside the program's speed span by design (rule 41b), and a policy
        // faded out while the layer is identified and on when it deploys is two machines.
        // Measured with the fade on: the layer verified and delivered 0.15x (plan §52.20).
        const belowCtx = { v: [wvx, wvy], a: [wax, way], q: [cmd[0].pos, cmd[1].pos], k: step,
          speed: null };
        if (auto.deployed.distil && auto._distilBelowStack) {
          belowCtx.lookRaw = (off) => {
            const q = st.posAt(off);
            if (distilRef === 'angles') return q;
            const qm = st.posAt(off - 1), qp = st.posAt(off + 1);
            const vel = [0.5 * (qp[0] - qm[0]), 0.5 * (qp[1] - qm[1])], acc = [qp[0] - 2 * q[0] + qm[0], qp[1] - 2 * q[1] + qm[1]];
            const tj = servo.jointTorques([{ theta: q[0], omega: vel[0], alpha: acc[0] }, { theta: q[1], omega: vel[1], alpha: acc[1] }]);
            const sc = 1 / Math.max(1e-12, servo.tauMax * arm.j1.N);
            return distilRef === 'torques' ? [tj[0] * sc, tj[1] * sc] : [q[0], q[1], tj[0] * sc, tj[1] * sc];
          };
        }
        // THE LADDER'S NEWEST SAMPLE, which a distilled policy with a state term reads below a
        // layer under identification; `run()` sets it through `auto.observe`, this loop does not.
        auto._lastMeas = measVec(arm, arm.encoders(), prevTau);
        const below = auto.actBelow('stack', belowCtx);
   ...
   */
  /**
   * THE LIVE PROGRAM AS A TEACHABLE RUN (plan §52.18): the same descriptor the training
   * programs get — `run`, `teach`, `refAt`, `closed` — for the program the machine is
   * running, so the ladder's parametric law can be applied on it with the tracker attached.
   */
  async function liveRuns() { return distilRuns({ live: true }); }
  async function distilRuns(o3 = null) {
    ...
    if (o3 && o3.live) paths = [DEPLOY.path];
    if (o3 && o3.paths) paths = o3.paths;
    if (!paths || !paths.length) return null;
    ...
    //
    // A REFUSED CASCADE IS STILL A TEACHER, AND ITS VERIFY SCORE MEASURES THE ONE THING THE
    // TEACHING PORT BYPASSES (`distilTeachRefused`, opt-in, plan §52.33). As a rung the cascade
    // is judged on whether its FORECAST inverts the machine well enough to ship; as a teacher it
    // is handed the measured error through `oracleF0` and asked only for the increment that
    // cancels it, so the forecast is exactly what is replaced. The evidence that the score does
    // not reach the policy is already on record twice — §52.29's guided phase moved the teacher
    // 4.7557e-1 -> 2.7425e-1 and returned a BIT-IDENTICAL policy, and three commissioning seeds
    // at a raised loop gave cascades of 1.03x, 1.13x and 1.25x and one policy to five figures.
    // It matters because raising the servo bandwidth makes the cascade's verify marginal: at
    // bw 1.6e-2 it refuses on 3 of 5 seeds, and each refusal drops the rung onto `HarmonicFF`
    // for 1.3-1.7x at 107-117 machine-minutes where the pilot teacher reaches 6.6x in 10.
    // The cascade still never ships — `teacherOnly` decides that, not this — and the byte-identity
    // control is the seeds where it was admitted anyway.
    // DEFAULT ON (plan §52.37). Byte-identical wherever the cascade was admitted anyway — measured
    // exactly, on the seed whose cascade passed AND on the whole shipped-loop ladder — and where it
    // was not, it is the difference between 6.6x in 10.1 machine-minutes and 1.3x in 107.
    const teachRefused = o.distilTeachRefused !== false;
    const admitted = auto.stack && auto.stack.layers.some((p) => auto.stack._admitted(p));
    // The refused cascade is retained in `built.stacks`; `auto.stack` has been put back by then.
    const fallbackStack = (!admitted && teachRefused && auto.built && auto.built.stacks
      && auto.built.stacks.length) ? auto.built.stacks[auto.built.stacks.length - 1] : null;
    const teachStack = admitted ? auto.stack : fallbackStack;
    const engine = (o.distilEngine ?? 'pilot') === 'pilot' && !!teachStack
      && teachStack.layers.some((p) => teachStack._admitted(p) || teachRefused);
    // THE RUNG'S DECISION STRIDE COMES FROM THE TEACHING CASCADE, and with no cascade at all it
    // falls to ONE — eight times the rows and eight times the decisions, which is most of why a
    // refusal costs 107 machine-minutes against 10. `auto.stack` is what the ladder reads it
    // from, so the fallback is published there; `deployed.stack` stays 0, which is what arms a
    // cascade, so a teacher that is not a candidate still cannot reach the machine.
    if (fallbackStack && !auto.stack) auto.stack = fallbackStack;
    // AND IT IS PUBLISHED AS `built.stack` TOO, BECAUSE A TEACHER THAT EXISTS MUST BE FINDABLE
    // (plan §52.37). `built.stack` is set from the attempt that WON, so a refused cascade left it
    // null — and the page gates `Learn on this program` on it, the law worth 6.63x -> 8.23x. With
    // the refused cascade teaching, the rung deploys and the one feature that could improve it
    // further was unreachable exactly on the seeds that needed the fallback. `deployed.stack`
    // stays 0, which is what arms a cascade, so this publishes a teacher and never a controller.
    if (fallbackStack && !auto.built.stack) { auto.built.stack = fallbackStack; auto.built.depth = 1; }
    ...
    const TEACH_ITERS = o.distilTeachIters ? Math.max(1, Math.round(o.distilTeachIters)) : null;
    const qFilter = (x) => {
    ...
    const out = [];
    // THE COMMISSIONING'S INSTRUMENT (plan §52.42). Every number in this project is
    // obtained with GROUND-TRUTH TOOL POSITION available while commissioning — a laser
    // tracker, a ballbar or an instrumented artefact — and this file has never priced what
    // a CHEAPER truth delivers. `distilTruth` says what the teacher is allowed to measure,
    // and it governs the oracle record, the prefix's convergence AND its monotone gate, so
    // a degraded commissioning never peeks at the tracker for any of the three:
    //
    //   'tracker'  where the tool actually is. What ships, and what every number assumes.
    //   'encoder'  the motor encoders and a RIGID model — free on every machine that has a
    //              position loop, and blind to BOTH the gearbox wind-up and the link bend.
    //   'wu'       encoders plus wind-up readings, links still rigid — a permanently mounted
    //              instrument set, and the half §52.23 measured as observable from that side.
    //
    // It degrades the EXPENSIVE half only: the teacher needs truth at every sample of every
    // lap, where a rung's verify needs one rms per run — which a touch probe on the cut part
    // supplies. Rung scoring and the delivered number stay on the tracker, deliberately, so
    // that what is measured is the cost of the cheap teacher and not two wrongs agreeing
    // (rule 15).
    const TRUTH = o.distilTruth || 'tracker';
    if (!['tracker', 'encoder', 'wu'].includes(TRUTH)) throw new Error(`distilTruth ${TRUTH}: tracker, encoder or wu`);
    // ---- AND THE INSTRUMENT A SHOP ACTUALLY OWNS: A TOUCH PROBE ON THE CUT PART.
    //
    // The three modes above vary WHAT is measured and leave WHEN alone — all of them read the
    // machine at every sample of every lap, which is a tracker's property and not a probe's. A
    // touch probe gives the part's geometry at a HANDFUL of points, once, after the cut. This
    // file's own comment four lines up already names the gap ("the teacher needs truth at every
    // sample of every lap, where a rung's verify needs one rms per run — which a touch probe on
    // the cut part supplies") and nothing measured it.
    //
    // `distilProbePts: K` is that instrument: the teacher may read the truth at K evenly spaced
    // points of the lap and NOWHERE ELSE. Both halves are degraded together, because a probe
    // degrades both:
    //   - the RECORD the oracle inverts is those K points, linearly interpolated around the
    //     CLOSED lap — the teacher's best guess between touches, and the honest one, since an
    //     unmeasured sample is not a zero (rule 25);
    //   - the SCORE its monotone gate reads is the rms of those K points alone, which is what a
    //     probe report gives a shop.
    // The delivered number and every rung's verify stay on the tracker, exactly as §52.42 keeps
    // them, so what is measured is the cost of the cheap TEACHER and not two wrongs agreeing
    // (rule 15). `K = 0` is the default and is byte-identical.
    const PROBE_PTS = Math.max(0, Math.round(o.distilProbePts || 0));
    const toolAt = TRUTH === 'tracker' ? (a) => a.toolXY()
      : TRUTH === 'encoder' ? (a) => a.toolXY(true)
      : (a) => {
        // thL = thM/N - windup exactly, so this is the encoder corrected by the one
        // instrument, with the links left rigid — the gearbox seen and the bend not.
        const q1 = a.j1.encoder().angle - a.j1.windup();
        const q2 = a.j2.encoder().angle - a.j2.windup();
        return [Math.cos(q1) * a.L1 + Math.cos(q1 + q2) * a.L2,
          Math.sin(q1) * a.L1 + Math.sin(q1 + q2) * a.L2];
      };
    const describe = (dp) => {
      ...
      const drive = async ({ corr = null, pre = null, active = false, rec = null, uOut = null, trace = false, laps: lapsOpt = null, tap = null } = {}) => {
        ...
        const st = active ? (teachStack || auto.stack) : null;
        ...
        const S = (teachStack || auto.stack) ? (teachStack || auto.stack).sample : 1;
        ...
        // THE PROBED POINTS OF THIS LAP, evenly spaced and the same set every run — a shop
        // probes the same features on every part, and a set that moved would make two runs
        // incomparable (rule 20). Always built so the record can be interpolated back onto them.
        const probeAt = PROBE_PTS > 0 ? (() => {
          const ix = new Array(PROBE_PTS);
          for (let i = 0; i < PROBE_PTS; i++) ix[i] = Math.round(i * L / PROBE_PTS) % L;
          const on = new Uint8Array(L);
          for (const i of ix) on[i] = 1;
          return { ix, on };
        })() : null;
        ...
        if (st && TEACH_CAP != null) {
          capSave.push([st, 'uMax', st.uMax]); st.uMax = TEACH_CAP;
          for (const p of st.layers) { capSave.push([p, 'uMax', p.uMax]); p.uMax = TEACH_CAP; }
        }
        // THE TEACHER'S SOLVE MAY RUN TO CONVERGENCE (`distilTeachIters`, plan §52.17). The
        // deployed cascade truncates its QP at a few projected-gradient iterations because
        // the converged solve RINGS on the machine; but the teacher is handed the exact error
        // and asked for the increment that cancels it, and a truncated solve returns a
        // fraction of that inverse every pass — which is a plateau after four.
        if (st && TEACH_ITERS) for (const p of st.layers) { capSave.push([p, 'qpIters', p.qpIters]); p.qpIters = TEACH_ITERS; }
        if (st) {
          st._initRun();
        ...
        try {
          ...
          for (let nn = 0; nn < total; nn++) {
            ...
            lastK = k; kNow = k;
            {
              ...
              const td = TRAIN_DISTURB ? TRAIN_DISTURB(nn, dp.lap, tau, [q1 + ff.dq[0] + u[0], q2 + ff.dq[1] + u[1]]) : null;
              arm.step(tau[0] + (td ? td[0] : 0), tau[1] + (td ? td[1] : 0), 1); mSamples++;
              if (st) {
                const en = arm.encoders();
                st.observe(measVec(arm, en, tau), null);
              }
              // AN INSTRUMENT'S TAP (plan §52.21): the six measured signals in the pilot's own
              // units and the per-step joint-space truth, on THIS program — so an observer fitted
              // on one program can be scored on another rather than on later laps of the same one
              // (rule 36). Off by default and costs nothing when off.
              if (tap) {
                const en = arm.encoders(), tp = arm.toolXY();
                // ...and the INSTRUMENTS a customer could bolt on (plan §52.22): the tool's
                // position (an accelerometer, once differenced by the reader), the links' tip
                // deflections and link 1's tip slope (strain gauges), the gearboxes' wind-up.
                // None of these is a deploy-time signal of the shipped controller.
                tap(nn, k, [en[0].angle, en[1].angle, en[0].speed * 1e3, en[1].speed * 1e3, tau[0] * 1e3, tau[1] * 1e3],
                  worldToJoint([tp[0] - c.x, tp[1] - c.y], { q: [q1, q2] }),
                  { tool: tp, w: [tipDeflection(arm.l1), tipDeflection(arm.l2)], s1: tipSlope(arm.l1), wu: [arm.j1.windup(), arm.j2.windup()] });
              }
              if (l >= laps - A) {
                const tp = toolAt(arm);
                const j = worldToJoint([tp[0] - c.x, tp[1] - c.y], { q: [q1, q2] });
                e0[k] += j[0] / A; e1[k] += j[1] / A;
                // A PROBE KNOWS THE PART ONLY WHERE IT TOUCHED IT. The score the teacher's
                // monotone gate reads is the rms over the probed points alone; everywhere else
                // this instrument has no reading at all, and averaging in a value it never took
                // would be the tracker leaking into a measurement of its absence (rule 15).
                if (!probeAt || probeAt.on[k]) { s2 += j[0] * j[0] + j[1] * j[1]; n++; }
          ...
          for (const [obj, key, v] of capSave) obj[key] = v;
          await release(held);
        }
        // ---- THE RECORD IS WHAT THE PROBE TOUCHED, INTERPOLATED — NOT WHAT THE TRACKER SAW.
        //
        // Degrading only the SCORE would leave the teacher inverting a per-sample tracker record
        // while pretending to hold a probe report, which is the instrument leaking into the
        // measurement of its own absence. The lap is CLOSED, so the interpolation wraps: the
        // segment after the last probed point runs back to the first.
        if (recOut && probeAt) {
          const { ix } = probeAt, K = ix.length;
          const got = ix.map((i) => [recOut[i][0], recOut[i][1]]);
          for (let seg = 0; seg < K; seg++) {
            const a0 = ix[seg], a1 = ix[(seg + 1) % K];
            const span = ((a1 - a0) + L) % L || L;
            for (let d = 0; d < span; d++) {
              const t = d / span, k = (a0 + d) % L;
              recOut[k][0] = got[seg][0] * (1 - t) + got[(seg + 1) % K][0] * t;
              recOut[k][1] = got[seg][1] * (1 - t) + got[(seg + 1) % K][1] * t;
            }
          }
        }
      ...
      return ({
        ...
        /**
         * The measured state per in-lap step under a correction `corrAt(k, lastMeasured)` — the
         * converged prefix, or a policy reading the newest sample — one warm lap, one recorded.
         */
        captureState: async (corrAt) => {
          const rows = new Array(L); let last = null;
          await drive({ corr: { at: (k) => corrAt(k, last) }, laps: 2, tap: (nn, k, m) => { rows[k] = m; last = m; } });
          for (let k = 0; k < L; k++) if (!rows[k]) rows[k] = rows[k - 1] || rows[0];
          return (k) => rows[(((k % L) + L) % L)];
        },
        /**
         ...
         */
        run: async (corr, o2 = null) => (await drive({ corr, trace: !!(o2 && o2.trace), laps: o2 && o2.laps ? o2.laps : null, tap: (o2 && o2.tap) || null })),
        /**
    ...
    return describeAll(paths, out);
    function describeAll(ps, o4) { for (const dp of ps) o4.push(describe(dp)); return o4; }
  }
  ...
  /** Arm or clear the external disturbance between runs — commission clean, then push the plant. */
  const setDisturb = (fn) => { DISTURB = fn || null; };
  /** Push the plant while the TEACHER converges, so the targets themselves are taken disturbed. */
  const setTrainDisturb = (fn) => { TRAIN_DISTURB = fn || null; };
  /** The worst drive-distress reading the commissioning saw — the plant guard's own scale. */
  const loadSeen = () => satSeen;

  return { auto, run, drivePilot, recordDemo, distilRuns, liveRuns, refsFor, worldToJoint, attach,
    actAt, dispose, samples, setDisturb, setTrainDisturb, loadSeen, wv, wa, AVG };
}
