...
export function makeArmHost(o) {
  ...
  async function distilRuns() {
    ...
    for (const dp of paths) {
      ...
      const drive = async ({ corr = null, pre = null, active = false, rec = null, uOut = null, trace = false } = {}) => {
        ...
        const S = auto.stack ? auto.stack.sample : 1, LAPS = Math.ceil(L / S);
        const recOut = trace ? Array.from({ length: LAPS }, () => [0, 0]) : null;
        let s2 = 0, n = 0, step = 0, kSamp = 0, t2 = 0;
        ...
        if (st) {
          ...
          const or = ctl ? ((c, leadSamp, fitted) => fitted)
            : rec ? ((c, leadSamp, fitted) => {
              const v = rec[(((kSamp + leadSamp) % LAPS) + LAPS) % LAPS][c];
          ...
          for (let l = 0; l < laps; l++) {
            for (let k = 0; k < L; k++) {
              ...
              if (l >= laps - A) {
                ...
                if (recOut && k % S === 0) { recOut[kSamp][0] += j[0] / A; recOut[kSamp][1] += j[1] / A; }
              }
              if (yieldEvery && ++step % yieldEvery === 0) {
      ...
      out.push({
         ...
         */
        run: async (corr) => (await drive({ corr })),
        /**
        ...
        ...(engine ? { converge: async (passes = DISTIL_PASSES) => {
          ...
          for (let pass = 0; pass < passes; pass++) {
            ...
            for (let c = 0; c < 2; c++) for (let k = 0; k < L; k++) pre[c][k] += uOut[c][k];
            const r = await drive({ pre, trace: true });
            aligns.push(on.align);
            if (dbg) dbg(`pass ${pass}: pilot ON with prefix scored ${on.score.toExponential(4)}, uPk ${pk.toFixed(4)}, prefix alone ${r.score.toExponential(4)} (best ${best.toExponential(4)})`
            ...
            else { for (let c = 0; c < 2; c++) for (let k = 0; k < L; k++) pre[c][k] -= uOut[c][k]; break; }
          }
