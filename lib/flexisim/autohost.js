 ...
 * @param {boolean} [o.reuseMachine=false]  build and settle ONE machine and restore it per
 *   run instead of rebuilding. Bit-identical and 1.5x faster; opt-in because every published
 *   number was measured on the rebuilding path.
...
export function makeArmHost(o) {
  ...
  //
  // OPT-IN. Every number this project has published was measured while rebuilding, so the
  // default keeps that path exactly (rule 21: the cases a change should not touch must come
  // back byte-identical).
  let heldM = null, heldSnap = null;
  async function acquire() {
    if (!o.reuseMachine) return makeMachine();
