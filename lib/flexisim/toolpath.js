...
export class ToolPath {
  ...
  _profile() {
    ...
    for (let i = 1; i <= N; i++) {
      ...
      t[i] = t[i - 1] + dw[i - 1] + (vm > 1e-12 ? this.ds / vm : 0);
    }
    this.t = t; this.dw = dw;
  ...
  _locate(time) {
    ...
    if (time <= leave) return { i, f: 0, rest: true };
    if (time >= this.t[i]) return { i, f: 1, rest: this.dw[i] > 0 };
    return { i, f: (time - leave) / (this.t[i] - leave), rest: false };
  }
  ...
  timeAt(s) {
    ...
    const f = (u - (i - 1) * this.ds) / this.ds, leave = this.t[i - 1] + this.dw[i - 1];
    return leave + f * (this.t[i] - leave);
  }
  ...
  at(k) {
    ...
    const { i, f, rest } = this._locate(time);
    const span = rest ? 0 : 1;
    const u = Math.min(this.length, (i - 1 + f) * this.ds);
    const speed = rest ? 0 : this.v[i - 1] + f * (this.v[i] - this.v[i - 1]);
