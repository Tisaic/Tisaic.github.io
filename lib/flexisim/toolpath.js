...
export class ToolPath {
   ...
   * @param {boolean} [o.cornerStop] EXACT STOP: come to rest at every corner instead of the
   *   deviation rule's junction speed. The deviation rule still turns the velocity VECTOR in
   *   one step at the junction speed — on the bench arm that asked the shoulder for ~2,240x
   *   its torque at every corner of the sharp square. Stopping is what a robot or a CNC in
   *   exact-stop mode does, and it is the only way through a corner with a finite torque.
   * @param {number} [o.dwell] steps held AT REST at each stop (and at the seam of a closed
   *   path that stops there). A setpoint smoother downstream (`jointProgram`'s `smooth`)
   *   needs a dwell at least as long as its window, or it averages the two edges of a corner
   *   together and cuts it.
   */
  constructor({ segments, start = [0, 0], feed, accel, closed = false, cornerDt = 40, cornerStop = false,
    dwell = 0 }) {
    this.feed = feed; this.accel = accel; this.cornerDt = cornerDt;
    this.cornerStop = cornerStop; this.dwell = Math.max(0, dwell);
    this.segs = [];
    let s = 0, p = start;
  ...
  _profile() {
    ...
    //
    // AND EVERY SEGMENT BOUNDARY IS A SAMPLE. The samples used to be spaced evenly over the
    // whole path, so a corner fell between two of them wherever the spacing did not divide the
    // edge; the stop was imposed on the NEAREST sample, and the tool halted short of the corner
    // (2e-4 on the bench square at 3e-3), crept on, and turned while moving: a step of
    // acceleration at every corner, which the jerk filter cannot remove. Each segment now has its
    // own whole number of samples at the same resolution, and `sS[i]` is sample i's arc length.
    const h = Math.max(this.feed / 4, 1e-9), counts = this.segs.map((g) => Math.max(1, Math.ceil(g.len / h)));
    const N = Math.max(2, counts.reduce((a, b) => a + b, 0));
    const sS = new Float64Array(N + 1), segStart = new Int32Array(this.segs.length + 1);
    { let i = 0; this.segs.forEach((g, j) => { segStart[j] = i; for (let m = 0; m < counts[j]; m++) sS[i++] = g.s0 + (m / counts[j]) * g.len; }); segStart[this.segs.length] = i; sS[N] = this.length; }
    this.sS = sS;
    this.ds = this.length / N;                     // the mean spacing, for reporting only
    const dsAt = (i) => sS[i] - sS[i - 1];         // the interval ending at sample i
    ...
    for (let j = 1; j < this.segs.length; j++) {
      ...
      const vj = this.cornerStop ? 0 : this.accel * this.cornerDt / (2 * Math.sin(phi / 2));
      const i = segStart[j];
    ...
    const kAt = (i) => Math.abs(this.segAt(sS[i]).curvature);
    ...
    const reach = (P, k, ds) => {
      const c = 1 + 4 * ds * ds * k * k;
      const disc = P * P - c * (P * P - 4 * ds * ds * this.accel * this.accel);
    ...
    for (let r = 0; r < rounds; r++) {
      for (let i = 1; i <= N; i++) {
        ...
        v[i] = Math.min(v[i], Math.sqrt(reach(v[i - 1] * v[i - 1], k, dsAt(i))));
      }
      if (this.closed) v[0] = Math.min(v[0], v[N]);
      for (let i = N - 1; i >= 0; i--) {
        const k = Math.max(kAt(i), kAt(i + 1));
        v[i] = Math.min(v[i], Math.sqrt(reach(v[i + 1] * v[i + 1], k, dsAt(i + 1))));
      }
    ...
    // A STOP HOLDS FOR THE DWELL, AT REST: `t[i]` is when the tool ARRIVES at sample i and
    // `dw[i]` how long it then stays there, so the motion from i to i+1 starts at
    // t[i] + dw[i]. (Folding the dwell into the arrival time instead spreads the last
    // interval's travel across the hold — measured, the tool crept through the corner at
    // a fraction of the speed it was meant to have stopped at, and the velocity vector
    // still turned at speed.) Sample 0 of a closed path is sample N: it is held once, at N.
    const t = new Float64Array(N + 1), dw = new Float64Array(N + 1);
    if (this.dwell > 0) for (let i = this.closed ? 1 : 0; i <= N; i++) if (v[i] === 0) dw[i] = this.dwell;
    for (let i = 1; i <= N; i++) {
      const vm = 0.5 * (v[i - 1] + v[i]);
      t[i] = t[i - 1] + dw[i - 1] + (vm > 1e-12 ? dsAt(i) / vm : 0);
    }
    this.t = t; this.dw = dw;
    ...
    this.lap = t[N] + dw[N];
    this.period = Math.max(1, Math.ceil(this.lap));
  }

  /**
   * Where the tool is in the time table at `time` (0 <= time <= lap): the interval (i-1, i)
   * and the fraction through it, or — during a dwell at sample j — i = j + 1 with f = 0
   * and `rest` set.
   */
  _locate(time) {
    let lo = 0, hi = this.N;
    while (lo < hi) { const mid = (lo + hi) >> 1; if (this.t[mid] < time) lo = mid + 1; else hi = mid; }
    const i = Math.max(1, lo), leave = this.t[i - 1] + this.dw[i - 1];
    if (time <= leave) return { i, f: 0, rest: true, speed: 0 };
    if (time >= this.t[i]) return { i, f: 1, rest: this.dw[i] > 0, speed: this.dw[i] > 0 ? 0 : this.v[i] };
    // UNIFORM ACCELERATION IN TIME ACROSS THE INTERVAL, which is what the table's own time
    // (ds over the mean speed) assumes. Interpolating the ARC LENGTH linearly in time instead
    // held the speed constant through each interval and stepped it at the next sample: leaving
    // a stop, where one interval lasts many scans, the speed rose in a staircase with steps of
    // 2.5% of the feed against an acceleration limit of 0.3% a scan, and the jerk filter turned
    // each step into a step of acceleration (measured: 7.7% of its peak in one scan, which
    // the conventional rung's acceleration term passed to the setpoint as a 6.9e-3 rad kick).
    const T = this.t[i] - leave, tau = time - leave, v0 = this.v[i - 1], v1 = this.v[i];
    const a = (v1 - v0) / T, sIn = v0 * tau + 0.5 * a * tau * tau;
    return { i, f: Math.min(1, Math.max(0, sIn / (this.sS[i] - this.sS[i - 1]))), rest: false, speed: v0 + a * tau };
  }
  ...
  timeAt(s) {
    ...
    let lo = 1, hi = this.N;                       // the first sample at or past u
    while (lo < hi) { const mid = (lo + hi) >> 1; if (this.sS[mid] < u) lo = mid + 1; else hi = mid; }
    const i = lo, d = u - this.sS[i - 1], leave = this.t[i - 1] + this.dw[i - 1];
    // the inverse of `_locate`'s uniform acceleration: v0 tau + a tau^2 / 2 = d
    const T = this.t[i] - leave, v0 = this.v[i - 1], a = (this.v[i] - v0) / T;
    if (!(d > 0)) return leave;
    // the root written without dividing by a, which cancels catastrophically when a is ~0
    const den = v0 + Math.sqrt(Math.max(0, v0 * v0 + 2 * a * d)), tau = den > 0 ? 2 * d / den : T;
    return leave + Math.min(T, Math.max(0, tau));
  }
  ...
  at(k) {
    ...
    const { i, f, rest, speed } = this._locate(time);
    const span = rest ? 0 : 1;
    const dsI = this.sS[i] - this.sS[i - 1];
    const u = Math.min(this.length, this.sS[i - 1] + f * dsI);
    ...
    const aT = span > 0
      ? (this.v[i] * this.v[i] - this.v[i - 1] * this.v[i - 1]) / (2 * dsI) : 0;
...
export function roundedRect({ w, h, r, centre = [0, 0], feed, accel, cornerDt = 40,
  ...
  return new ToolPath({ segments: segs, start, feed, accel, closed, cornerDt, ...timing });
}
 ...
 */
export function circle({ r, centre = [0, 0], feed, accel, ccw = true, cornerDt = 40, ...timing }) {
  const [cx, cy] = centre;
  const start = [cx + r, cy];
  const far = [cx - r, cy];
  return new ToolPath({ start, feed, accel, closed: true, cornerDt, ...timing,
 ...
 */
export function sharpRect({ w, h, centre = [0, 0], feed, accel, cornerDt = 40, ...timing }) {
  const [cx, cy] = centre;
  const x0 = cx - w / 2, x1 = cx + w / 2, y0 = cy - h / 2, y1 = cy + h / 2;
  return new ToolPath({ start: [x0, y0], feed, accel, closed: true, cornerDt, ...timing,
