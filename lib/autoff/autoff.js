 ...
 *     project's portfolio ships on most plants — tried once more WITHOUT its sign terms (which step the
 *     setpoint at every reversal) and shipped so where that is indistinguishable on the gain — commissioned IN PRODUCTION on the repeating program,
 ...
 *   the TWIN rung, on an arm given a twin (`cfg.aTwinP`, `twin2r.js`): every program the machine runs
 *     gets a table learned on the twin (`twinlearn.js`) from one lap of its reference, in the
 *     background on the scan's leftover budget, applied while that program runs in place of ① and ②
 *     (③ keeps priority on its own program), and withdrawn for good on a program where a lap under it
 *     is worse than the lap recorded without it. It needs no commissioning; the twin's
 *     IDENTIFICATION is not in the block;
...
import {
  ...
  affFeatures, affZeroIndex, affDecide, affSpeed, affCoverage, affProgMatch, affTableMatch, affTwinBlend,
} from './runtime.js';
import { TwinLearner, TL_UNIT, TL_LEADS, TL_WIDTHS } from './twinlearn.js';
...
const HEALTH_LAPS = 3, HEALTH_FACTOR = 1.5;
const SIGN_SHARE = 0.05;
...
const EXCITE_GUARD = 2;                             // ①: the step-free candidate may give up this share of the gain          // RUN: laps this much worse request a recommission
...
export const E_AFF_REASON = Object.freeze({
  ...
  LAP_TOO_SHORT: 16, BUSY: 17, PROG_NO_GAIN: 18, PROG_OFF: 19, TWIN_WORSE: 20,
});
/** The twin rung's state (`out.eTwinState`). */
export const E_AFF_TWIN = Object.freeze({ OFF: 0, WATCHING: 1, LEARNING: 2, APPLIED: 3, HELD: 4, REFUSED: 5 });
const TWIN_NAME = Object.fromEntries(Object.entries(E_AFF_TWIN).map(([k, v]) => [v, k]));
export const affTwinName = (s) => TWIN_NAME[s] || '?';
const TW_ITERS = 15;                                   // learning steps on the twin per program
const TW_CAP = 3;                                      // the twin table's authority, x the twin's untouched rms
...
export class FB_AutoFF {
   ...
   * @param {Float64Array} [cfg.aTwinP]  a TWIN of the machine under its own control (`twin2r.js`'s
   *   parameter vector; two channels, the arm's joints). With it, every program the machine runs gets
   *   a table learned on the twin from ONE lap of its reference, applied from the lap after it is
   *   learned while that program runs, and withdrawn for good on that program if a lap under it is
   *   worse than the lap recorded without it (the health guard).
   * @param {boolean} [cfg.xTwinNoHealth=false]  TEST ONLY: disable the twin table's health guard
  ...
  constructor(cfg) {
    ...
    if (cfg.aTwinP && nc !== 2) throw new Error('FB_AutoFF: a twin (aTwinP) is for a two-channel arm');
    this._twP = cfg.aTwinP || null;
    this.xTwinNoHealth = !!cfg.xTwinNoHealth;
    ...
    this.out = {
      ...
      eTwinState: E_AFF_TWIN.OFF, eTwinReason: E_AFF_REASON.NONE, rTwinGain: 0, rTwinFactor: 0,
      rTwinProgress: 0, rTwinPredicted: 0, udiTwinTables: 0, nTwinLap: 0, xConvSign: false, rConvSignFree: 0,
    ...
    // ---- the twin rung: three reference buffers rotate (recording, learning, applied) by index
    if (this._twP) {
      this._twB = [0, 1, 2].map(() => new Float64Array(2 * AFF_MAX_LAP));
      this._twCum = new Float64Array(2 * AFF_MAX_LAP);          // the recorded lap's running sum of e^2
      this._twU = new Float64Array(2 * AFF_MAX_LAP);            // the applied table
      this._twLearner = new TwinLearner(AFF_MAX_LAP);
      this._twCap = new Float64Array(AFF_MAX_CH);
      this._twBeforeL = new Float64Array(2); this._twBefore = new Float64Array(2); this._twRmsOut = new Float64Array(2);
    }
    this._twIR = 0; this._twIL = 1; this._twIA = 2;
    this._twRecOk = false; this._twRecMax = 0; this._twLearning = false; this._twLearnLap = 0; this._twTolL = 0;
    this._twReady = false; this._twLap = 0; this._twTol = 0; this._twReach = 0; this._twRefused = false;
    this._twOff = true; this._twGain = 0; this._twCleanA = false; this._twCleanL = false; this._twLaps = 0;
  ...
  cycle() {
    ...
    if (edgeCycle) {
      if (this._twP) this._twinEdge(this._phase + 1);
    ...
    if (this._state === S.IDLE && (this._recArmed() || this._twP)) {
    ...
    if (this._twP) { this._twinGuard(); this._mac += affTwinBlend(this._aTarget, this._twU, 2, this._phase, this._twGain, this._twCap, nc); }
  ...
  _progGuard() {
    const L = this.live;
    if (!L.xProgArmed) { this._progGain = 0; return; }
    if (this._ownsRef || !affProgMatch(L, this._phase, this.in.aRefAhead, this.nAhead)) { this._progClean = false; this._progOff = true; }
    this._progGain = this._progOff ? 0 : 1;
    this._mac += 2 * this.nc;
  }
  ...
  _applyTrim() {
    ...
    for (let c = 0; c < nc; c++) {
      ...
      const auth = Math.max(this.live.aUMax[c] || this._uMax[c] || 0, this._twReady ? this._twCap[c] : 0) || 1;
      const cap = auth / this._rampN;
  ...
  _beginRefine() {
    ...
    this._Wacc.fill(0); this._bestW.fill(0); this._signTried = false; this._signTrial = false; this.out.xConvSign = false; this.out.rConvSignFree = 0;
   ...
   * setpoint trim it STEPS at every reversal: on the arm, 0.031 rad on a joint at every corner, and
   * with the gearboxes' backlash it left the machine in another periodic state after commissioning
   * (reported 9.79x, then 4.22x in production) and tripped the guard in the excitation it sits under.
   * Where the terms earn their gain they stay: the EMPS axis (real friction), and the soft arm
   * (1.90x with them, 1.57x without). Returns false when there is nothing to try.
   */
  _signFree() {
    const nc = this.nc, nb = this._nb, L = this.live;
    if (this._signTried || this.xKeepSign || !(this._best < 1 - this._margin)) return false;
    this._signTried = true;
    let any = false;
    for (let c = 0; c < nc; c++) for (let d = 0; d < nc; d++) if (this._bestW[c * AFF_MAX_NB + 3 * d + 2] !== 0) any = true;
    this.out.xConvSign = any;
    if (!any) return false;
  ...
  // =============================================================================== output
  // ============================================================== the twin rung (an arm's twin)
  /**
   * A LAP EDGE, for the twin rung. `n` is the length of the lap that just closed. In order:
   *   1. a table learned on the twin becomes the applied one, if the lap that closed was a clean lap
   *      of its program (the machine is then in that program's periodic state);
   *   2. the applied table: a lap run under it (after its first, which carries the switch) is judged
   *      against the lap recorded WITHOUT it; worse, and it is withdrawn for good on that program;
   *      it may act next lap only if the lap that closed was a clean lap of its program;
   *   3. a lap of a program with no table and none being learned starts the learning on it.
   */
  _twinEdge(n) {
    const S = E_AFF_STATE, O = this.out;
    const complete = this._state === S.RUN && !this._ownsRef && this._twRecOk && n >= 32 && n <= AFF_MAX_LAP;
    const ofLearn = complete && this._twCleanL && n === this._twLearnLap;
    if (this._twLearning && this._twLearner.xDone && ofLearn) {
      const t = this._twIA; this._twIA = this._twIL; this._twIL = t;
      const u = this._twU; this._twU = this._twLearner.aU; this._twLearner.aU = u;
      this._twLap = this._twLearnLap; this._twTol = this._twTolL;
      this._twReach = Math.round(TL_LEADS[3] * this._twLap) + 2 * Math.round(TL_WIDTHS[0] * this._twLap);
      for (let c = 0; c < 2; c++) {
        this._twBefore[c] = this._twBeforeL[c];
        this._twCap[c] = Math.max(this.live.aUMax[c] || 0, TW_CAP * this._twLearner._rms0[c]);
      }
      O.rTwinPredicted = this._twLearner.rFactor; O.nTwinLap = this._twLap; O.udiTwinTables++;
      this._twLearning = false; this._twReady = true; this._twRefused = false; this._twLaps = 0;
      this._twCleanA = true; this._twOff = true;
    }
    const ofApplied = complete && this._twReady && this._twCleanA && n === this._twLap;
    if (this._twReady) {
      if (ofApplied && !this._twOff && !this._twRefused) {
        if (++this._twLaps >= 2) {                       // a whole lap under the table, after its first
          const r = this._twRatio(n);
          O.rTwinFactor = r > 0 ? 1 / r : 0;
          if (r > 1 && !this.xTwinNoHealth) { this._twRefused = true; O.eTwinReason = E_AFF_REASON.TWIN_WORSE; }
        }
      } else if (!ofApplied) this._twLaps = 0;
      this._twOff = !ofApplied || this._twRefused;
    }
    if (complete && !ofApplied && !(this._twLearning && ofLearn)) {
      const t = this._twIL; this._twIL = this._twIR; this._twIR = t;
      this._twLearnLap = n; this._twTolL = 1e-6 * (1 + this._twRecMax);
      const r = this._twRms(n); this._twBeforeL[0] = r[0]; this._twBeforeL[1] = r[1];
      this._twLearner.start(this._twP, this._twB[this._twIL], 2, n, TW_ITERS);
      this._twLearning = true; this._twCleanL = true;
    }
    // the next lap: recorded from its first scan; clean until a scan says otherwise
    this._twRecOk = this._state === S.RUN && !this._ownsRef;
    this._twRecMax = 0; this._twCleanA = this._twReady; this._twCleanL = this._twLearning;
  }

  /** The recorded lap's rms per channel after the 5% drop, from the running sums (O(1)). */
  _twRms(n) {
    const d = Math.ceil(0.05 * n), C = this._twCum, out = this._twRmsOut;
    for (let c = 0; c < 2; c++) out[c] = Math.sqrt(Math.max(0, C[2 * (n - 1) + c] - (d > 0 ? C[2 * (d - 1) + c] : 0)) / (n - d));
    return out;
  }

  /** The lap under the table against the lap recorded without it: rms of the per-channel ratios. */
  _twRatio(n) {
    const r = this._twRms(n);
    let s = 0;
    for (let c = 0; c < 2; c++) { const q = this._twBefore[c] > 0 ? r[c] / this._twBefore[c] : 1; s += q * q; }
    return Math.sqrt(s / 2);
  }

  /** The twin table's guard: on while its program runs, from an edge that closed a clean lap of it. */
  _twinGuard() {
    if (!this._twReady || this._twRefused) { this._twGain = 0; return; }
    const m = affTableMatch(this._twB[this._twIA], this._twLap, 2, this._twTol, this._twReach, this._phase,
      this.in.aRefAhead, this.nAhead, 2);
    this._mac += 16;
    if (this._ownsRef || !m) { this._twCleanA = false; this._twOff = true; }
    this._twGain = (this._twOff || this._progGain > 0 || this._state !== E_AFF_STATE.RUN || !this.in.xArm) ? 0 : 1;
  }

  /** Every RUN scan: record the lap (reference and the error's running sums), then learn with what is left. */
  _twinScan() {
    const k = this._phase;
    if (!this.in.xArm) this._twRecOk = false;     // a disarmed lap is neither a program's record nor a judged lap
    if (this._twRecOk && k >= 0 && k < AFF_MAX_LAP) {
      const B = this._twB[this._twIR], C = this._twCum;
      for (let c = 0; c < 2; c++) {
        const r = this._aR0[c], e = this.in.aMeas[c] - r;
        B[2 * k + c] = r;
        C[2 * k + c] = (k > 0 ? C[2 * (k - 1) + c] : 0) + e * e;
        const a = r < 0 ? -r : r; if (a > this._twRecMax) this._twRecMax = a;
      }
      if (this._twLearning && this._twCleanL) {
        const L = this._twB[this._twIL];
        if (k >= this._twLearnLap || !(Math.abs(this._aR0[0] - L[2 * k]) <= this._twTolL && Math.abs(this._aR0[1] - L[2 * k + 1]) <= this._twTolL)) this._twCleanL = false;
      }
      this._mac += 16;
    } else this._twRecOk = false;
    if (this._twLearning && !this._twLearner.xDone) {
      const rem = this.udiMacBudget - this._mac;
      if (rem >= TL_UNIT) this._mac += this._twLearner.run(rem);
    }
  }
  ...
  _publish() {
    ...
    if (this._twP) {
      const T = E_AFF_TWIN;
      O.rTwinGain = this._twGain;
      O.eTwinState = this._twGain > 0 ? T.APPLIED : this._twLearning ? T.LEARNING
        : this._twRefused ? T.REFUSED : this._twReady ? T.HELD : T.WATCHING;
      O.rTwinProgress = this._twLearning ? this._twLearner.progress() : this._twReady ? 1 : 0;
    }
