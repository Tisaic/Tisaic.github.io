...
/**
 * DOES THE REFERENCE MATCH A STORED TABLE'S PROGRAM? `affProgMatch` for any table: `aTabRef` holds
 * the program, row-major with stride `nStride`, for `nLap` scans; true when the phase lies inside it
 * and every channel matches within `rTol`, now and `nReach` scans ahead (capped by the host's preview
 * `nAhead`, wrapping past the lap's end). Used for the table the arm's twin learned (`twinlearn.js`).
 */
export function affTableMatch(aTabRef, nLap, nStride, rTol, nReach, iPhase, aRefAhead, nAhead, nc) {
  if (!(nLap > 0) || iPhase < 0 || iPhase >= nLap) return false;
  const ahead = nReach < nAhead ? nReach : nAhead;
  for (let pass = 0; pass < 2; pass++) {
    const i = pass === 0 ? 0 : ahead, o = ((iPhase + i) % nLap) * nStride;
    for (let c = 0; c < nc; c++) {
      if (!(Math.abs(aRefAhead[i * AFF_MAX_CH + c] - aTabRef[o + c]) <= rTol)) return false;
    }
  }
  return true;
}
...
/**
 * THE TWIN'S TABLE, BLENDED INTO THE DECISION. With gain g in [0, 1] (the caller's guard, 0 while the
 * commissioned program table is engaged): trim = (1 - g) . trim + g . U[iPhase], clamped to
 * +-aCap per channel. The twin's table was learned on the machine's conventional control alone, so it
 * REPLACES the block's first two rungs where it engages rather than adding to them. A non-finite
 * result is 0. Returns the MAC count.
 */
export function affTwinBlend(aOut, aTwU, nStride, iPhase, rGain, aCap, nc) {
  if (!(rGain > 0)) return 0;
  const o = iPhase * nStride;
  for (let c = 0; c < nc; c++) {
    const u = (1 - rGain) * aOut[c] + rGain * aTwU[o + c], cap = aCap[c];
    aOut[c] = Number.isFinite(u) ? (u > cap ? cap : u < -cap ? -cap : u) : 0;
  }
  return 4 * nc;
}
