...
fn sampleAt(p : vec3<i32>) -> f32 {
  ...
  let i = cellIndex(c);
  // Both wall kinds are opaque geometry: SOLID (1) and MOVING (4).
  if (flags[i] == 1u || flags[i] == 4u) { return -1.0; }
...
fn fs(in : VSOut) -> @location(0) vec4<f32> {
  ...
  // Slab test against the box the lattice is mapped into. The box carries the
  // lattice's OWN aspect ratio (normalised to its longest axis) -- mapping every
  // lattice into a cube renders a 2:1:1 channel as a cube.
  let ext = vec3<f32>(f32(NX), f32(NY), f32(NZ)) / f32(max(max(NX, NY), NZ));
  let lo = -0.5 * ext;
  let hi = 0.5 * ext;
  let t0 = (lo - o) / d;
  let t1 = (hi - o) / d;
  ...
  var alpha = 0.0;
  var entered = false;          // has this ray reached fluid yet?
  for (var s = 0; s < steps; s = s + 1) {
    let wp = o + d * t;
    let q = ((wp - lo) / ext) * (dims - 1.0);
    let v = sampleLerp(q);
    if (v < 0.0) {
      // Solid. A closed domain is wrapped in no-slip walls, so EVERY ray hits
      // solid before it reaches any fluid -- rendering that opaquely produces a
      // featureless grey box, which is exactly what the first version did. So
      // solid is transparent until the ray has entered the fluid, and opaque
      // after: the enclosing shell disappears, interior obstacles stay solid.
      if (entered) {
        col = col + (1.0 - alpha) * vec3<f32>(0.42, 0.45, 0.52);
        alpha = 1.0;
        break;
      }
      t = t + dt;
...
export class VolumeRenderer {
  ...
  /**
   * Draw one frame. Returns false instead of throwing if the swap chain cannot
   * be acquired.
   *
   * This is not defensive programming for its own sake. In a headless browser
   * with a software adapter and no real surface, getCurrentTexture() does not
   * merely fail -- it tears down the whole WebGPU instance, after which every
   * COMPUTE call fails too and the simulation dies with the picture. Isolated to
   * fifteen lines of plain WebGPU with none of this engine involved: configure()
   * succeeds, the first getCurrentTexture() succeeds, and the next buffer
   * mapAsync reports "a valid external Instance reference no longer exists".
   * So the renderer reports failure and the caller falls back to the slice view,
   * rather than a visualisation being able to kill a simulation.
   */
  render(invViewProj, eye) {
    if (this.backend.lost) return false;
    let view;
    try { view = this.context.getCurrentTexture().createView(); }
    catch (e) { this.failed = String(e && e.message ? e.message : e); return false; }
  ...
  /** The shader source, so a test can compile it without acquiring a surface. */
  static shaderSource(lattice) { return SHADER(lattice); }
