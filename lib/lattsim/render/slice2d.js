...
export class SliceRenderer {
  ...
  constructor(canvas, { minInterval = 60 } = {}) {
    ...
    this.lastStats = null;
    this._img = null;
    // Field-reconstruction overlay: when showRecon is on and reconField is set
    // (a full-cellCount array the reconstruction controller fills over the display
    // plane), concentration mode draws THAT instead of the true scalar -- so the
    // same canvas shows "what N sensors reconstruct" beside the truth toggle.
    this.reconField = null;
    this.showRecon = false;
    // Cell indices to mark on the picture (the sensor array). Painted into the
    // image at lattice resolution, so they land on EVERY renderer showing this
    // plane without any screen-space bookkeeping -- the DOM markers used for the
    // probe need the canvas geometry, and there is now more than one canvas.
    this.markers = [];
    this.markerColor = [255, 60, 220];
    // A FIXED colour range for the scalar, instead of auto-scaling from this
    // renderer's own data. Comparing two independently auto-scaled pictures is
    // meaningless -- the eye reads contrast, not value -- so the comparison view
    // normalises the reconstruction against the TRUTH's range, and the
    // reconstruction's errors then show as real over/under-saturation.
  ...
  async draw(sim, now = (typeof performance !== 'undefined' ? performance.now() : Date.now())) {
    ...
    try {
      ...
      // Draw the reconstruction instead of the truth when it is the one asked for.
      const cfield = (wantConc && this.showRecon && this.reconField) ? this.reconField : conc;
      ...
      for (let b = 0; b < H; b++) {
        for (let a = 0; a < W; a++) {
          ...
          if (wantConc) { const cc = cfield[i]; if (cc < cLo) cLo = cc; if (cc > cHi) cHi = cc; }
        }
      }
      ...
      // Unless a shared range was imposed -- see forceScale.
      if (wantConc && this.forceScale) { cLo = this.forceScale.lo; cHi = this.forceScale.hi; }
      ...
      for (let b = 0; b < H; b++) {
        for (let a = 0; a < W; a++) {
          ...
          if (type !== 0 && (this.mode === SLICE_MODE.MATERIAL || type === 1)) {
            ...
            [r, g, bl] = viridis((cfield[i] - cLo) / cSpan);
          } else {
      ...
      // Sensor marks, drawn last so nothing paints over them. A single cell is one
      // pixel here and would be invisible once the canvas is scaled down, so each
      // mark is a small plus -- which also reads as "instrument", not "data".
      let shown = 0;
      if (this.markers && this.markers.length) {
        const [mr, mg, mb] = this.markerColor;
        const nx = lattice.nx, nxy = lattice.nx * lattice.ny;
        for (const cell of this.markers) {
          const z = Math.floor(cell / nxy), y = Math.floor((cell % nxy) / nx), x = cell % nx;
          const along = this.axis === 0 ? x : this.axis === 1 ? y : z;
          if (along !== k) continue;                 // not on the plane being drawn
          const a = this.axis === 0 ? y : x;
          const b = this.axis === 0 ? z : this.axis === 1 ? z : y;
          shown++;
          for (const [da, db] of [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1]]) {
            const aa = a + da, bb = b + db;
            if (aa < 0 || aa >= W || bb < 0 || bb >= H) continue;
            const o = 4 * ((H - 1 - bb) * W + aa);
            px[o] = mr; px[o + 1] = mg; px[o + 2] = mb; px[o + 3] = 255;
          }
        }
      }
      this.ctx.putImageData(this._img, 0, 0);
      this.lastStats = { peak, rhoLo, rhoHi, uScale, k, axis: this.axis, width: W, height: H,
        concLo: wantConc ? cLo : null, concHi: wantConc ? cHi : null, markersShown: shown };
      this.lastDraw = now;
      return true;
    } finally {
      this.busy = false;
