...
export class CPUBackend {
  ...
  /**
   * EVERY FIELD, COPIED — and the means to put it back exactly.
   *
   * WHY A SIMULATION NEEDS THIS. A study that scores many candidates on one machine has to
   * start each run from the same state or the runs are not comparable, and the only way to
   * do that without this is to REBUILD and re-settle. On the FlexiSim arm that costs 4.2 s
   * of every 13.2 s run — a third of the ladder's entire 32 minutes spent re-deriving a
   * state it already had.
   *
   * AND IT IS MORE EXACT THAN REBUILDING, NOT LESS. Re-settling only converges TOWARDS the
   * same state: measured on that arm, the 4000-step settle each run pays leaves the joint
   * wind-up 0.25% from its 40000-step value, so every "identical" machine actually differed
   * by a fraction of a percent that no amount of care in the caller could remove. A restore
   * is bit-identical by construction.
   *
   * NAMED `captureState`, NOT `snapshot`. This backend already has an
   * `async snapshot(name)` — the host-visible copy of ONE field, the CPU half of the
   * CPU/WebGPU parity API — and it is defined LATER in the class. A duplicate method name is
   * legal JavaScript and the later definition silently wins, so the first version of this
   * was dead on arrival: it parsed, `node --check` passed, the module parser passed, and the
   * call went to a function taking a field name and returning one array.
   *
   * CONTENTS ARE RESTORED, NOT IDENTITIES. `swap()` exchanges `a` and `b`, so the pair may
   * have flipped between the snapshot and the restore; copying the saved contents into
   * whichever buffer is currently `a` is what preserves the meaning (`a` is always the live
   * state) rather than the pointer.
   */
  captureState() {
    const s = new Map();
    for (const [name, e] of this.buffers) {
      s.set(name, { a: e.a.slice(), b: e.b ? e.b.slice() : null });
    }
    return s;
  }

  /** @param {Map} s a `captureState` from THIS backend — the field set must match exactly. */
  restoreState(s) {
    if (s.size !== this.buffers.size) {
      throw new Error(`restoreState: snapshot has ${s.size} fields, backend has ${this.buffers.size}`);
    }
    for (const [name, saved] of s) {
      const e = this.buffers.get(name);
      // REFUSE RATHER THAN PARTIALLY RESTORE. A snapshot from a different lattice would
      // otherwise leave some fields restored and some not, which is a state the simulation
      // was never in and cannot be reasoned about (rule 51).
      if (!e) throw new Error(`restoreState: no field "${name}" on this backend`);
      if (e.a.length !== saved.a.length) {
        throw new Error(`restoreState: field "${name}" is ${e.a.length}, snapshot is ${saved.a.length}`);
      }
      e.a.set(saved.a);
      if (saved.b && e.b) e.b.set(saved.b);
    }
  }
