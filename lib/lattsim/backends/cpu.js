...
class CPUElastic {
  ...
  step() {
    ...
    // Neighbour index one step along an axis, honouring topology. Out of range
    // on a bounded axis returns -1, and the caller substitutes the value that
    // makes the difference vanish -- a zero-gradient edge. That is NOT a free
    // surface (which sets traction to zero, a different condition on a different
    // component) and it is not a clamp; it is a placeholder that keeps this brick
    // well defined while the boundary system is still to be written, and the
    // periodic wave-speed tests never reach it.
    ...
    for (let z = 0; z < nz; z++) {
      for (let y = 0; y < ny; y++) {
        for (let x = 0; x < nx; x++) {
          ...
          const dxy = (ym < 0 ? 0 : s[SXY + i] - s[SXY + ym]);
          const dxz = (zm < 0 ? 0 : s[SXZ + i] - s[SXZ + zm]);
          v[VX + i] = keep * v[VX + i] + invRho * (dxx + dxy + dxz);

          const yp = stepIdx(x, y, z, 1, +1);
          const xm = stepIdx(x, y, z, 0, -1);
          const dyx = (xm < 0 ? 0 : s[SXY + i] - s[SXY + xm]);
          const dyy = (yp < 0 ? 0 : s[SYY + yp] - s[SYY + i]);
          const dyz = (zm < 0 ? 0 : s[SYZ + i] - s[SYZ + zm]);
          v[VY + i] = keep * v[VY + i] + invRho * (dyx + dyy + dyz);

          const zp = stepIdx(x, y, z, 2, +1);
          const dzx = (xm < 0 ? 0 : s[SXZ + i] - s[SXZ + xm]);
          const dzy = (ym < 0 ? 0 : s[SYZ + i] - s[SYZ + ym]);
          const dzz = (zp < 0 ? 0 : s[SZZ + zp] - s[SZZ + i]);
          v[VZ + i] = keep * v[VZ + i] + invRho * (dzx + dzy + dzz);
        }
      }
    }
    ...
    for (let z = 0; z < nz; z++) {
      for (let y = 0; y < ny; y++) {
        for (let x = 0; x < nx; x++) {
          ...
          const ym = stepIdx(x, y, z, 1, -1);
          const zm = stepIdx(x, y, z, 2, -1);
          const exx = (xm < 0 ? 0 : v[VX + i] - v[VX + xm]);
          const eyy = (ym < 0 ? 0 : v[VY + i] - v[VY + ym]);
          const ezz = (zm < 0 ? 0 : v[VZ + i] - v[VZ + zm]);
