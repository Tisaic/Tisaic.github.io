...
class CPULBM {
  ...
  initialize() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const ux = type === CELL.INLET || type === CELL.MOVING ? u0[0] : 0;
      const uy = type === CELL.INLET || type === CELL.MOVING ? u0[1] : 0;
      const uz = type === CELL.INLET || type === CELL.MOVING ? u0[2] : 0;
      ...
      for (let y = 0; y < ny; y++) {
        for (let x = 0; x < nx; x++) {
          ...
          for (let q = 0; q < Q; q++) {
            ...
            fi[q] = flags[si] === CELL.SOLID ? src[OPP[q] * N + i] : src[q * N + si];
          }
          ...
          // ---- driven cells override the velocity they just computed
          // MOVING is a wall: its own gathered density is the right one to use.
          // INLET and OUTLET are open boundaries and are finished off in a
          // second pass, where the neighbouring cell's macroscopic state is
          // available; what is written here is provisional.
          if (type === CELL.INLET || type === CELL.MOVING) {
 ...
 */
export function reduceMacro(lattice, flags, macro) {
  const N = lattice.cellCount;
