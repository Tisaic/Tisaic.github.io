...
import { collideStreamWGSL, openBoundaryWGSL, initWGSL, reduceWGSL, REDUCE_STRIDE,
  collideStreamScalarWGSL, scalarOutletWGSL, initScalarWGSL } from './wgsl.js';
...
export class WebGPUBackend {
  ...
  constructor({ lattice, fields, flags, adapter, device }) {
    ...
    this.kernels = new KernelRegistry()
      .register('lbm.d3q19', (ctx, op) => new GPULBM(ctx, op))
      .register('scalar.d3q19', (ctx, op) => new GPUScalar(ctx, op));
    this.buildReduce();
  }
  ...
  async assertShadersCompile() {
    const sources = [
      ...
      ['scalarInit', initScalarWGSL(this.lattice, { workgroup: [WORKGROUP, 1, 1] })],
      ['scalarStep', collideStreamScalarWGSL(this.lattice, { workgroup: [WORKGROUP, 1, 1] })],
      ['scalarOutlet', scalarOutletWGSL(this.lattice, { workgroup: [WORKGROUP, 1, 1] })],
    ];
    const problems = [];
  ...
  /**
   * A DESTROYED BACKEND MUST SAY SO, not read `undefined.a`.
   *
   * `destroy()` clears the buffer map, and a render or a readback already in
   * flight can arrive after that -- two slider changes in quick succession queue
   * two rebuilds, and the frame between them is exactly this case. The old code
   * threw `Cannot read properties of undefined (reading 'a')`, which is a real
   * page error with a red badge and no indication that a benign race caused it.
   * A NAMED error lets a caller tell "the run I was drawing has been replaced"
   * from "the field I asked for does not exist", and only the first is ignorable.
   */
  _entry(name) {
    const e = this.buffers.get(name);
    if (!e) {
      const why = this.buffers.size === 0
        ? 'this backend has been destroyed' : `no field named '${name}'`;
      const err = new Error(`lattsim: cannot access '${name}' — ${why}`);
      err.stale = this.buffers.size === 0;
      throw err;
    }
    return e;
  }

  read(name) { return this._entry(name).a; }
  write(name) { const e = this._entry(name); return e.b || e.a; }
  swap(name) { const e = this._entry(name); if (e.b) { const t = e.a; e.a = e.b; e.b = t; } }
   ...
   */
  /** Queued, for the same reason snapshot() is -- see _queue. */
  async probe(name, cell) { return this._queue('_probing', () => this._probe(name, cell)); }

  async _probe(name, cell) {
    if (this.lost) throw new Error('GPU device lost');
    const e = this._entry(name);            // named `stale` error if destroyed, not a raw TypeError
  ...
  /**
   * MANY cells at once, one submit and one map.
   *
   * The field-reconstruction demo samples a whole sensor array plus a target
   * region every frame. Calling probe() per cell serialises them through the
   * single probe buffer (the _probing guard) -- N awaits, N submits -- so a batch
   * of a hundred cells would stall the frame. This copies every requested cell's
   * components into ONE staging buffer in a single command encoder, then maps once.
   * Returns one component-array per cell, in the order asked.
   */
  async probeMany(name, cells) { return this._queue('_probing', () => this._probeMany(name, cells)); }

  async _probeMany(name, cells) {
    if (this.lost) throw new Error('GPU device lost');
    const e = this._entry(name);
    const comps = e.spec.components;
    const N = this.lattice.cellCount;
    const bytes = Math.max(64, cells.length * comps * 4);
    if (!this.probeManyBuffer || this.probeManyBuffer.size < bytes) {
      if (this.probeManyBuffer) this.probeManyBuffer.destroy();
      this.probeManyBuffer = this.device.createBuffer({
        label: 'probeMany', size: bytes,
        usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
      });
    }
    const enc = this.device.createCommandEncoder({ label: 'probeMany' });
    for (let j = 0; j < cells.length; j++) {
      const base = j * comps;
      for (let c = 0; c < comps; c++) {
        enc.copyBufferToBuffer(e.a, (c * N + cells[j]) * 4, this.probeManyBuffer, (base + c) * 4, 4);
      }
    }
    this.device.queue.submit([enc.finish()]);
    const readBytes = cells.length * comps * 4;
    await this.probeManyBuffer.mapAsync(GPUMapMode.READ, 0, readBytes);
    const flat = new Float32Array(this.probeManyBuffer.getMappedRange(0, readBytes));
    const out = new Array(cells.length);
    for (let j = 0; j < cells.length; j++) {
      const v = new Array(comps);
      for (let c = 0; c < comps; c++) v[c] = flat[j * comps + c];
      out[j] = v;
    }
    this.probeManyBuffer.unmap();
    return out;
  }
   ...
   */
  /**
   * CHECK-THEN-SET IS NOT A QUEUE, and with three callers it stops serialising.
   *
   * This was `if (this._snapping) await this._snapping; this._snapping = ...`.
   * With two callers that works. With THREE, B and C both await A, A settles, and
   * both wake and run `_snapshot` -- two mapAsync on one staging buffer, which
   * WebGPU rejects with "Buffer already has an outstanding map pending". The
   * waiters never re-checked after waking.
   *
   * It was latent until the comparison view added a second renderer: the frame
   * then reads the field from the stats reduction, the main slice, the comparison
   * slice and the reconstruction sampler, and three of those can overlap.
   *
   * Chaining each call onto the previous one is a real queue and is correct for
   * any number of callers. `.catch` on the stored link keeps one failed readback
   * from poisoning every later one, while the caller still sees its own rejection.
   */
  async snapshot(name) { return this._queue('_snapping', () => this._snapshot(name)); }

  /**
   * Serialise readbacks that share a staging buffer.
   *
   * The slot MUST fall back to null once the queue drains: destroy() treats a
   * truthy slot as "a readback is in flight" and defers, so a chain that stayed
   * truthy forever would make destroy() re-enter itself without end. Only the
   * LAST link clears it, so a queue that is still filling is never mistaken for
   * an idle one.
   */
  _queue(slot, run) {
    const mine = (this[slot] || Promise.resolve()).catch(() => {}).then(run);
    const link = mine.catch(() => {});
    this[slot] = link;
    link.then(() => { if (this[slot] === link) this[slot] = null; });
    return mine;
  }

  async _snapshot(name) {
    // A snapshot in flight when a rebuild destroys the backend must raise the
    // catchable `stale` error, not a raw "reading 'spec'" TypeError. read()/write()
    // route through _entry for this reason (v140); snapshot()/probe() were missed,
    // and a slice draw racing a teardown surfaced it as a red badge on the page.
    const e = this._entry(name);
...
class GPULBM {
  ...
  uploadParams() {
    ...
    // Equation of state. eosMode 0/1/2 = ideal/linear/quadratic. Offsets 124/128/
    // 132, computed from the struct: eosMode lands in initVel's trailing 4 bytes.
    const P = this.op.params;
    const eosMode = P.eos === 'linear' ? 1 : P.eos === 'quadratic' ? 2 : 0;
    const csEff2 = P.soundSpeed != null ? P.soundSpeed * P.soundSpeed : (1 / 3);
    d[31] = eosMode; d[32] = csEff2; d[33] = P.eosParam || 0;
    d[4] = force[0]; d[5] = force[1]; d[6] = force[2];
    // The inlet on THIS step, which is not the same thing as the INITIAL velocity
    // two lines up -- naming them both `iv` made this module a parse error, so
    // WebGPUBackend could not be imported at all and the page fell back to the CPU
    // reference, which then refused the default lattice as too large. The visible
    // symptom was "build failed - CPU reference backend is capped", three layers
    // away from the cause.
    const inletNow = this.op.inletVelocityNow
      ? this.op.inletVelocityNow() : inletVelocity;
    d[8] = inletNow[0]; d[9] = inletNow[1]; d[10] = inletNow[2];
  ...
  step() {
    ...
    // A MODULATED INLET IS THE SAME CASE. Without this the uniform would keep the
    // FIRST step's value for the whole run: the disturbance would be computed on
    // the CPU, reported in the diagnostics, and never actually reach the shader --
    // a control that looks connected and is not, which is the hardest kind of
    // defect to notice because every number on the page moves.
    const drive = !!(this.op.params.inletAmplitude
      && this.op.params.inletMode && this.op.params.inletMode !== 'steady');
    const live = !!(im && (im.radius > 0 || this._hadImpulse));
    // A parameter changed through setParams (viscosity, inlet speed, the equation
    // of state) must reach the shader, or the page would show the new number while
    // the kernel kept solving the old one.
    if (live || osc || drive || this.op.paramsDirty) {
      this.uploadParams();
      this.op.paramsDirty = false;
      this._hadImpulse = !!(im && im.radius > 0);
    }
...
/**
 * Passive-scalar transport on the GPU. Mirrors GPULBM, with two differences that
 * matter: the velocity field `mac` is bound READ-ONLY (the scalar reads the flow
 * it is carried by and never writes it, so there is no conflict with the fluid
 * kernel that wrote it earlier this step), and there is no reduction -- the
 * concentration is a single field the renderer and the soft sensor read directly.
 */
class GPUScalar {
  constructor(ctx, op) {
    this.ctx = ctx;
    ...
    const { lattice, backend } = ctx;
    const dev = backend.device;
    // 48 bytes: see SParams in wgsl.js. sourceValue is an f32 after a vec3, so it
    // lands at offset 28 (the vec3's trailing bytes), initialValue at 32, rounded
    // up to 48. Float32Array(12) covers it with three trailing pad slots.
    this.params = dev.createBuffer({
      label: 'scalar.params', size: 48,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    this.paramData = new Float32Array(12);
    this.uploadParams();
    ...
    this.initPipeline = mk(initScalarWGSL(lattice, { workgroup: [WORKGROUP, 1, 1] }), 'scalar.init');
    this.stepPipeline = mk(collideStreamScalarWGSL(lattice, { workgroup: [WORKGROUP, 1, 1] }), 'scalar.collideStream');
    this.outletPipeline = mk(scalarOutletWGSL(lattice, { workgroup: [WORKGROUP, 1, 1] }), 'scalar.outlet');
    this.groups = Math.ceil(lattice.cellCount / WORKGROUP);
    this.hasOutlet = ctx.flags.includes(CELL.OUTLET);
  }
  ...
  uploadParams() {
    ...
    d[1] = p.inletValue || 0;
    d[2] = p.clamp ? 1 : 0;
    const s = p.source || { centre: [0, 0, 0], radius: 0, value: 0 };
    d[3] = s.radius || 0;
    d[4] = s.centre[0]; d[5] = s.centre[1]; d[6] = s.centre[2];   // sourceCentre (offset 16)
    d[7] = s.value || 0;                                          // sourceValue  (offset 28)
    d[8] = p.initialValue || 0;                                   // initialValue (offset 32)
    this.ctx.backend.device.queue.writeBuffer(this.params, 0, d);
  }
  ...
  bindStep() {
    ...
    return b.device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 1, resource: { buffer: b.write(this.op.distribution) } },
        { binding: 2, resource: { buffer: b.flagBuffer } },
        { binding: 3, resource: { buffer: b.read(this.op.velocity) } },        // velocity IN
        { binding: 4, resource: { buffer: b.write(this.op.concentration) } },
  ...
  async verifyFirstDispatch() {
    ...
    this.step();
    const err = await dev.popErrorScope();
    if (err) throw new Error('GPU scalar dispatch rejected: ' + err.message);
    return true;
  }
  ...
  initialize() {
    const b = this.ctx.backend;
    const e = b.buffers.get(this.op.distribution);
    const enc = b.device.createCommandEncoder({ label: 'scalar.init' });
    const pass = enc.beginComputePass();
    pass.setPipeline(this.initPipeline);
    pass.setBindGroup(0, b.device.createBindGroup({
      layout: this.initPipeline.getBindGroupLayout(0),
      entries: [
        ...
        { binding: 1, resource: { buffer: e.b } },
        { binding: 2, resource: { buffer: b.flagBuffer } },
        { binding: 3, resource: { buffer: b.read(this.op.velocity) } },
        { binding: 4, resource: { buffer: b.write(this.op.concentration) } },
        { binding: 5, resource: { buffer: this.params } },
      ],
    }));
  ...
  step() {
    const b = this.ctx.backend;
    // A small per-step uniform write, so a moved injector or a changed tau_g is
    // picked up without a rebuild. It is 48 bytes against a Q-population dispatch.
    this.uploadParams();
    const enc = b.device.createCommandEncoder({ label: 'scalar.step' });
    const pass = enc.beginComputePass();
    pass.setPipeline(this.stepPipeline);
    pass.setBindGroup(0, this.bindStep());
    pass.dispatchWorkgroups(this.groups);
    if (this.hasOutlet) {
      // Zero-gradient outlet, a second dispatch in the same pass -- it reads the
      // populations the first dispatch just wrote (WebGPU orders dispatches within
      // a pass), exactly as the fluid's open-boundary pass does.
      pass.setPipeline(this.outletPipeline);
      pass.setBindGroup(0, b.device.createBindGroup({
        layout: this.outletPipeline.getBindGroupLayout(0),
        entries: [
          { binding: 0, resource: { buffer: b.write(this.op.distribution) } },
          { binding: 1, resource: { buffer: b.flagBuffer } },
          { binding: 2, resource: { buffer: b.write(this.op.concentration) } },
        ],
      }));
      pass.dispatchWorkgroups(this.groups);
    }
