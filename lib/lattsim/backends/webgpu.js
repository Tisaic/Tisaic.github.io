...
export class WebGPUBackend {
  ...
  async _reduce(macroField) {
    ...
    let rhoMin = Infinity, rhoMax = -Infinity, uMaxSq = 0, sumDU = 0, sumU2 = 0;
  ...
  destroy() {
    ...
    const inFlight = [this._reducing, this._snapping].filter(Boolean);
    if (inFlight.length) {
      this._destroying = true;
      Promise.allSettled(inFlight).then(() => { this._destroying = false; this.destroy(); });
      return;
    }
...
class GPULBM {
  constructor(ctx, op) {
    ...
    this.params = dev.createBuffer({
      label: 'lbm.params', size: 80,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    this.paramData = new Float32Array(20);
    this.uploadParams();
  ...
  step() {
    ...
    if (live) { this.uploadParams(); this._hadImpulse = im.radius > 0; }
