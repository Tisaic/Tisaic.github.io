...
// 80 bytes. Every vec3<f32> member is 16-byte aligned in WGSL, so the layout is
// omega/forcePre (0,4) - force (16) - inletVel (32) - impulse (48) -
// impulseCentre (64) with impulseRadius packed into its trailing slot. Sizing
// this buffer wrong is not a compile error: it is an invalid bind group, a
// dropped command buffer, and a simulation of zeros.
struct Params {
  omega         : f32,        // 1 / tau
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  for (var q : u32 = 0u; q < Q; q = q + 1u) {
    ...
    if (outside) {
      ...
      if (nt == CELL_SOLID) {
        ...
        let cu = dot(vec3<f32>(CV[q]), params.inletVel);
  ...
  for (var q : u32 = 0u; q < Q; q = q + 1u) {
    ...
    if (driven) {
      ...
      v = v + params.forcePre * WT[q] * dot(g, bf);
    }
    dst[q * NCELL + i] = v;
  }
 ...
 * fluidCells, nonFinite, sumDeltaUSq, sumUSq].
 *
