...
import { ScalarTransportOperator } from './operators/scalar.js';
...
export function channelFlow({ resolution = 48, tau = 0.55, inletVelocity = 0.06, obstacle = 'sphere',
  collision, trtPolicy, smagorinsky, eos, soundSpeed, eosParam,
  inletMode = 'steady', inletAmplitude = 0, inletRate = 0.004,
  // Inject a passive scalar (a dye needle upstream of the obstacle) and transport
  // it with the flow. Off by default -- it adds a second D3Q19 distribution, so it
  // is only paid for where the plume is the point. scalarTau sets its diffusivity.
  dye = false, scalarTau = 0.6 } = {}) {
  const n = resolution;
  ...
  sim.addPhysics(new LBMFluidOperator({ tau, inletVelocity: [inletVelocity, 0, 0],
    ...
    collision, trtPolicy, smagorinsky, eos, soundSpeed, eosParam }));
  // A dye needle upstream of the obstacle, so the plume is drawn into the wake --
  // which is exactly the field a downstream soft sensor is asked to reconstruct.
  const dyeCentre = [Math.round(n * 0.45), (n >> 1) + 1, span >> 1];
  const dyeRadius = Math.max(2, Math.round(n / 12));
  if (dye) {
    sim.addPhysics(new ScalarTransportOperator({ tau: scalarTau,
      source: { centre: dyeCentre, radius: dyeRadius, value: 1 } }));
  }
  sim.meta = {
    name: dye ? 'Dye plume past an obstacle' : 'Channel flow past an obstacle',
    hasScalar: dye,
    dyeCentre, dyeRadius,
    obstacleRadius: r,
...
export function lidCavity({ resolution = 40, tau = 0.6, lidVelocity = 0.05, lidFrequency = 0,
  ...
  sim.addPhysics(new LBMFluidOperator({ tau, inletVelocity: [lidVelocity, 0, 0],
    wallFrequency: lidFrequency, collision, trtPolicy, smagorinsky, eos, soundSpeed, eosParam }));
  sim.meta = {
...
export const SCENES = {
  ...
  // A cylinder wake carrying a dye plume -- the scalar field a downstream soft
  // sensor reconstructs. Defaults to a cylinder so the shed wake stirs the dye.
  dye: { label: 'Dye plume', make: (o = {}) => channelFlow({ obstacle: 'cylinder', ...o, dye: true }) },
};
