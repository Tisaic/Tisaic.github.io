# NGRC playground — the measurement history

**What is settled** is in `CLAUDE.md`'s current-state section; this file is the record of
how each number was arrived at, what was rejected and on what evidence. Where this file
and `CLAUDE.md` disagree, `CLAUDE.md` is what ships.

Four tabs: chaotic systems, the motor/load soft sensor, the finger trace, and the
anti-slosh axis. The library is `lib/ngrc/`; the page is `ngrc.html`.
