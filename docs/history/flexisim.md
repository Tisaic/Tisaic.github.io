...
## Brick 73 — the seven-tab page is retired; its record, verbatim

The FlexiSim page was rebuilt from scratch as a single commissioning bench (plan §52). Every
tab below — ① Move, ② Chain, ③ Path with its eleven correction modes, ④ Black box, ⑤ Verify,
⑥ Deploy, ⑦ Architecture — is gone from the page. The LIBRARIES they drove are untouched and
their Node tests still run, so every number in this record is still reproducible off-page.
What follows is the page's description from CLAUDE.md at the moment it was retired, moved
here unchanged because it is the measurement record and not a specification.

### (retired) FlexiSim — `flexisim.html` on `lib/flexisim/` + `lib/lattsim/` + `lib/blackbox/`
