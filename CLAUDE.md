...
| `test/pilot/select.mjs` | **COMMISSION k TIMES AND KEEP THE BEST — and the only honest way to test it.** A spread of 2.2x to 4.2x across commissioning seeds is a loss if the draws cannot be told apart and a FACTOR if they can, and the representative gate now ranks. But selecting on a number and validating against that same number measures nothing (rules 15, 16), so the test is HELD OUT: the arm's gate scores the rounded rectangle and the CIRCLE is never part of the selection. **What would kill it:** if the selected draw is no better on the held-out program than the median draw, the ranking does not survive being used for selection — the classic way a validation signal dies — and the spread must be shrunk instead, which neither excitation length (3x, measured, buys nothing) nor the solver knobs do. |
