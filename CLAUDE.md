...
8. **MEASURED AGAINST SOMETHING THAT IS NOT US. ONE LITERATURE METHOD IS NOW BUILT AND RUN,
   ...
   **145,082 MAC/decision, 1451% of a PLC scan against the policy's 78 MAC at 0.8% — 1,860x** — and
   ...
   feedforward — and that is a product number this project did not have. **TARGET 8 THEREFORE
   STILL HAS ONE ADMISSIBLE RIVAL, NOT TWO**, and the two that would actually contest the
   deployed object are both unbuilt: ZPETC / stable inversion, which this file already names as
   the thing we are a data-regressed version of, and a NONLINEAR learner on the identical rows —
   which is the sharper one, because §52.36's "no basis will move it" ceiling was established
   by six experiments that were all LINEAR-IN-PARAMETERS ridge with different features, and a
   different function class is not more features. Koopman-EDMD, L1 and modern MPC remain absent,
   and L1 and MPC are inadmissible on the same grounds as DeePC.
