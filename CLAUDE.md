...
one plant. **AND THE FIELD IS NOW THREE ADMISSIBLE RIVALS RATHER THAN ONE (plan §56, §113)** —
norm-optimal ILC, ZPETC/stable inversion and now KOOPMAN-EDMD, with DeePC built and DISQUALIFIED
for needing the instrument at runtime. Still absent: modern MPC and L1 adaptive, both inadmissible
on the same ground as DeePC, and behavioural methods generally. "Best in the world" is a claim
about a field, and three rivals on one axis are not a field. The defensible sentence today is: *beats the conventional machine; beats an engineered
...
count is a thing to commission, and INS is one plant's evidence again. **AND THAT ACCOUNT IS ITSELF
REFUTED BY THE ROUTE §121 COULD NOT RUN, ON FOUR PLANTS (plan §125).** §121 wrapped the descriptor's
`run` and `teach` on a SPREAD COPY, so the ORACLE teacher — built by the harness, closed over its own
drive loop — kept reading the full instrument and three rows were a vacuous control (rule 9c). The
seam is repaired, pinned by a test checked to FAIL on the pre-repair state, and the column's ORACLE
teacher, which does not identify from the record at all, knees where `hff` does — **0.48x at 64
against 0.47x** — so it is not `hff`'s identification bandwidth. `2·nh` holds on ONE plant of four:
the arm needs 64 of 512, the BARREL 128-256 of 512, and the CART-POLE, the only plant asked whose
`nh` is not capped, **≤32 of a predicted 260-366**. Three more proxies were tried and refuted,
including this file's own — a longer diet segment with the window held makes the column's knee
WORSE. **THE CART-POLE IS THE FIRST PLANT WHERE THE SHOP'S INSTRUMENT IS FREE** (11.93x flat from
K=32 to 512), and on the BARREL a 256-touch probe BEATS the tracker at 1.19x — which is rule 14 and
was checked rather than celebrated: with the lap-incommensurate drift held flat it reverses to 0.73x,
so it is §80.7's `TAVG` mechanism through a new knob and a statement about that plant's disturbance.
**THE LADDER AXIS THAT WOULD COMMISSION THE COUNT IS REFUSED ON MEASUREMENT**: a candidate costs a
FULL commissioning where a ridge candidate costs a refit — **335 barrel-days to choose among seven
counts against a 35.3-day commissioning, the REFUSED candidate the most expensive of the eight** —
and scoring one uses the full instrument by construction (rule 15), so the axis presupposes the
tracker it exists to let the customer not buy. **AND THE SHARPEST RISK IS NEW: THE PROBE LIES TO THE
TEACHER IN THE FLATTERING DIRECTION.** On the column's oracle route the teacher reports **22.7x while
delivering 1.71x** at K=32 and 4.7x against 5.49x on the tracker — a 13x lie shrinking monotonically
to nothing as the instrument improves, read by the monotone gate that stops the iteration. So a
customer commissioning on a probe alone watches a teacher converge on a machine that is not getting
better, and nothing here detects it. **And the FEED test — the other half — was BUILT and cannot be asked on this plant**, which is a
...
1. **PROGRAM-AGNOSTIC. ASKED ON ALL TEN PLANTS NOW, MET ON FIVE, AND THE COUNT IS A SCRAPE (plan §88, §88.9).**
   ...
   ```
     ...
     quad tank         7.9         0.369          NOT MET  (still helps at 7.337x)
   ```

   **AND THE TANK IS ASKED NOW, BY THE COLUMN'S OWN INSTRUMENT RATHER THAN BY RE-QUOTING THE
   NUMBER §88.9 STRUCK.** That section removed it for a good reason — §79.3's 2.657x comes from
   the GAIN LADDER'S OWN candidate scoring, which drives the distilled policy ALONE, and putting
   that beside rows produced by the driver's scored loop is rule 19. The repair is not a re-quote:
   the harness's own `run0` is parameterised over the program exactly as `rigs/ladder.mjs`'s is,
   so the SAME held-out recipe is scored through the loop that scores production, and the object
   asked is **whatever the block ships — since §97.3 the CONVENTIONAL rung** and not a rung the
   machine refused. **19.910x scored against 7.337x held out — 0.369, NOT MET, nothing made
   worse**, and the held-out program is verified out of the diet FROM THE DATA (0 of 4 of its
   transitions and 0 of 5 of its level pairs occur in the four training recipes) rather than from
   a comment. Two controls: the shipped row comes back at 5.0636e-1 → 2.5432e-2 = 19.910x, every
   digit unchanged (rule 21), and the held-out BASELINE reads 4.2608e-1 by `scoreOn` and
   4.2608e-1 by the gain ladder's own `driveAlt`, which shares no code with it (rule 15).
   **AND §84.9's SCREEN PREDICTED IT ON A TENTH POINT**: `prog/rise` 7.9 puts this plant below the
   ~10 split with the column's 7.6 and the barrel's 5.2, and all three read NOT MET while every
   plant above the split reads MET.
   ...
   **FIVE OF TEN ASKED MEET THE BOUND, ONE HAS A PROGRAM MADE WORSE, NOTHING IS LEFT
   UNASKED — AND THE AGGREGATE IS NOW SCRAPED RATHER THAN INFERRED (plan §109.2, §115).** This
   used to say *TEN OF TEN is an INFERENCE and the aggregate has NOT been re-scraped*, verified
   per row and pending as a total. The full pass has now run with `OBJTABLE_OUT` set and been
   read back: **`TARGET 1 asked on 11 of 11: 5 MET, 1 made WORSE on the held-out program`** —
   eleven ROWS over ten plants, the cart-pole contributing two, and the prose confirmed. **The
   read's first answer was 4 MET and the instrument was the fault**: the reader keyed rows on the
   SCRIPT, and `distil-pend.mjs` is registered twice, so the TUNED loop's 0.4245 overwrote the
   SHIPPED loop's **0.9495** — the row this table quotes. Resolved IN FAVOUR of the prose, which
   is the opposite of §88.9, and a `COLLAPSED` check now fires when a script registered N times
   returns fewer than N rows, asserted both ways (rule 9). The flagship plant is the cleanest row: the 2R arm reads **10.88x on the rounded rectangle and 15.94x on
...
4. **COMMISSIONING IN MINUTES, NOT AN AFTERNOON. MET ON THE ARM — 17 MINUTES TO 2, AND THE
   ...
   **THE CONTROLS ARE WHY THE NUMBERS ARE WORTH ANYTHING**, and three exist because this project
   ...
   barrel only. **AND *NOTHING INTEGRATED* IS NOW HALF FALSE, WHICH IS THE HALF THAT MATTERS
   (plan §112).** That clause used to read *`dirinvall.mjs` is an INSTRUMENT and no `AutoStack` rung
   offers this route*; the rung EXISTS — ①d, before the cascade, fitting a window of the achieved
   output mapped back through the plant's own nominal inverse onto `c - inv(y)` from the open-loop
   ...
   **AND THE ROUTE NOW REACHES FIVE PLANTS OF TEN, AND ON THE TWO ADDED IT BEATS WHAT SHIPS AT A
   ...
   MET on both (3.149, 1.338) and nothing made worse. **EVERY CALENDAR IN THIS SENTENCE IS
   UNDERSTATED AND THE DELIVERED FACTORS ARE NOT — see §131 below, where these two rigs are found to
   settle OUTSIDE their own meter: read 19.3 days → 28.0 h and 3.1 days → 16.9 h.** **THE PLACEMENT IS THE WHOLE RESULT AND THE
   ...
   **AND EVERY CALENDAR IN THE TWO PARAGRAPHS ABOVE IS UNDERSTATED, BECAUSE ON THOSE TWO PLANTS A
   `fresh()` COST THE METER NOTHING (plan §131).** `RT.makeMachine` and `RX.makeMachine` ran their
   settle loop on the RAW plant and only THEN wrapped it in the object whose `step` ticks — 2,000
   steps of tank and 1,500 of exchanger, once per excitation segment and once per scored run,
   invisible to the instrument that prices these plants (rules 17, 25). **It was not hidden: the
   project's own second route PRINTED it** — `dirinvall.mjs`'s calendar line read *of which each
   `fresh()` pre-roll is 0 steps — NOT counted by this rig, so this calendar OMITS its settle* on
   both plants, in the same runs §126 and §127 published `priceFrom`'s number from. Two routes in
   one report with nothing comparing them is one route and a decoration (rule 15b, written after
   §117 paid for this exact shape), so the comparison is a CHECK now. Repaired, with every
   delivered number byte-identical (rule 21): **the tanks' ships-today 15.9 days is 19.3, their
   budgeted 21.3 h is 39.1 h, the exchanger's 2.7 days is 3.1, and its 104.72x costs 18.9 h and not
   9.8.** The whole-ladder bills are out by 1.15-1.21x and the SHORT budgeted routes by 1.84-1.93x,
   which is the shape the fault must have — a settle is a per-`fresh()` constant, so the route that
   spends fewest plant steps is understated most. **§127's headline is reduced accordingly: the
   tanks are 11.8x less plant time than what ships, not 17.9x**, at the same 9.11x. And a budget is
   a number in PLANT STEPS, so correcting the meter changes what a fixed one buys: at the same
   60,000 the exchanger's rung ① flips ADMITTED → SKIPPED and the plant ships **94.70x in 6.9 h**,
   while 90,000 buys it back at **104.72x in 18.9 h** — a product table that could not exist while
   one of its rows was a number the instrument could not see.

   **AND THE LEVER §127 NAMED IS CLEAN ON BOTH, WITH THE PREDICTION WRITTEN FIRST (plan §131,
   rule 59).** §123's surviving candidate for the barrel's VOID is a fresh-segment initial-condition
   mismatch — `barrelSpec.fresh` settles at the recipe's FIRST level while its segments start at a
   RANDOM one — and neither real-provenance plant has that shape, both diets starting every segment
   where `fresh()` settles. Both were predicted clean and both are: the carry removes **38% of the
   excitation** on each, **every SHUFFLE control reads 1.000x** on all six draws per plant, every
   ...
   **AND THE PLACEMENT IS NOW SCORED ON THE MACHINE, ON SIX PLANTS, WITH NO FLAG — AND THE OBVIOUS
   WAY TO SCORE IT IS WRONG, WHICH IS THE HALF WORTH READING (plan §133).** §126 and §128 both ended
   with *scoring the placement is the next step and is not built*. Scoring the {①d, ①} PAIR both
   ways is what anyone builds first, and on the CART-POLE it reads **①d-first 11.791x against the
   declared pair's 5.019x** and duly picks FIRST — while the COMPLETE declared ladder reads 11.93x,
   because ①d contributes 1.08x there and leaves ②d 2.38x still to find. **The pair's own scores
   are wrong by the entire contribution of the rungs above them**, and that form shipped a machine
   1.2% worse for 19% more plant time. It is the ④ drop-one pass's own opening sentence — *a greedy
   ladder cannot see that a rung which helps alone may cost the rung above it* — arriving one rung
   lower down, inside the code written to fix the greedy ladder. **So the comparison is between two
   COMPLETE CONTROLLERS**: ①d is probed alone on the bare plant before the ladder runs, the declared
   ladder then runs untouched, and the finished machine is compared against that probe beside ④.
   Six plants, and the machine picks the better of the two library defaults on every one:
   **exchanger 94.697x against 89.773x → FIRST · quadruple tank 7.254x against 19.910x → DECLARED ·
   cart-pole 11.791x against 11.927x → DECLARED · real tanks 9.112x against 8.694x → FIRST · barrel
   4.208x against 6.997x → DECLARED · column 3.850x against 3.959x → DECLARED.** Against today's
   default it GAINS 1.05x on two plants and loses nothing on any; against `first: true` it gains
   1.83x, 1.66x, 1.03x and 1.01x on four. **THE COST IS ONE SCORED RUN AND IT IS NOT FREE** — 46.2
   → 50.9 min, 19.3 → 22.2 days, 3.1 → 3.6 days, +10% to +16% — so `DIRPLACE` ships OFF (rule 31):
   a customer who knows their plant declares the placement and pays nothing. **NOT MEASURED, and it
   is worth 1.11x on one plant (rule 25)**: the third candidate, *①d first WITH the rungs above it*,
   reaches 104.72x on the exchanger and costs a second COMPLETE ladder, teacher included.
   ...
   **AND FOUR MORE PLANTS ARE NOW PRICED FOR THE PRODUCT, AND ONE OF THEM MEETS THIS TARGET
   ...
   ```
     ...
     real heat exchanger    3.1 days   MISSED     teacher 68% · verify 32%   (was 2.7 d — §131)
     real cascaded tanks   19.3 days   MISSED     teacher 65% · verify 35%   (was 15.9 d — §131)
...
8. **MEASURED AGAINST SOMETHING THAT IS NOT US. ONE LITERATURE METHOD IS NOW BUILT AND RUN,
   ...
   **AND KOOPMAN-EDMD IS NOW BUILT, WHICH MAKES IT THREE — BUT IT IS TWO OBJECTS AND ONLY ONE OF THEM
   IS A RIVAL (plan §113).** The literature's Koopman CONTROLLER lifts the MEASURED STATE and solves an
   MPC in the lifted space, so the commissioning instrument stays bolted on for ever: **INADMISSIBLE on
   DeePC's own ground**, and priced rather than waved at — ~4,700-5,100 MAC/decision at one
   projected-gradient iteration, about half a PLC scan, so **the arithmetic would have been affordable
   and the INSTRUMENT is what disqualifies it.** The admissible form uses the lift only at
   COMMISSIONING and inverts the identified model into a fixed map of the commanded reference; that it
   reads nothing at runtime is ASSERTED rather than stated (start the machine 5 mm off and the
   correction sequence is BIT-IDENTICAL over 4,000 decisions). It is the first rival here COMPETITIVE
   with the shipped object on the distilled policy's own convention — **39.00x against 32.75x at home
   and 25.62x against 33.15x on the trajectory neither has run, at 43 MAC and 544 bytes** — and the
   first to SURVIVE the 1.6 µm noise falsifier that killed both the others (19.58x, where DeePC reads
   1.00x and ZPETC 0.06-0.27x). **IT IS NOT A DEPLOYABLE FACTOR AND THE REASON IS §56's OWN CONTROL,
   WHICH IT FAILS**: one fixed cell across four identification draws reads **38.07x / 6.27x / 3.63x /
   0.08x** at a flat fit R² of 0.70-0.79 — the last draw 12x WORSE than doing nothing — where ZPETC's
   fixed cell reproduces at 2.18-2.45x. Each draw's OWN best-of-sweep does reproduce (24.05x-49.28x),
   so the METHOD is real and the CONFIGURATION must be re-selected per commissioning against a
   delivered score, which is not a procedure a customer can run. **AND THE LIFT ITSELF IS WORTH
   NOTHING**: the matched LINEAR control has the best transfer of twelve dictionaries, the Coulomb term
   buys 5% at home for 23% off the held-out program, and the best in-sample fit in the table (RBFs on
   acceleration, R² 0.901 against 0.730) is among the worst transfers — §54.9's finding reached through
   the literature's own function class and read on DELIVERED MACHINE ERROR rather than a held-out R².
   **What decides this plant is the EXCITATION, not the lift**: both other rivals identify at the
   AUTHORITY, ±0.02 m, where the correction this axis needs peaks at 7.3e-4 m, and two decades below it
   this route goes 0.28x → 38.07x with the optimum INTERIOR in a ladder spanning 6667x (rule 41b).
   **That looked like a retraction of §56 and is NOT**: swept through `zpetc.mjs`'s own `ZPROBE` knob —
   unset BYTE-IDENTICAL, and reproducing 1.9921e-1/2.45x and 9.8764e-2/3.28x exactly — ZPETC's ladder
   PEAKS at the published ±0.02 and falls away on both sides, so §56 was right about its own cap. Two
   admissible rivals on one axis want identification excitations 100x apart, and the probe amplitude is
   a property of what the model will be USED for rather than a harness constant.
   ...
   **It diverges at every setting**, its best move on this plant is not to move, and the sweep is
   ...
   this project's own machinery in it. **Still absent entirely: modern MPC and L1 adaptive, both
   inadmissible on §54.8b's grounds. DeePC is built and DISQUALIFIED, ZPETC is built and beaten, and
   Koopman-EDMD is built in its admissible form and is the first rival to be competitive with the
   shipped object and to survive the noise falsifier (plan §113). THREE methods on ONE axis are still
   not a field.**

**The plan to get there is `docs/plan.md`**, sequenced so the experiment that CHOOSES the
route runs early rather than last.

**AND THE FORWARD SEQUENCE IS `docs/roadmap.md`, WHICH DID NOT EXIST UNTIL §118 AND IS THE SAME
FAULT `docs/scorecard.md` WAS BUILT TO FIX.** `plan.md` is a RECORD — 117 sections of what was
measured and what was retracted — and reading a priority order out of a log is exactly the
condition rule 30 names. The scorecard closed that for the ten COLUMNS; nothing closed it for the
CLAIM, so *what to do next* lived in this file's prose and a task list. The roadmap orders by what
blocks the north star rather than by column gap, and those orderings differ at the first line: the
scorecard's largest gap is INT at −6 and that file itself calls it the one that does not matter,
while the gap that makes this unbuyable is **COMMISSIONING TIME — target 4, met on 2 plants of 8,
the misses 30 and 33 DAYS**. It carries a falsifier per step and a section naming what would make
the whole ordering wrong.

**AND ABOVE THE ROADMAP SITS `docs/program.md`, WHICH TURNS THE OWNER'S TWO ADJECTIVES INTO BARS.**
*World-class* and *universal* are read as ten bars, each a count off the record — U1-U4 for the
north-star sentence, W1-W6 for what a buyer checks — and the phases that close them, each with the
measurement that would kill it. Its one-line reading: the object is closer to UNIVERSAL than to
WORLD CLASS, and the three bars it is furthest from (a commissioning in a shift, the shop's own
instrument, a real machine) are all about COMMISSIONING and none is about the controller.
...
1. **ONLINE ADAPTATION, which is the plant-based way to get memory-like accuracy.** A frozen
   ...
   **AND IT IS NOW TWO DIFFERENT LAWS WITH OPPOSITE VERDICTS, WHICH THIS ENTRY USED TO CONFLATE
   (plan §110, §110.1).** Everything below is GUIDED ADAPTATION — the pilot cascade's RLS updating
   DURING laps against a tracker — and §108 measured it, WITH A PERFECT TRACKER, monotonically
   destroying a program it never ran (circle 7.72x → 4.57x → 1.29x → **0.88x**, below doing
   nothing, on all three seeds). That is a LAW failure and not an instrument one, and it belongs to
   a component `objtable.mjs` says ships on ZERO of ten plants. **The law that DOES reach the
   shipped object is `learnLive`/`_iteratePolicy` — the page's own *Learn on this program* — and it
   does NOT have that failure**: both programs the commissioning never ran IMPROVE, monotonically,
   on three seeds, at every pass, worst of 36 scored program-rungs 8.175x. **What protects it is the
   COMMISSIONING DIET staying in every pass's fit, NOT the monotone gate**, proved by its own
   falsifier — remove the gate and diet mode still does not cross at 16 passes, while `LEARNMODE=fresh`
   (the same harness with the diet dropped) crosses at **pass 1** to 0.774x and 0.355x and its
   circle never recovers. So the gate is a stopping rule that reports convergence, not a safety
   mechanism. **The live hypothesis this raises and does not test**: §108's collapse may itself be a
   DIET-COVERAGE failure rather than an RLS-law failure, since guided adaptation updates against one
   program's rows with no diet retained — structurally `fresh` mode. Cheap to check and not checked.
   ...
   **AND THAT IS NOW ANSWERED: THE INSTRUMENT IS NOT THE BLOCKER — THE LAW IS (plan §108).** A
   ONE-STAGE soft sensor from MOTOR-SIDE SIGNALS ALONE, with no instrument anywhere at deploy,
   substitutes for the tracker INSIDE guided adaptation and delivers **0.92x-1.11x of the tracker-fed
   result over 36 scored cells**, on the FIRST SCORED LAP of programs never run, six of nine
   held-out cells inside 3%. **AND §52.23's OWN RECOMMENDED OBJECT IS THE ONE THAT FAILS**: the
   CHAIN is both the worse estimator (LOPO 0.804-0.909 against the direct map's 0.954-0.968) and the
   one that collapses over 24 guided laps, and adding §52.23's estimated-state block on top of a
   full row makes the sensor WORSE (0.95/0.97 → 0.80/0.79) — capacity transferring worse again.
   **THE BLOCKER MOVES, AND THAT IS THE RESULT**: with a PERFECT TRACKER, guided adaptation on one
   program monotonically destroys a program it never ran — the circle reads **7.72x → 4.57x → 1.29x
   → 0.88x** at 0/2/8/24 guide laps on all three seeds, BELOW doing nothing. That is not the sensor.
   Of the three killers named in advance, the R²-too-poor one did NOT fire, rule 35 FIRES and is
   bounded (in-situ R² 0.91 → 0.89 → 0.82 direct, 0.87 → 0.77 → 0.47 chain, FLAT at the bench cell),
   and *bad slowly, invisible to a short test* fires for the chain AND for the tracker. The control
   that licenses it: **`IDENT=1`, a soft-truth closure returning the tracker's own value, reproduces
   every SOFT row to every digit including update counts**, and `trackaway.mjs` is byte-identical
   across the rig change. **THE SCOPE LIMIT TRAVELS WITH ALL OF IT**: this is the PILOT CASCADE,
   which ships on ZERO of ten plants, and §52.29 measured guided adaptation as BIT-IDENTICAL on the
   shipped DISTILLED object with `oracleF0` armed — **so none of this yet touches the object a
   machine receives.** One plant, two cells, 24 laps is not for ever, and the tracker's own collapse
   was NOT bisected (lambda, the innovation gate and the authority not separated; neither
   `lambda = 1` nor `setAnchor` run). **AND IT NARROWS THE 1.79x BELOW WITHOUT REFUTING IT**: that
   figure is six laps on the ROUNDED RECTANGLE and stands there, but the GUIDE PROGRAM decides the
   transfer — guiding on the diamond takes the sharp square 1.71x → 1.93x where the rounded rectangle
   takes it 1.71x → 1.66x, worse than static.

   **THE PARAGRAPH BELOW IS SUPERSEDED AND KEPT FOR ITS REASONING (§108, §110).** Its claim —
   *what blocks this from deploy is EXACTLY ONE THING, the instrument* — is refuted: §108 built the
   soft sensor it asks for, measured it at 0.92x-1.11x of the tracker over 36 cells, and found the
   blocker had MOVED to the adaptation law, which fails with ground truth. And §110 then found the
   failure is not general to adaptation: it belongs to guided RLS and not to `learnLive`. What
   survives is the instrument ARGUMENT, which is why it is kept rather than deleted.
...
named overrides and `makeMill`'s operating point are three instruments for looking. **AND ONE HAS
NOW BEEN FOUND, AND ARMING THE GUARD ON IT IS STILL A LOSS (plan §136.4, §136.5).** This used to
read *and none has found one*. Two of fourteen mill DIET DRAWS make the machine worse at 6.5 m/s —
**0.953x and 0.985x** — so §100's stated licence is granted. Armed there the guard does exactly what
it was built to do (both SPEED rows to exactly 1.000x, the commissioned point and both GAUGE rows
untouched, the shipped diet reproducing §100 digit for digit) and **on each licensing draw it is a
1.19x and 1.22x LOSS, because it also refuses the 4.0 m/s row that is 0 of 14 harmful and never
below 1.485x**. Over the 28 uncommissioned operating points those fourteen draws carry, the guard
replaces a geometric mean of **1.861x** with exactly 1.000x to remove a worst case of 4.7%. So the
criterion is what was wrong, not the verdict: **a guard's support is the whole region it refuses and
a single point below 1.000x cannot decide it** (rule 19). The replacement is measurable — *licensed
where the DISTRIBUTION over the refused region has a geometric mean at or below 1.000x, or where the
worst-case harm exceeds the mean benefit given up* — and on this plant it reads 1.861x against 4.7%
and the guard stays OFF. Read "the deployed object carries guards" as *the object implements them
and two plants arm one*.
...
**AND THAT ROW IS THE MINORITY'S OBJECT, WHICH MATTERS THE MOMENT ANYONE PORTS THIS (plan §137).**
`DEPLOY` is `['lib/pilot/deploy.js']` and nothing else, so the whole boundary apparatus — the
117-line no-import reimplementation, `artefact.test.mjs`'s bit-exactness over 4,000 windows,
...
actually ship**, and §137's PID loop makes it six. There is no `deploy.js` analogue for
`classic.js`, no bit-exact pin, no conformance vector and no log spec: **the object the majority of
plants ship is outside the boundary this project audits.** It is also the easiest thing here to
port — four coefficients on the reference's own derivatives, 8 MAC/cycle, 0.0 kB — so the gap is
cheap to close, and it is the FIRST thing an ST translation should close rather than `deploy.js`.
**CLOSED BY §139**: `lib/pilot/autoff_runtime.js` — FB_AutoFF's deployed half — carries the
conventional rung AND the learned map in one decision, imports nothing, is classified DEPLOY, and a
reloaded record is pinned deciding bit-exactly.
...
9c. **A CONTROL THAT CANNOT FAIL IS NOT A CONTROL, AND THIS SUITE MAKES TWO OF THEM EASY TO WRITE
   BY ACCIDENT.** Both forms appeared on one day, on different files, from different readers.
   **(a) THE TIER SKIP.** A full-tier file run without `SUITE=full` prints `SKIPPED` and exits 0, so
   diffing two such runs compares two skip messages and reports BYTE-IDENTICAL — which is rule 25's
   *did not run* wearing *ran and agreed*. It happened on `distil-pend.mjs` (plan §109) and then
   again on `zpetc.mjs` (§113) while checking the first one's lesson. **Assert the run PRODUCED ITS
   ROWS** — a gain-ladder line, a factor, a count — before comparing anything.
   **(b) THE KNOB SET TO ITS OWN DEFAULT.** `ZPROBE=0.02` against unset proves nothing when
   ...
   **(c) THE SELECTOR THAT MATCHES NOTHING (plan §116).** `ONLY=pend` against a plant named
   ...
   looked at.
   **(d) AND THE MIRROR IMAGE, WHICH IS WORSE: A CHECK THAT CANNOT PASS (plan §118.1).** The three
   above cannot FAIL; this one cannot SUCCEED, for the same reason — its tier skipped the thing it
   reads. `smoke.mjs` asserts the page logs `flowsim DIVERGED` while the block that PROVOKES the
   divergence sits inside `if (FULL)`, so `--all` at the quick tier asserted the page had shouted
   about something nothing asked it to do. **A vacuous pass is silent; a vacuous FAILURE reads as a
   live regression** and sends whoever meets it looking at code that is fine. It survived because
   `FOCUS` defaults to `flexisim` (so a plain run never opens that page) and anything shared is
   verified `--all --full` (which provokes it) — the one failing combination being the cheapest one,
   which is the combination a suite is least likely to run.
   The general form: before quoting a control, ask what would make it go red — **and what would make
   it go green.** If nothing would do either, it is a sentence and not a measurement. **Assert the
   run PRODUCED ITS ROWS — a count, a factor, a named line — before comparing anything**, and where
   a check depends on a tier-gated provocation, make the other tier say so (rule 25) rather than
   pass or fail in silence.
...
61b. **A LOOP THAT EXISTS TWICE — ONCE ON THE LIBRARY'S PATH, ONCE IN A CLOSURE A HOST WRITES —
    WILL DISAGREE, AND THE CHECK BELONGS ON THE BOUNDARY, NOT BEHIND IT (plan §138).** Every rung
    `AutoStack` trains through `host.run` sees `act()`; the distilled rung's training runs are
    host-written closures, eleven copies, and every one taught on the BARE plant while the rung
    deployed on top of the conventional one — a double correction on every plant where that rung
    ships, which read as the exchanger's *nothing left* and as gain ladders walking to 0.27. It is
    the fourteenth defect of one shape in this record (lookRaw, `closed`, v/a never passed, the
    verify clock, decls, guard flags, dirInv, ①d's placement, the probe wrapper, the meter, the
    PID's tuning point...): a piece of the commissioning loop in two places and nothing checking
    they agree. So a boundary test drives `commission()` and inspects what the HOST'S CLOSURE is
    handed, and a unit test of the helper is not one. And never guess a quantity's UNITS across
    that boundary: the tank's rung is built on levels while its runs speak volts, and the first
    composed tank run was off by the plant's gain with nothing thrown.
...
| `test/pilot/freshmeter.test.mjs` | **EVERY PLANT'S SETTLE IS INSIDE ITS OWN METER, OR THE PLANT STATES IT HAS NO CLOCK (plan §131).** `meter.mjs` puts the tick inside the rig's own `step` so no caller can bypass it, and that does not defend against the other shape: `realtanks-rig.mjs` and `realexch-rig.mjs` ran their settle on the RAW plant and wrapped it AFTERWARDS, so `fresh()` cost the meter ZERO on the two plants whose calendars §126 and §127 published — 2.2 h of tank and 25 min of exchanger, once per excitation segment and once per scored run. **It was not a hidden fault**: `dirinvall.mjs` printed *each `fresh()` pre-roll is 0 steps — NOT counted by this rig, so this calendar OMITS its settle* on both, in the same runs, and nothing compared the two routes (rule 15b, which §117 wrote after paying for exactly this). Three states, one of them a fault (rule 25): ticks and `fresh() > 0` is charged; never ticks at all is UNKNOWN and is asserted to be exactly ONE plant so the exemption cannot grow; ticks and `fresh() === 0` is RED. **Checked to FAIL on the pre-repair state** — three red, naming both plants (rule 9c) — with both halves pinned against a reconstructed fault shape and its repair. |
| `test/pilot/placement.test.mjs` | **WHERE ①d GOES, DECIDED BETWEEN TWO COMPLETE CONTROLLERS — AND THE OBVIOUS FORM OF THAT DECISION PINNED AS WRONG (plan §133).** §126 proposed a placement RULE and §128 refuted it on a fourth plant, both ending with *scoring it is the next step and is not built*. Scoring the {①d, ①} PAIR is what anyone builds first and it is wrong by the entire contribution of the rungs above them: on the cart-pole it reads 11.791x against 5.019x and picks FIRST, where the COMPLETE declared ladder reads 11.93x. So the ladder probes ①d ALONE on the bare plant, runs the declared ladder untouched, and compares the two finished controllers beside ④. **BOTH HALVES (rule 9)**: a plant where ①d alone wins ships it alone with everything else dropped and the shipped score IS the probe's own scored run; a plant where the declared ladder wins ships BYTE-IDENTICALLY to the run that never scored the placement, leaving no row behind for the candidate it did not keep. **The lever that makes the two trade places had to be the INSTRUMENT and not the window** — a first-order lag is invertible from `y[k]` and `y[k-1]`, so three straddling taps read 2.4e6x where thirteen read 1.5e6x, and what weakens ①d is noise on the record it is fitted from (§50.1's own axis). The inert cases are STATED rather than skipped: no conventional rung to place against, and a caller that already declared `first` is not charged a probe to re-decide it. |
...
| `test/pilot/composebelow.test.mjs` | **THE DISTILLED RUNG'S TEACHER RUNS ON THE MACHINE THE RUNG DEPLOYS ON (plan §138).** Until §138 every host-written training run applied the teacher's correction to the BARE plant while the rung deployed on top of the conventional one, so on every plant where that rung ships the policy double-corrected — the exchanger's 0.02x *nothing left*, gain ladders walking to 0.27. `AutoStack.composeBelow` now puts the armed conventional rung under every training run, BY DEFAULT, in the rung's OWN units, which only the host knows — a run states them as `motion(k)` (`rigs/ladder.mjs` supplies it from the differencer it builds the basis with; the tank states its own, because its rung is on levels and its runs speak volts), and a run that states none is left bare with the report saying composition was NEEDED and not done. **It tests the PATH, not the helper (rule 9b)**: it drives `commission()` and inspects what the host's closure is handed, in four states — composed, the `composeBelow: false` control, needed-and-impossible, and nothing below (untouched and SILENT, so a plant with no conventional rung is byte-identical) — and it goes RED on the pre-§138 library (rule 9c). `DCOMPOSE=0` is the plant harnesses' control. |
...
| `test/pilot/distil-realexch.mjs` | **THE ONE REFUSAL THAT IS NOT ABOUT WHAT THE MAP CAN EXPRESS (plan §86.5).** The conventional rung takes 89.77x on the real steam exchanger and the distilled rung reads 0.045x of it, refused — while helping every one of its own training runs **1.33x** in sample. So the map does express this plant's correction; there is nothing left. By §84.9's screen this is the easiest plant in the set (**43 response times per program** against 5-8 for the losers) and the cheapest rung wins outright, which is the screen's prediction holding from the other side. Its ±23-step window is what found `deriveWindow`'s duplicate taps. **AND *NOTHING LEFT* IS RETRACTED (plan §138)**: the 1.33x in-sample column was taken on the BARE plant and the 0.045x verify on top of the conventional rung — two machines read as one (rule 19) — and the teacher itself had been taught on the bare plant, so the rung double-corrected. With the teacher on the machine it deploys on, the rung **DEPLOYS at 1.16x and the plant ships 103.94x** (held-out 127.5x), so this plant now ships the deployed object on top of the incumbent. |
| `test/pilot/rigs/arm-rig.mjs` | The 2R arm rig — plant, paths, routing, `commissionArm` and `deployOn`. Every harness drives the arm through this; three separate copies of pieces of it have each shipped a defect. **AND IT IS THE SERVO LOOP'S THIRD HOME, WHICH §52.37 CLAIMED HAD ONE — NOW A STATED DIVERGENCE RATHER THAN A SILENT ONE (plan §111.1, §111.2).** Its default is `ARM_BW_LEGACY = 2e-3`, the PRE-§52.37 loop, against `compensator.js`'s shipped `BENCH_SERVO.bandwidth = 1.6e-2`, and it reaches six instruments (`twin.test.mjs`, `_ssforecast`, `_mimobudget`, `_hpose`, `_armswhich`, `_binding`) whose whole recorded history is on that cell — so unifying it is a re-measurement with its own controls and not a tidy-up (rule 31). **The SHIPPED path is unaffected**: `distil-arm.mjs` routes through `autohost.js`, which reads `bandwidthFor`. What changed is only the silence: the shipped constant is IMPORTED at the site that disagrees with it, the old value is NAMED with its reason, and `servoProvenance()` reports both — reading `BENCH_SERVO` at CALL TIME, so a future unification reports agreement by construction rather than needing this line edited too (rule 30). Asserted both ways (rule 9): unset reads `onShippedLoop: false` naming both values, `ARM_BW=1.6e-2` reads true. `invert.mjs` is BYTE-IDENTICAL across the change over 83 lines and 13 plant rows (rules 21, 9c). **The debt is NOT paid** — six instruments still measure the arm at a loop the product does not use, and that is now defensible only because it is stated. |
...
| `test/pilot/koopman.mjs` | **TARGET 8's LAST NAMED RIVAL, AND THE ADMISSIBILITY VERDICT IS HALF THE RESULT (plan §113).** There are TWO Koopmans and only one is a rival: the literature's Koopman MPC lifts the MEASURED STATE and solves in the lifted space, so the commissioning instrument stays bolted on for ever — **INADMISSIBLE on DeePC's own ground (§54.8b)** — and it is PRICED rather than waved at, ~4,700-5,100 MAC/decision at one projected-gradient iteration, about half a PLC scan, so the arithmetic would have been affordable and the INSTRUMENT is what disqualifies it. The admissible form uses the lift only at COMMISSIONING, inverts the identified model and deploys a fixed map of the commanded reference; **that it reads nothing at runtime is ASSERTED — start the machine 5 mm off and the correction sequence is BIT-IDENTICAL over 4,000 decisions** (§81's form). Built as the nonlinear generalisation of §56's ZPETC, with `LIFTS=none` the MATCHED CONTROL inside the same table so whatever the lift is worth is the gap between two rows (rule 20). **THE LIFT IS WORTH NOTHING AND THE EXCITATION IS WORTH 136x.** Twelve dictionaries: the linear control has the BEST transfer, the Coulomb term buys 5% at home for 23% off the held-out program, and the best in-sample fit (RBFs on acceleration, R² 0.901 against 0.730) is among the worst transfers — §54.9 reached through the literature's own function class and read on DELIVERED ERROR. What moves it is rule 41b: `zpetc.mjs` and `deepc.mjs` both identify at the AUTHORITY, ±0.02 m, where the correction this axis needs peaks at **7.3e-4 m** (measured — the open-loop error applied as a hand feedforward reaches 65.7x at |u| 7.27e-4), and the ladder reads **0.28x at that default and 38.07x two decades below it, INTERIOR in a span of 6667x**. **THAT LOOKED LIKE A RETRACTION OF §56 AND IS NOT**: `zpetc.mjs` gained `ZPROBE` (unset BYTE-IDENTICAL, reproducing 1.9921e-1/2.45x and 9.8764e-2/3.28x exactly) and its ladder PEAKS at the published ±0.02 — 1.01x at 1e-1, 1.53x at 4e-2, **2.45x at 2e-2**, 1.53x below — so two admissible rivals on one axis want excitations 100x apart and the probe is a property of what the model is USED for. **IT IS THE FIRST RIVAL TO SURVIVE THE NOISE FALSIFIER** (19.58x at the rig's own 1.6 µm, where DeePC reads 1.00x and ZPETC 0.06-0.27x) and the first COMPETITIVE with the shipped object on the distilled policy's OWN convention — 39.00x against 32.75x at home, 25.62x against 33.15x on the sine, at 43 MAC and 544 bytes. **AND IT IS NOT A DEPLOYABLE FACTOR, BY §56's OWN CONTROL, WHICH IT FAILS**: one fixed cell over four identification draws reads **38.07x / 6.27x / 3.63x / 0.08x** at a flat fit R² of 0.70-0.79, the last 12x worse than doing nothing, where ZPETC's fixed cell reproduces at 2.18-2.45x. Each draw's own best-of-sweep does reproduce (24.05x-49.28x), so the METHOD is real and the CONFIGURATION must be re-selected per commissioning against a delivered score — which is not a procedure a customer can run. Both scoring conventions are printed because the two on record are not the same loop (rule 19), and the (b) object is RE-COMMISSIONED in `driveRef`'s own loop since the two differ 18% in open loop. Nothing is integrated; it is an INSTRUMENT. |
...
| `lib/pilot/autostack.js` — the ②d gain axis, `distilkit.mjs` — `gainLadder` | **THE APPLIED GAIN, THE THIRD LADDER AXIS AND THE CHEAPEST ONE THIS PROJECT HAS (plan §79).** The ridge axis regularises the FIT; the gain the fitted map is APPLIED at is a different quantity with its own optimum, and nothing here had ever scored it. It was found by an ACCIDENT — §78.5's false refusal zeroed the tank's map on 28% of its steps and read 19% better — and then reproduced without the bug: a uniform 0.90 gives the guard's number to three figures, so there was no structure in it, only a gain. A candidate needs **NO REFIT** (scaling the stored weights by `g` is exactly scaling the output), so it costs ONE SCORED RUN against the ridge axis's refit-plus-run, and the deployed object is unchanged in form — same vector, same MAC, same bytes, because the gain never appears at deploy. **1.0 IS IN THE GRID**, which is what makes it a measurement rather than a tuning: the TANK picks 0.85 (2.593x → 3.268x, and 2.301x → 2.657x on a recipe it was not chosen on). **AND §84.6 RETRACTS THE REST OF THIS ROW'S OWN HEADLINE.** It read *the mill, column, barrel and arm all pick 1.0 and come back byte-identical* — but 1.0 was the TOP OF THE GRID, so three of those four picks were EDGES and not optima, which is the exact fault this row names two sentences later and fixed only for the arm. Widened above 1: **only the MILL's 1.0 survives as an interior optimum; the COLUMN picks 1.15 for 3.643x → 3.959x and the BARREL picks 1.15 for 6.116x → 6.997x** — three plants of four want a gain off 1.0 and two want it ABOVE, which a one-sided grid could never have found. `DEFAULT_GAINS` is now `[0.72, 0.85, 1, 1.15, 1.3]` (0.5 dropped: worst on all four, never picked). **And both of §79's hypotheses for why the tank differs are dead from the same scrape**: the picked ridges are mill 1, barrel 1, tank 0.1, column 1e-3, so the two plants SIX decades from the arm's `1e-6` pick 1.00 and 1.15 while the tank at five decades wants shrinking; and the barrel and column are setpoint sequences too. What the four points do line up with — offered as a hypothesis — is the fit's own held-out R², monotonically: 0.993 → 0.85, 0.857 → 1.00, 0.85 → 1.15, 0.57 → 1.15. A heavily-ridged fit is shrunk by its own regulariser and the machine wants some back; a fit that interpolates its diet wants damping, because confidence about the DIET is not confidence about production. Ridge and gain as two ends of one knob, which is `qpIters` and `lambda` on a second route. The arm's is the one worth the extra wiring — it does not carry the ridge ladder, so it would have been byte-identical BY CONSTRUCTION, and "the block was skipped" and "the block ran and chose 1.0" are different states (rule 25). Asked through `GAINS=` with the grid widened ABOVE 1, because a one-sided grid cannot find an optimum at its own edge, it reads **3.969x at 0.85 and 3.973x at 1.15** against 6.623x at 1.0 — a locally quadratic optimum sitting exactly on 1, on the fitted program and both it has never run. Rule 42's band breaks toward the LARGEST gain: change the shipped object as little as the measurement allows. `GAINS=none` is the control. **AND THE GRID NOW FOLLOWS ITS OWN PICK, BECAUSE §84.6's REPAIR WIDENED ONE SIDE AND LEFT THE OTHER (plan §86.6).** The bottom stayed at 0.72 and the three plants added since — the cart-pole, the real tank, the real arm — every one picked 0.72, which is the same EDGE fault at the other end. A wider fixed grid is the wrong repair (it charges every plant a scored run for a region only some occupy, and the tank's verify is already 44% of its bill), so if the best-scoring candidate is the smallest or largest tried the grid steps once more in that direction at its own geometric ratio, bounded at six steps and at (0.02, 4). It extends on the ARGMIN and not on the band's tie-break winner, or a rung inert at every gain would walk upward for ever. **AND A FREE SCRAPE OF THE PASSING SUITE'S OWN ROWS SAYS THE UPWARD RUNAWAY DOES NOT HAPPEN ON SHIPPED DIETS AND THE BOUND BINDS AT THE OTHER END (plan §107).** Three of four picks are genuine INTERIOR optima with the score rising on both sides (mill 1.00, column 1.15, barrel 1.15) — but the TANK's extension walks DOWNWARD through all six steps monotonically, 0.72 → 0.266 with the machine score falling 4.36e-1 → 1.63e-1 the whole way, so **it exhausts the six-step bound without the argmin ever turning and the pick IS the bound** — the same EDGE fault §86.6 was built to remove, surviving at the far end because the bound binds before the curve does. **And the rung is REFUSED anyway at 0.16x**, losing to the conventional rung's 19.91x, so the axis spends 11 scored runs selecting a gain for a candidate the machine throws away — which itemises §84.5's standing observation that this plant's verify share is the outlier at 44% against 16-19%. The fix is NOT widening the bound (that buys a better gain for a refused rung) but an EARLY EXIT when the best candidate already loses by more than the remaining extension can close. **The control is that mill 2.62x at 1.0, tank 3.268x at 0.85, column 3.96x at 1.15 and barrel 7.00x at 1.15 all come back BYTE-IDENTICAL** — four interior picks, nothing to extend (rule 21) — and where it fires it is worth the cart-pole's 6.30x → 11.93x and the real tank's interior 0.314. **AND THAT EARLY EXIT IS NOW BUILT, AND THE BOUND IT REPLACES WAS TIGHT ENOUGH TO CHECK IT (plan §109).** `gainLadderExit` takes the BEST per-step improvement observed anywhere on this ladder and asks whether SUSTAINING it for every remaining step could reach the bar — optimistic by construction, because the gain → 0 limit applies no correction at all and so converges TO the bar rather than passing it, which makes *still a factor away at your own best rate with the whole budget spent* a real statement that there is nothing to select. On the tank it stops **BEFORE EXTENDING AT ALL, saving all six scored runs** on the plant whose verify share is the outlier at 44%. **That count is a correction of §109's own (plan §109.1): it first read "one step, five runs saved", because §107 printed only the tank's EXTENSION rows and the replay therefore fed the exit one candidate at a time, where the live ladder scores the whole BASE grid first and so decides one step earlier.** The instrument was incomplete before anything else was (rule 17), and a LIVE run settled it rather than more reasoning; the test now replays base-then-extension as `AutoStack` does and pins `spent === 0` exactly, because the bound it used instead is what let the discrepancy hide. **It cannot move a winner**, because it is consulted only where the best candidate already LOSES the bar — asserted, since that is the one case where being wrong costs a shipped result. **And it predicted the thing it skipped to within 0.9%**: from the base ladder ALONE the exit says the full walk reaches only 1.6197e-1, and the recorded six-step walk actually ends at 1.6348e-1 — so on the one plant where it can be checked against a completed record the bound is tight rather than merely conservative, and deciding a step EARLIER on strictly less information did not cost it accuracy. **The tank's refused rung therefore now reads 4.3624e-1, 0.06x where §97.3 and §107 record 0.16x** — both stand, 0.16x being what the rung reaches when six scored runs are spent finding its best gain and 0.06x what it reaches when they are not; it is REFUSED either way and the machine receives the same controller, which is the whole argument for not paying for them. The delivered 19.910x and the entire ridge ladder are bit-identical including 16-digit held-out R² (rule 21), and commissioning falls 3.2 → 2.9 days. **THE CART-POLE IS THE CONTROL AND IT IS SHARPER THAN IT WAS CHOSEN FOR**: its extension traverses the SAME SIX GAINS as the tank — 0.6099 · 0.5166 · 0.4376 · 0.3707 · 0.3140 · 0.2660, identical because both start at 0.72 and step at the grid's own ratio — and walks downward just as monotonically, yet is untouched, so the rule discriminates on the BAR and not on the shape of the walk; `SUITE=full distil-pend.mjs` is byte-identical over all 70 lines against `dcdf93e` in an isolated worktree, 11.93x shipped and the rung DEPLOYED at 2.560x. **Its first control run was VACUOUS and that is on the record**: `distil-pend.mjs` is full-tier and prints SKIPPED without `SUITE=full`, so the first diff compared two skip messages and read BYTE-IDENTICAL — rule 25's *did not run* against *ran and agreed*, for the fifth time in this project. `gainEarlyExit: false` is the knob, `rep.distil.gainExit` carries the reason, and the shared printer leads with it (rule 27). **AND THE REACHABILITY HALF WAS NOT WHAT IT LOOKED LIKE EITHER — rule 9b, a fifth time.** That diagnostic lives in `distilkit.mjs`'s `reportDistil`, and **`distil-tank.mjs` does not call `reportDistil`**: it keeps its own copy of the gain-ladder format, so the one plant §107 measured the walk on had no route to the message explaining why its walk had stopped. Asked whether the line appears, the honest answer was *no* — and that absence would have been REPORTING and not reachability, which is exactly the confusion that let §82's, §78.5's and §100's guards each survive a passing unit test. The print is in the tank's own block now; the STANDING DEBT is that the second copy of the format is still there and belongs in the shared printer. |
...
| `docs/roadmap.md` | **THE FORWARD SEQUENCE — what has to happen for the north-star sentence to be TRUE, ordered by what blocks the CLAIM (plan §118).** Three documents could each drift into being the others (rule 30), so it states its boundaries: `plan.md` is the RECORD, `scorecard.md` rates ten COLUMNS against the incumbent and orders them by gap size, and this orders by blockage — which differs at the first line, since the scorecard's largest gap is the one it calls irrelevant. **Every number in it is a citation and never a re-measurement.** Four gaps: COMMISSIONING (target 4, met on 2 of 8, the misses 30-33 DAYS — the only one that makes this unbuyable rather than weaker, with §117's teacher-free placement as the lever and §109's early exit as the unbuilt half that turns a factor into a calendar); PROGRAMS (5 of 11, where §84.9's `prog/rise` screen already predicts which miss and the repair is a DIET demonstrated twice); the INSTRUMENT (64 touches buy a tracker on ONE plant, because `distilProbePts` lives in `autohost.js` and nowhere else — `_iteratePolicy`'s arm-only history repeating); and that **NOTHING HERE HAS EVER MOVED A MACHINE**, which no item in the first three reduces by any amount. It also lists what is deliberately NOT on it with the measurement that closed each — a better map, deleting the cascade, a runtime guard, more rivals, INT — and ends with the four results that would make its own ordering wrong. |
| `docs/program.md` | **THE BARS — what *world-class* and *universal* mean as measurements, and the program that closes each (plan §119-§122 are its first four steps).** Ten bars, every one a count on the record: UNIVERSAL is any plant (10 of 10 improve-or-refuse), any program (5 of 11 MET, 1 made worse, of the SHIPPED configuration), any loop, any feed (one plant); WORLD CLASS is a commissioning in a shift (2 of 8; the two 30-day plants 3.1 and 3.8 days budgeted, §120), the shop's own instrument (a constant to commission since §121), the scan (met deployed, the fit scoped), the field (3 rivals, 1 axis), robust and self-reporting (one plant), and a real machine (ZERO). Six phases ordered by what a buyer checks — calendar, program, instrument, robustness, rivals, hardware — each citing its roadmap steps, its closing measurement and its kill. It also lists what world-class CANNOT mean here (a better map, runtime adaptation, a runtime guard, loop synthesis, readable coefficients, more rivals on one axis), each closed by a cited measurement. |
| `test/pilot/rigs/pidloop-rig.mjs`, `distil-pidloop.mjs` | **THE SANITY CHECK — THE MOST ORDINARY PLANT HERE, AND THE INCUMBENT TAKES ALL OF IT (plan §137).** One PV, one MV, one PID, a setpoint that moves: FOPDT (K 80 °C, tau 60 s, theta 20 s) under an ISA-form PID with back-calculation anti-windup and derivative on measurement, tuned by **SIMC at its published rule**. Every other plant here was chosen for being hard, and a block about to be translated to ST has to do the obviously right thing on the case whose answer everyone already knows. **TWO CONSTRAINTS DESIGNED IT AND BOTH COME OUT OF THE RECORD**: the setpoint must MOVE (§105 — a regulator's window is identical at every k and the mill reads a correction spread of exactly `0.0e+0` to prove it), and the valve must be NONLINEAR (§55 — equal-% trim R=50, two-parameter stiction, a rate limit; `LINEAR=1` is the matched control). **The block TRIMS THE SETPOINT and does not replace the loop**, which is `pendSpec`'s shape and the only retrofit an installation accepts; the measured vector is the PV and the valve output, so no instrument is added. **It ships the CONVENTIONAL rung at 5.91x in 13 laps on 4 coefficients, 8 MAC/cycle and 0.0 kB, refuses the distilled rung with a stated reason, meets target 1 at 0.933, and makes nothing worse.** **§52.32's KILL DID NOT FIRE**: swept over an 8x span of SIMC's own constant the denominator moves 2.8x and the factor moves 1.13x (5.77x / 5.91x / 6.52x / 6.53x at tauc 10/20/40/80 s), target 1 MET in every row. **§55's MECHANISM IS CONFIRMED AND MY PREDICTION NAMED THE WRONG OBJECT**: the LINEAR valve reads **27.93x against the nonlinear plant's 5.91x, a 4.7x collapse** — P2 predicted the DISTILLED increment would collapse and there is none, so what collapses is the INCUMBENT's factor. The size orders correctly against the record (a hard clip 307x, a smooth exponential 15x, an equal-% valve 4.7x), which is what makes it a mechanism (rule 18), and **5.91x is the number that belongs in a portfolio table, never 27.93x**. **Its first version built the straw man it exists to avoid** and the smoke test caught it: tuned at travel 0.5 where the schedule lives at 0.70-0.90, `Kc` was 2.2-4.9x too high, the loop never settled and the conventional machine would have been a badly tuned PID — §52.32 on the rig rather than on the plant. **AND THE OPEN ITEM IT LEFT WAS SYSTEMIC, NOT THIS PLANT'S (plan §138)**: the distilled rung's teacher ran on the BARE loop while the rung deployed on top of the conventional one, on every plant — the 0.30x refusal was that double correction. Taught on the deployed machine it refuses at **1.00x** with held-out R² 0.09, which is the real reason: what a self-tuned PID+FF leaves here is lap-specific, not a function of the reference. The end-to-end trace that found it is §138, and `composebelow.test.mjs` pins it. |
| `lib/pilot/autoff.js`, `lib/pilot/autoff_runtime.js` | **FB_AUTOFF — THE WHOLE COMMISSIONING AS ONE SCAN-CYCLIC FUNCTION BLOCK, SHAPED FOR THE ST PORT (plan §139, `docs/block.md`).** §138's fourteen defects were one shape — a piece of the commissioning loop written twice, library path and host closure — so this block has NO host closures: it is called once per scan with the reference preview and the measurement, and every experiment is applied by writing it into a live record and calling the same `affDecide` the deployed controller runs for ever. 1-4 channels, LREAL throughout, every array allocated in the constructor, every heavy computation a resumable job charged against `udiMacBudget` before it runs, integer state and reason enumerations, a checksummed record with a fail-safe load. v1 carries the CONVENTIONAL rung (`ClassicFF`'s algorithm, in production) and the TEACHER-FREE map ①d fitted from a generated excitation with the conventional rung armed underneath. On the §137 PID loop it reads **6.16x against `ClassicFF`'s 6.14x on the same program, running 0.4067 against 0.4066 °C**; on Wood–Berry under BLT **5.17x** (3.53x × 1.46x) in 15.7 days of column; the worst scan anywhere, the commissioning included, is 9,997 of 10,000 MAC. The build measured four things its first draft got wrong: per-channel capping bends a coupled Newton step (1.74x → 3.62x on Wood–Berry when fixed); an experiment's switch is a transient the next lap scores (a MEASURED warm-up lap, 4.56x → 5.17x); two kernels charged for themselves inside checked units (the budget check went red by one MAC); and a test plant with a derivative cross-coupling tripped the guard correctly. The runtime half imports nothing and is the first time the object most plants ship sits inside the deploy boundary. |
| `test/pilot/fb_autoff.test.mjs` | **THE BLOCK, DRIVEN AS A PLC DRIVES IT (plan §139).** A host writes the preview and the measurement, pulses `xCycleStart`, applies `aRefOut` and holds its program while `xOwnsRef` — nothing internal is called. On the PID loop, Wood–Berry under BLT and four channels, it asserts: the zero control bit-exact; nothing made worse; the reported factor against an independent scored run, and the conventional rung against `ClassicFF` on the same program (rule 15b, asserted within 1.25x); the MAC budget on every scan including the commissioning (it went red by one MAC before the fix, so it can fail — rule 9c); save/load bit-exact with a one-ulp weight change, another key, another channel count and a missing record each REJECTED; abort, disable-mid-excitation, a program change mid-commissioning and a denied excitation each doing the stated thing. ~15 s. |
| `docs/block.md` | **THE ENGINEER'S PAGE FOR FB_AUTOFF** — interface tables (config, VAR_INPUT, VAR_OUTPUT), the three-line host contract, the state flow, the typical PID temperature-loop wiring with its measured numbers, what v1 excludes with the measurement that would justify adding each, NOT CLAIMED, and the notes for the ST port (types, record layout and checksum, jobs as CASE statements, conformance by bit-exact replay). |
