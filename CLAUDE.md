...
| `test/pilot/dirinv.mjs` | **LEARN THE COMMAND DIRECTLY — the owner's routing, run dynamically, and the first thing here that wins without a TEACHER (plan §93).** The proposal: *set and actuals routed in, the ground truth routed as the "setpoint" at commissioning, and after commissioning the setpoint is the setpoint.* It is direct inverse learning, it deletes §49's law, §73.13's 74-89% teacher bill, §80.3's lap-indexed DIS ceiling and §90.2's cascade dependence at once, and this project had already done it STATICALLY at 23-44x (brick 40's `ikfree.test.mjs`) and never dynamically. Fit a window of the ACHIEVED `y` onto `c - y`; deploy `c = r + f(r)`. The truth LABELS THE INPUT rather than computing a target, so it reuses `DistilPolicy` UNCHANGED — same coefficients, same straddling window, same clamp, same guard, same `deploy.js`. **THE SHIPPED LINEAR ARTEFACT LOSES AND ITS FIT IS INTACT**: 0.588x on a scribble diet and 0.701x on an own-class one, both worse than doing nothing, with in-diet held-out R² 0.9953/0.9265 and R² on the PROGRAM of **-1.886/-1.017**. Six causes measured and every one dead — SCALE (1.000x→0.588x monotone over a 50-fold gain ladder, so the shape is wrong not the size), the DEPLOY INPUT (achieved reads 0.587x against desired's 0.588x, so the textbook distribution objection is worth 0.1%), the DIET (an own-class diet at 98.0% of the program's own error against the scribble's 10.7% moves 0.588x → 0.701x — right direction, nowhere near enough), the FORWARD WINDOW (**my own hypothesis, refuted by its own signature**: shrinking the forward reach 96→1 was predicted to move in-diet R² DOWN and program R² UP, and both move together and barely, 0.701x → 0.703x), and NON-UNIQUENESS both WITHIN the diet (0.0317 at the closest bin) and — the one that matters — **ACROSS the diet→program boundary, where each program window's NEAREST diet window carries a command disagreeing by 0.0462 of the target's rms, which BOUNDS R² at 0.998 on the program FROM THE DATA before any fit**. The within-diet check had passed because it was pointed at the wrong pair of signals, which is rule 19 aimed at a falsifier. **SO WHAT IS LEFT IS THE FUNCTION CLASS, AND IT INVERTS §54.9.** The same rows through a LOCAL model read **R² 0.9966 and deliver 22.599x on the machine** — the bound predicting it and the machine returning it by routes sharing no arithmetic (rule 15) — against the linear map's 0.701x. Under the TEACHER routing §54.9 measured ridge 0.8610 beating kernel ridge, locally weighted, an MLP and kNN; under THIS routing the linear map is the only thing in the way. Two routings, opposite verdicts on one question, one plant. **THREE THINGS AT THE SAME VOLUME.** It CANNOT SHIP: 8769 windows x 49 taps is **430k MAC/decision, 43x over the whole budget rather than 10% of it, in 1.7 MB**, against the deployed object's 78 MAC and 0.2 kB. The diet reading is CONFOUNDED and the file says so — the scribble bank applies 0.05 mm against a 0.58 mm target, so it is not correcting wrongly but barely correcting, and class is confounded with magnitude at 10.7%; what is established is that a diet nine times too gentle buys nothing, not that the class must match, and the shipped object's own 33.15x on a two-tone sine is a STRONGER transfer claim than this. And it is one plant, one seed, one program, on the rival's own strong ground. **WHAT IT OPENS**: the local model is a FREE TEACHER — every teacher here spends plant time and this one is a lookup over records already taken — and the distillation machinery has never been pointed at a teacher that costs no laps. Its falsifier is on record and G is already evidence against it (a linear map of `c - y` on these rows reads 0.701x), the distinction being that fitting a linear map to the LOCAL MODEL'S OUTPUT across many programs is a different fit. `REACH`, `TAPS`, `RIDGE`, `SCRIB`, `SEED`. **AND THE ONE QUESTION THAT MATTERS COMMERCIALLY IS NOW ANSWERED AND IT IS NO (plan §94).** The local model is a FREE TEACHER — a lookup over records already taken — and it cannot be distilled onto the shipped artefact. **The distillation set is free too, which is what makes this decisive rather than suggestive**: every other teacher here must be CONVERGED ON THE MACHINE per training program (§84.5 prices a diet enlargement at 10-17% of a commissioning), while this one's input is a window of the COMMANDED REFERENCE and can be evaluated on ANY reference with the machine switched off. Four trajectories, **25,167 labels, ZERO plant steps**, target rms 0.5371 mm against the true target's 0.5764 — and a global linear map of that window reads **held-out R² -0.1142 against a shuffled null of -0.0060** and REFUSES, applying nothing. So *more data would have fixed it* is ruled out BY CONSTRUCTION, which no other negative in this record can claim. The prediction was written down first and held (rule 59): a map right locally and wrong globally is a locally-valid LINEAR APPROXIMATION of a relation that is not linear, and relabelling does not change a function class. **AND IT CORRECTS §93's OWN IN-DIET NUMBER**: that section reports the direct fit at held-out 0.9265 within its diet, and those folds are contiguous within trajectories whose accelerate-hold-decelerate segments repeat, so rows far apart in time are near neighbours in the window and the fold measures NEIGHBOURHOOD LEAKAGE (rule 36). Pooled across four trajectories the same class reads -0.11, so the honest pair is **-0.11 and -1.02**, not 0.93 and -1.02. **The boundary is therefore the FUNCTION CLASS and it is §54.9 exactly inverted** — the relation is a function of the window to R² 0.998 from the data, a LOCAL model of it reads 0.9966 and delivers 22.599x, and a GLOBAL LINEAR map of it reads -0.1142 and delivers nothing, where under the TEACHER routing §54.9 measured ridge 0.8610 beating kernel ridge, locally weighted, an MLP and kNN. **WHAT IS CLOSED**: the direct inverse cannot ship through the artefact this project deploys — not for want of data, diet or teacher, but of expressiveness. **WHAT IS OPEN, with its falsifier and its cost**: the artefact need not be linear in the RAW window, and a LIFTED basis is still linear-in-parameters, still a weight vector, still `deploy.js`'s dot product, costing MAC rather than architecture — §54.9's negative on lifts belongs to the verdict §94 inverts and does not carry. Relabel with the same free teacher, fit over a lift, read held-out R² against the teacher's own labels: **below about 0.9 the route is closed for good; above it the only remaining question is how many MAC the lift costs.** **AND THE LIFT RAN AND THE BAR REFUTED IT, SO THE ROUTE IS CLOSED (plan §94.1).** `signOffsets` — the library's own lift, `classic.js`'s `sign(v)` and `|v|` basis, which is the shape a 61-bin friction curve, a drive saturation and an encoder quantisation actually have, none of them recoverable from a window of POSITIONS — fitted on the identical free label set reads held-out **-0.4340 / -0.0182 / 0.0020 at 1 / 5 / 15 sign taps** against the raw window's -0.1142. The best of four is 0.0020 against a bar of 0.9. **And the one row that DEPLOYS is the capacity gate's own false positive caught by the machine**: at 15 taps the fit vouches at R² 0.0020, a hair over its shuffled null, and delivers **0.698x — worse than doing nothing**, reproducing the raw map's 0.701x. That is `distil.js`'s stated property in the open — the gate is a PRE-FILTER that deploys about one commissioning in twelve on a pure-noise target and the decision is a machine-scored verify — so the two-stage design worked (rule 16). **§93 and §94 together are therefore a COMPLETE BOUNDED RESULT**: the routing is real and its teacher is free (22.599x, no teacher, no cascade, no lap index, from records already taken), the information is in the window to R² 0.998 measured with nothing fitted, and no linear-in-parameters map of that window can express it — **-1.02 fitted directly, -0.11 distilled from the free teacher on an unlimited set, 0.00 lifted in the direction the physics names**. The obstacle is the FUNCTION CLASS, measured from four directions rather than argued. **WHAT WOULD REOPEN IT IS NOT A FIT**: the artefact would have to stop being a dot product, and the thing that works is 430k MAC in 1.7 MB against a 10,000-MAC budget — 5,500x the shipped object — so that is a deploy-boundary decision and not a regression to improve. **AND §94's MEASUREMENTS STAND WHILE ITS MECHANISM NO LONGER DOES — AN OPEN DISCREPANCY, STATED RATHER THAN CLOSED (plan §98).** §94 tested only GLOBAL forms and named the gap itself; built PLC-shaped — R local linear maps on the window's own velocity, blended, 51 MAC — the table reads **0.9607 / 0.9601 / 0.9581 / 0.9554 / 0.9539 and 5.681x / 5.616x / 5.436x / 5.206x / 5.084x at R = 1 / 2 / 4 / 8 / 16**, and **`R = 1` IS A GLOBAL FIT AND THE BEST ROW** — locality contributes nothing and is monotonically harmful, the capacity signature for the ELEVENTH time. What is left to explain is that **two global linear fits of the same window, on the same diet, deploying the same `c = r + f(r)`, read 0.701x and 5.681x on the machine**. THREE EXPLANATIONS TESTED AND ALL DEAD: the RIDGE (rule 32, with §70's and `headroom.mjs`'s precedent behind it — swept seven decades, program R² -1.017 → -0.346, never recovering, so §94's figure is a measurement and not a carried constant); STANDARDISATION and the SIGN TAPS (a 2x2, all four cells -0.999 to -1.017 and 0.701-0.706x). **So §94's measurements survive every test aimed at them and its CONCLUSION — *the obstacle is the FUNCTION CLASS, measured from four directions* — is no longer safe to quote**, because a plain global fit of that same class reads 5.681x. One of the two paths is not measuring what it is being read as measuring, and it is not yet known which. **NOTHING IS INTEGRATED ON THE STRENGTH OF IT** (rule 16), **§94 IS NOT RETRACTED**, and the route is in no plant's ladder — on the only plant it has been measured on it delivers 5.681x against an incumbent's 424.8x, so its value is hypothetical until it is asked on the barrel and the column where the teacher costs 82-84% of a 30-day commissioning. **THE NEXT STEP IS NAMED SO IT CAN BE PICKED UP COLD**: dump both paths' feature rows for the SAME window index and diff them element by element — the difference is in the row or in what the row is evaluated against, both paths are ~60 lines, and `consist.mjs`'s `DUMP=` already does exactly this for the shipped routing and is the model to copy (rule 61). |
