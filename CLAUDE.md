...
| Linear AND nonlinear alike | **STILL CONTRADICTED, BUT NO LONGER ONE ERROR CLASS** | The ordering by nonlinearity is no longer wrong at the T⁴ end — **the BARREL now deploys at 10.61x** (plan §66) — while the tank sits marginal, and the barrel — correctly refused, and now for a measured reason rather than an inferred one (plan §63.9). **WOOD–BERRY IS NO LONGER THE COUNTEREXAMPLE**: the DEPLOYED object beats doing nothing AND the published BLT in both scoring conventions (plan §64), where the pilot cascade it used to be measured through delivers 0.39x. What changed is the middle: the COLD MILL is a linear plant with a dominant transport delay and a periodic exogenous disturbance, and it now wins 1.45x median across 8 of 8 seeds, past the gaugemeter AGC that AMPLIFIES its dominant disturbance by 3/2. So the honest description is no longer "one error class" — it is two: mechanical compliance and friction (arm, EMPS), and periodic disturbance rejection through a declared dead time (mill). Three of six is still not "alike", and what changed on Wood–Berry is the OBJECT rather than the plant: declaring its delay is inert (0.3%), its horizon is inert from N 71 to N 236, and arming the off-diagonal solve is worth 1.74x and still refuses — the win comes from replacing the cascade with the deployed map entirely (plan §62.4, §64). |
...
4. **COMMISSIONING IN MINUTES, NOT AN AFTERNOON. MET ON THE ARM — 17 MINUTES TO 2, AND THE
   ...
   **ONE of the four plants that state a clock meets this target, and the spread is 1643x.** The
   ...
   a target counted in steps is measuring the simulator. **AND THE TWO PLANTS THAT REFUSE ARE THE
   TWO MOST EXPENSIVE** — Wood-Berry spends 6.3 days to arrive at "no controller" and the barrel
   3.8 days to arrive at 0.22x and the same answer. The refusals are correct and are this project's
   strongest claim; on the two plants where refusing is right it is the most expensive outcome
   available, and nothing in this north star prices a refusal.
...
7. **BREADTH, WHICH MEANS WINNING WHERE IT CURRENTLY LOSES. HALF MET.** The second clause is
   ...
   **and the six-plant pass REFUSED it**: five plants byte-identical, zero improvements, and the
   ...
   the other standing refusal and **IT IS ONE NO LONGER: 10.61x DEPLOYED (plan §66)**, by the
   object that ships rather than the teacher, once three harness defects and a diet were repaired
   — so BOTH standing refusals are closed and neither was closed by a controller change. A method that wins only on compliance and
