...
1. **PROGRAM-AGNOSTIC. ASKED ON ALL TEN PLANTS NOW, MET ON FIVE, AND THE COUNT IS A SCRAPE (plan §88, §88.9).**
   ...
   **FIVE OF TEN ASKED MEET THE BOUND, ONE HAS A PROGRAM MADE WORSE, AND NOTHING IS LEFT
   UNASKED — BUT *TEN OF TEN* IS AN INFERENCE AND THE AGGREGATE HAS NOT BEEN RE-SCRAPED
   (plan §109.2).** Each ROW above is scraped where it was measured, and the tank's new row is a
   live run; what has NOT been re-run is `objtable READ=1` over all ten, which is days of plant
   time. What was checked instead is cheap and is stated for what it is: every one of the ten
   `distil-*` harnesses contains a `t1:` emit, and the tank's row renders. **§88.9's own lesson is
   that prose and scrape disagreeing is the condition this table exists to remove**, and it caught
   this file once already reading *5 of 8 MET* against a scrape's *4 of 7* — so until the next
   full suite emits them together, treat the membership as verified per row and the TOTAL as
   pending (rule 25). The flagship plant is the cleanest row: the 2R arm reads **10.88x on the rounded rectangle and 15.94x on
