 ...
 *
 * THE TWIN COLUMN (plan §46, the owner's live-optimisation architecture). The pilot is a live
 * path optimiser already; what bounds it is measured — it sits AT its forecast bound, and an
 * ILC-class result needs sixty times less residual variance than a lag-window regression will
 * reach. The compiled twin predicts this machine far better, so the question that decides the
 * architecture is whether a TWIN-derived forecast beats the FITTED one at the leads the QP uses.
 *
 * The two are asked different questions and the asymmetry favours the regression, which is what
 * makes a twin win decisive: the readout predicts e[k+L] from a lag window of MEASURED history up
 * to k, while the twin is simulated open loop FROM t=0 with no measurements at all. A simulation
 * also predicts every lead in one pass, so its R² is FLAT in L where the regression's decays —
 * that shape is the finding, not a detail. The twin runs at its LM-FITTED parameters, never at
 * truth: at truth it is the same lattice code as the machine and would win trivially (rule 15).
 *
...
// The twin at its LM-FITTED parameters, driven open loop over the same program through the SAME
// routing the pilot's records use (rule 61: one routing, or the comparison is between two).
// These four numbers are what `twin.test.mjs` phase 4 identifies on this cell — K 1 / E 0.06,
// true damping 3e-3 and backlash 1e-4 — so the backlash is 1.6x out, which is the model error a
// real deployment carries and the reason this is not a truth-parameter oracle.
const TWIN_FIT = { K: 0.9993, E: 0.05997, damp: 3.00e-3, bl: 6.08e-5 };
async function recordTwin(pilot, shape, feed) {
  const { arm, servo } = await makeArm(TWIN_FIT);
  ...
  for (let k = 0; k < total; k++) {
    ...
    if (k % S === 0) e.push(routeSignals(arm, [{ pos: q1 }, { pos: q2 }], tau).truth);
  }
  await arm.l1.destroy(); await arm.l2.destroy();
  return e;
}

/** The twin's R2 on the SAME rows, at the same lead, against the same truth. */
function scoreTwin(pilot, rec, twinE, c, li) {
  ...
  const to = Math.min(rec.e.length, twinE.length) - L - 1;
  let sse = 0, n = 0, sy = 0, sy2 = 0;
  for (let k = from; k < to; k++) {
    const y = rec.e[k + L][c], p = twinE[k + L][c];
...
console.log(`\n  ${'program'.padEnd(18)} ${'ch'.padStart(3)}`
  ...
  + `  ${'twin l0'.padStart(9)} ${'twin mid'.padStart(9)} ${'twin far'.padStart(9)}`
  + `  ${'truth rms'.padStart(10)} ${'fit resid'.padStart(10)} ${'twin resid'.padStart(10)}`);
for (const shape of SHAPES) {
  for (const feed of FEEDS) {
    const rec = await recordOpenLoop(pilot, shape, feed);
    const twinE = await recordTwin(pilot, shape, feed);
    for (let c = 0; c < 2; c++) {
      const idx = [0, Math.floor(pilot.N / 2), pilot.N - 1];
      const s = idx.map((i) => score(pilot, rec, c, i));
      const t = idx.map((i) => scoreTwin(pilot, rec, twinE, c, i));
      console.log(`  ${`${shape} @${feed}`.padEnd(18)} ${String(c).padStart(3)}`
        ...
        + `  ${t[0].r2.toFixed(3).padStart(9)} ${t[1].r2.toFixed(3).padStart(9)}`
        + ` ${t[2].r2.toFixed(3).padStart(9)}`
        + `  ${s[0].truthRms.toExponential(2).padStart(10)}`
        + ` ${s[0].rms.toExponential(2).padStart(10)}`
        + ` ${t[0].rms.toExponential(2).padStart(10)}`);
    }
  }
}
