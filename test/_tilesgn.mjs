...
for (let round = 0; round < ROUNDS; round++) {
  // draw directions: 3/4 residual-weighted hats, 1/4 smooth
  const dirs = [];
  for (let i = 0; i < M; i++) {
    if (i % 4 === 3) dirs.push(smoothDir(128));
    else { const { p, c } = sampleBin(R0); dirs.push(hatDir(p, c)); }
  }
  // central-difference response columns
  const C = [];
  for (const D of dirs) {
  ...
  // solve min ||R0 + C a||^2 + lam ||a||^2  (normal equations, MxM)
  ...
  let tr = 0;
  for (let i = 0; i < M; i++) tr += G[i][i];
  const lam = 1e-3 * tr / M;
  const A = G.map((row, i) => { const r = Array.from(row); r[i] += lam; return r; });
  const rhs = Array.from(g);
  for (let c = 0; c < M; c++) {
    let piv = c;
    for (let i = c + 1; i < M; i++) if (Math.abs(A[i][c]) > Math.abs(A[piv][c])) piv = i;
  ...
  for (let i = M - 1; i >= 0; i--) {
    let v = rhs[i];
    for (let j = i + 1; j < M; j++) v -= A[i][j] * alpha[j];
    alpha[i] = v / A[i][i];
  }
  ...
  for (let k = 0; k < BINS * nCh; k++) {
    let v = R0[k];
    for (let i = 0; i < M; i++) v += C[i][k] * alpha[i];
    pr2 += v * v;
  }
  ...
  console.log(`round ${round}: binned residual ${r0Bin.toExponential(3)} -> predicted ${predBin.toExponential(3)} (linear model)`);
  // backtracking step
  let took = false;
  for (const sc of [1, 0.5, 0.25, 0.1]) {
    const Tn = T.map((r, p) => {
      let d0 = 0, d1 = 0;
      for (let i = 0; i < M; i++) { d0 += alpha[i] * dirs[i][p * nCh]; d1 += alpha[i] * dirs[i][p * nCh + 1]; }
