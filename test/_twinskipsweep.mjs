...
for (const it of BUDGETS) {
  ...
  const r = await compileTwin({
    ...
    // THE OBJECTIVE IS THE PROGRAM, NOT THE PRE-ROLL (plan §45). Measured at skip 0 the
    // pre-roll carried 74% of the energy and the delivered gain peaked at 5 iterations
    // while the sim rms kept improving; this scores what the machine is scored on.
    skip: Number(process.env.SKIP_PRE || 0) ? PRE : 0,
  });
