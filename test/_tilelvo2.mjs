...
// improvement that lives only in the 4-lap binning shows up as a gap here
{
  ...
  let s2 = 0, n2 = 0;
  for (let s = start8; s < e8.length; s++) for (let c = 0; c < nCh; c++) { s2 += e8[s][c] ** 2; n2++; }
  evals += 2;
  console.log(`cycle ${cycle} holdout (8 sim laps): ${Math.sqrt(s2 / n2).toExponential(4)}  vs 4-lap f ${f0.toExponential(4)}`);
}
}
console.log(`LVO2 tile: ${f0.toExponential(4)}   (shipped 3.12e-3, 50x needs 2.7e-3, floor ~4.3e-4)`);
...
const score = async (duF, laps) => {
  ...
  const rows = []; let cAcc = 0, cn = 0, lapNo = 0, jAcc = 0;
  for (let k = 0; k < total; k++) {
    const now = Math.floor(k / path.lap);
    if (now !== lapNo) { rows.push(Math.sqrt(cAcc / cn)); cAcc = 0; cn = 0; jAcc = 0; lapNo = now; }
    const c = path.at(k);
  ...
  if (cn > 10) { rows.push(Math.sqrt(cAcc / cn)); rows.jointLast = Math.sqrt(jAcc / cn); }
  await destroy(m);
  return rows;
};
...
console.log(`delivered joint rms (last lap): ${got.jointLast.toExponential(3)}  vs sim tile f ${f0.toExponential(3)} — the transfer instrument`);
