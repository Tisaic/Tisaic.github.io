...
async function score(prog, trim) {
  ...
  const lap = () => { const ss = [0, 0]; for (let k = 0; k < L; k++) { mm.meas(o); const r = prog.at(k); if (k >= d) for (let c = 0; c < 2; c++) ss[c] += (o[c] - r[c]) ** 2; const u = trim ? trim(k) : [0, 0]; mm.step([r[0] + u[0], r[1] + u[1]]); } return ss.map((x) => Math.sqrt(x / (L - d))); };
  lap(); return lap();
}
const out = [];
