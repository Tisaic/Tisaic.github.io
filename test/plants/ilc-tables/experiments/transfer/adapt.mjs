...
const WORK = [['circle', 3e-3], ['sharp', 3e-3], ['rounded', 2e-3], ['circle', 2e-3]].map(([s, f]) => ({ name: `${s}-${f}`, prog: benchProgram(s, f, ik) }));
...
for (const tau of TAUS) {
  ...
  for (let wi = 0; wi < WORK.length; wi++) {
    ...
    const cmd = new Float64Array(2 * 2 * L), yy = new Float64Array(2 * 2 * L), gr = (i, c) => prog.at(i)[c];
    const laps = [];
