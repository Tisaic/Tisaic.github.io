...
import { benchProgram, ikOf, buildArm, BENCH } from '../../../../../lib/flexisim/bench.js';
...
const PROGS = ['circle', 'rounded', 'sharp'].flatMap((s) => [3e-3, 2e-3].map((f) => ({ name: `${s}-${f}`, prog: benchProgram(s, f, ik) })));
...
const CLASSES = {
  ...
  densePhys: { offs: Array.from({ length: 2 * Math.round(R / (R / 75)) + 1 }, (_, i) => Math.round(-R + i * (R / 75))), pose: false, phys: true },
};
function feats(cls, g, k, out) { // g(i, c) -> signal; level + deltas + bias, optionally x [1, cos q2, sin q2]
  ...
  if (cls.phys) base.push(Math.cos(g0[0]), Math.sin(g0[0]), Math.cos(g0[0] + g0[1]), Math.sin(g0[0] + g0[1]));
...
// ---- deploy on each program: bare laps vs trimmed laps, joint rms after a 5% drop (the ILC tables' metric)
async function score(prog, trim) {
  const mm = await arm2r.make(prog), L = prog.lap, d = Math.ceil(0.05 * L);
  const tool = new Float64Array(2 * L); let tss = 0;
  const lap = (last) => { const ss = [0, 0]; for (let k = 0; k < L; k++) { mm.meas(o); const r = prog.at(k); if (k >= d) for (let c = 0; c < 2; c++) ss[c] += (o[c] - r[c]) ** 2; const u = trim ? trim(k) : [0, 0]; mm.step([r[0] + u[0], r[1] + u[1]]);
    if (last) { mm.meas(o); } } return ss.map((x) => Math.sqrt(x / (L - d))); };
  lap(false); const j = lap(true);
  return j;
}
const out = [];
for (const { name, prog } of PROGS) {
  const bare = await score(prog, null), row = [name.padEnd(14)];
  const gr = (i, c) => prog.at(i)[c];
  for (const [cn, list] of Object.entries(models)) {
    let best = null;
    for (const md of list) {
      const f = new Float64Array(md.nf); const U = new Float64Array(2 * prog.lap);
      for (let k = 0; k < prog.lap; k++) { feats(md.cls, gr, k, f); for (let c = 0; c < 2; c++) { let v = 0; for (let i = 0; i < md.nf; i++) v += md.w[c][i] * f[i]; U[2 * k + c] = v; } }
      const r = await score(prog, (k) => [U[2 * k], U[2 * k + 1]]);
      const x = 1 / Math.sqrt(((r[0] / bare[0]) ** 2 + (r[1] / bare[1]) ** 2) / 2);
      if (!best || x > best.x) best = { x, lam: md.lam };
    }
    row.push(`${cn} ${best.x.toFixed(2)}x`);
  }
