...
// ---- FF1: the STATIC pose map, measured with the arm held at a grid of poses (settled)
import('../../../../../lib/flexisim/approach.js');
const { driveTo } = await import('../../../../../lib/flexisim/approach.js');
const { calibrateComp, conventional } = await import('../../../../../lib/flexisim/bench.js');
const GM = +(process.env.GM || 7), Q1 = [-1.3, -0.25], Q2 = [1.5, 2.6], SETTLE = 12000, MAP = new Float64Array(2 * GM * GM);
{ const mh = await buildArm(), rch = await calibrateComp(mh), ch = conventional(mh, rch);
  for (let i = 0; i < GM; i++) for (let j = 0; j < GM; j++) { const q = [Q1[0] + (Q1[1] - Q1[0]) * i / (GM - 1), Q2[0] + (Q2[1] - Q2[0]) * j / (GM - 1)];
    await driveTo(mh.arm, mh.servo, q, BENCH.feed); ch.reset(q); let s0 = 0, s1 = 0, n = 0;
    for (let k = 0; k < SETTLE; k++) { ch.step(q); if (k >= SETTLE - 3000) { const t = mh.arm.toolXY(), y = ik(t[0], t[1]); s0 += y[0] - q[0]; s1 += y[1] - q[1]; n++; } }
    MAP[2 * (i * GM + j)] = s0 / n; MAP[2 * (i * GM + j) + 1] = s1 / n; } }
const staticErr = (q, c) => { const x = (q[0] - Q1[0]) / (Q1[1] - Q1[0]) * (GM - 1), yv = (q[1] - Q2[0]) / (Q2[1] - Q2[0]) * (GM - 1);
  const i = Math.max(0, Math.min(GM - 2, Math.floor(x))), j = Math.max(0, Math.min(GM - 2, Math.floor(yv))), fx = Math.max(0, Math.min(1, x - i)), fy = Math.max(0, Math.min(1, yv - j));
  const v = (a, b2) => MAP[2 * (a * GM + b2) + c]; return (1 - fx) * (1 - fy) * v(i, j) + fx * (1 - fy) * v(i + 1, j) + (1 - fx) * fy * v(i, j + 1) + fx * fy * v(i + 1, j + 1); };
...
const STATIC = { static: true };                     // FF1 marker: -map(reference)
models.static = [STATIC];
{ const rs = (k) => { const t = tgt(k), q = [PY.q[2 * k], PY.q[2 * k + 1]]; return [t[0] + staticErr(q, 0), t[1] + staticErr(q, 1)]; };
  models.staticDyn = [STATIC, fit([PH, { ...GE, w: 0.3 }], 1e-5, rs)]; }
delete models.phys; delete models.generic;
...
for (const { name, prog } of WORK) {
  ...
  for (const [mn, stack] of Object.entries(models)) {
    const U = new Float64Array(2 * L); for (let k = 0; k < L; k++) for (const md of stack) { const u = md.static ? [-staticErr(prog.at(k), 0), -staticErr(prog.at(k), 1)] : predict(md, pp, k, true); U[2 * k] += u[0]; U[2 * k + 1] += u[1]; }
