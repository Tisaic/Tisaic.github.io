...
let E = r0.e; const cs0 = cornerSignatures(prog, tool, { smooth: BENCH.JERK });
const ROUGH = +(process.env.ROUGH || 0.02), SPREAD = cs0.spread;
let bestRms = r0.rms, beta = [0.5, 0.5], ci = [0, 0], rej = 0;
for (let it = 0; it < IT; it++) {
  const Un = U.slice();
  for (let cc = 0; cc < 2; cc++) { const [tau, W] = CANDS[ci[cc]]; const d = new Float64Array(2 * L); for (let k = 0; k < L; k++) d[2 * k + cc] = U[2 * k + cc] - beta[cc] * E[2 * ((k + tau) % L) + cc]; const q = Qc(d, cc, W); for (let k = 0; k < L; k++) Un[2 * k + cc] = q[k]; }
  const rn = run(Un), cs = cornerSignatures(prog, tool, { smooth: BENCH.JERK });
  const ok = rn.rms < bestRms * 0.99 && cs.rough <= Math.max(ROUGH, cs0.rough) && cs.spread <= SPREAD;
  if (ok) { U = Un; E = rn.e; bestRms = rn.rms; rej = 0; }
  else { beta = beta.map((b) => b * 0.5); if (++rej >= 3) { ci = ci.map((c) => (c + 1) % CANDS.length); rej = 0; beta = beta.map((b) => Math.min(0.5, 4 * b)); } }
  process.stdout.write(`  it ${it} ${ok ? 'kept' : 'rej '} rms ${rn.rms.toExponential(2)} rough ${(100 * cs.rough).toFixed(1)}% spread ${(100 * cs.spread).toFixed(0)}%\n`);
}
