...
// ONE pass over the open program — there is no lap to repeat, so `laps` is 1 and the
// tail-tiling accessor is not used at all: the artifact IS the program.
console.log('compiling the open program (no laps, no tiling)…');
const res = await compileTwin({
  simulate: sims.compileSim(openPath, { laps: 1, preRoll: PRE }),
  H, iters: 11,
  onProgress: (m) => console.log(`  ${m}`),
});
const tComp = (Date.now() - t0) / 60000;
console.log(`compiled: sim rms ${res.report.rms.toExponential(3)} from ${res.report.openRms ? res.report.openRms.toExponential(3) : 'n/a'}, ${tComp.toFixed(1)} min wall`);
...
const got = await score(res.du);
console.log(`open loop  : joint rms ${open.joint.toExponential(3)} over ${open.n} samples`);
console.log(`compiled ⑩ : joint rms ${got.joint.toExponential(3)} over ${got.n} samples (pre-roll excluded)`);
console.log(`gain over open: ${(open.joint / got.joint).toFixed(1)}x   (closed sharp square at this cell: 44.0x with the tile refine, compile alone below that)`);
console.log('EXIT 0');
