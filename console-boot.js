...
// errors; the page sets window.__BUILD (stamped) beforehand for the version line.
  (function () {
    ...
    // A STALE CACHED MODULE IS INVISIBLE TO THE VERSION CHECK, which is how this
    // reached the owner's phone: the stale banner busts the HTML's own URL with a
    // ?v= query, but the app is a MODULE GRAPH and `import './lib/.../x.js'`
    // carries no query, so the browser happily pairs a brand-new page with a
    // cached module from a previous build. The version line then reads "✓ latest"
    // -- truthfully, about the HTML -- while the page is dead, and the only
    // symptom is a SyntaxError naming an export that exists in the repo.
    // These messages are the browser's own wording for exactly that mismatch.
    var MODULE_MISS = /does not provide an export named|failed to (fetch|resolve) (dynamically imported )?module|error loading dynamically imported module|Cannot find module|Failed to load module script/i;

    window.addEventListener("error", function (e) {
      if (e && e.message) {
        record("error", e.message + (e.filename ? ("  @ " + e.filename + ":" + e.lineno + ":" + e.colno) : "") +
                        (e.error && e.error.stack ? "\n" + e.error.stack : ""));
        if (MODULE_MISS.test(e.message)) showModuleStale(e.message);
    ...
    function checkVersion() {
      ...
      fetch(url, { cache: "no-store" })
        ...
        .then(function (latest) {
          ...
          } else if (moduleStaleShown) {
            // The document IS current. Saying "✓ latest" here is true and useless:
            // it is the reading that made this failure invisible in the first place.
            paintModuleStale();
          } else {
    ...
    /**
     * Reload, and REFRESH THE MODULE GRAPH TOO, not just this document.
     *
     * A ?v= query on the page's own URL is enough to re-fetch the HTML and nothing
     * else: every `import './lib/.../x.js'` inside it is an unqueried URL that the
     * browser serves from cache. `fetch(u, {cache:"reload"})` bypasses the cache
     * AND writes the fresh copy back into it, so the modules the reloaded page then
     * imports are the new ones. modules.json is generated by stamp-version.sh, so
     * the list cannot drift from what actually ships; if it is missing (an
     * unstamped dev checkout) this degrades to the plain reload it used to be.
     */
    function reloadFresh() {
      var base = location.href.split("#")[0].split("?")[0];
      var go = function () { location.href = base + "?v=" + (new Date().getTime()); };
      if (!window.fetch) { go(); return; }
      fetch("modules.json?t=" + (new Date().getTime()), { cache: "no-store" })
        .then(function (r) { return r.ok ? r.json() : Promise.reject(r.status); })
        .then(function (m) {
          var list = (m && m.modules) || [];
          record("log", "Refreshing " + list.length + " module(s) past the cache before reloading");
          return Promise.all(list.map(function (u) {
            return fetch(u, { cache: "reload" }).catch(function () { /* one miss must not block */ });
          }));
        })
        .catch(function () { /* no manifest: reload the document alone */ })
        .then(go, go);
    }

    /**
     * The page loaded but a module did not match it. Offer the same recovery the
     * stale banner offers, because it IS the same recovery -- and say which, since
     * "reload" without a reason is what the owner already tried twice.
     */
    var moduleStaleShown = false;
    function showModuleStale(msg) {
      if (moduleStaleShown) return;
      moduleStaleShown = true;
      record("warn", "Stale MODULE, not a stale page: " + msg + " — the document is current but a "
        + "cached script is not. Tap the banner; a plain reload will not fix it.");
      // THE BANNER CAN BE RAISED BEFORE THE UI EXISTS. This bootstrap runs first in
      // <head> precisely so it catches load-time errors, and a module failure is one
      // -- so painting straight into `els` would throw INSIDE the error handler and
      // lose the very report it was called to make. paintModuleStale() is therefore
      // idempotent and mount() calls it too.
      paintModuleStale();
    }

    function paintModuleStale() {
      if (!moduleStaleShown || !els.build || !els.stale) return;
      setBuildText("v" + BUILD.version + " · ⚠ STALE MODULE — the page is newer than a cached script", "stale");
      els.staleText.textContent = "A cached script does not match this page — tap to reload properly";
      els.stale.style.display = "block";
    }
    ...
    function mount() {
      ...
      checkVersion();
      // A module error may already have fired before this ran; the banner it asked
      // for has to survive that ordering, and checkVersion() must not overwrite it.
      paintModuleStale();
    }
